...
**Current product state (2026-07-10):** shipped runtime is the native Swift
...
product decisions visible rather than reporting them as implemented.
...
| 字段 / 环境变量 | 当前状态 |
|------|----------|
| `ENGRAM_EMBEDDING_API_KEY` / `embeddingApiKey` / `aiApiKey` | 用于 service embedding 与 summary；敏感值通过 Keychain 存储，旧明文设置会在校验成功后迁移 |
