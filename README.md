...
- **网络设置**：保留旧配置字段的只读兼容说明；Swift service 不启用 peer sync
...
Swift service 目前未实现 peer sync。macOS App 不会启用或触发 sync；旧版 TypeScript peer-sync engine/API 已删除。配置文件可能仍保留 peer-sync 字段供旧版本降级或迁移读取，但当前产品运行路径不使用它们。
