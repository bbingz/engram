...
**Current product state (2026-07-12):** shipped runtime is the native Swift
...
product decisions visible rather than reporting them as implemented. There is
one active, not-deployed delivery: exact-source dual-replica archive v2; its
3 conditional archive-v2 follow-ups are outside the zero-delete first release.
...
会话详情只把 `user` / `assistant` 消息作为可见正文展示；tool/event/system-like 数据用于索引、工具统计或诊断，不作为普通对话气泡混入正文。分页 offset 按 adapter 原始消费位置推进，避免隐藏消息造成可见消息漏页或重复页。
...
- **价值提示**：索引时计算 `quality_score`；搜索结果页可显示高/中/低 value band（会话卡片徽章仍为 roadmap decision-pending）
...
Engram 的 Swift MCP runtime 当前暴露 27 个工具，覆盖上下文获取、搜索、记忆管理、统计分析、会话整理和项目迁移等场景：

| 工具 | 说明 |
|------|------|
| `get_context` | 🔑 **核心工具** — 自动提取当前项目的历史上下文，开始新任务时调用 |
| `search` | FTS 关键词搜索；当 session vectors 可用时 tools/list 可广告 `semantic`/`hybrid`，不可用时 hard-error（不静默降级）。Service IPC 路径可 soft-fallback 到 keyword 并带 reason |
...
当前 Swift 产品路径使用 SQLite FTS5 trigram 关键词搜索，并在配置了 embedding provider 且 `semantic_chunks` 可用时提供 brute-force cosine semantic/hybrid 检索（**不是** sqlite-vec）。

- **keyword**：已上线。覆盖用户消息、助手回复、摘要和索引文本；支持中英文、代码片段、函数名、UUID 直查。
- **semantic / hybrid（service）**：可用时执行向量 KNN + 可选 RRF；不可用时 soft-fallback 到 keyword 并返回 warning reason。
- **semantic / hybrid（MCP）**：tools/list 按 `SessionVectorSearchAvailability` 门控；不可用时 `searchModeUnavailable` hard-error（**不**静默降级为 keyword 结果体）。
- **App UI**：故意 keyword-only（mode pills 不暴露 semantic）；不代表 service/MCP 无向量路径。
- **insights memory**：`get_memory` 在有 insight embeddings 时走 hybrid，否则 FTS/recent fallback。
...
| 字段 / 环境变量 | 当前状态 |
|------|----------|
| `ENGRAM_EMBEDDING_API_KEY` / `embeddingApiKey` / `aiApiKey` | 用于 service embedding 与 summary；签名产品中新值使用 Keychain + `@keychain` marker。旧明文仅在 Keychain 写入、回读校验和 settings 重写全部成功后移除；失败时保留可恢复明文 |
| `embeddingModel` / dimension | 写入 `embedding_meta`；query 须与存储 model+dim 一致 |
| `session_embeddings` / `semantic_chunks` | 产品 brute-force KNN 语料；`vec_sessions` 为历史兼容残留 |
...
历史 TypeScript Web/API 入口已删除；当前产品路径优先使用 Swift MCP 工具。macOS App 暂未提供 Project Aliases 管理 UI。
...
精确源归档 v2 与上面的 `/v1/bundles` 卸载不是同一条数据链：它在解析前保存
Claude Code / Codex 已证明可重放的单文件原始字节，并直接复制到
`macmini-hq` 和异地 `macmini-m1`。首版为 default-off、Tailscale-only、
zero-delete：不删除源文件、本地归档或远端对象，也不提供 GC。当前代码仍是未部署的
交付分支；完整边界、配置和恢复前提见
...
- **索引库位置：** `~/.engram/index.sqlite`，存储元数据、FTS，以及可选但当前写入和查询的
  `semantic_chunks`、`insight_embeddings` 和 `embedding_meta`。配置可用 embedding
  provider 后，service/MCP 会使用这些表；App UI 仍故意保持 keyword-only，不能据此把
  semantic 存储描述成未来兼容占位
- **设置与密钥：** 签名产品中新 secret 使用 Keychain + `@keychain` marker；旧的明文
  `embeddingApiKey` 或 `aiApiKey` 只有在 Keychain 写入、回读和 settings 重写全部成功后
  才移除，迁移失败时保留可恢复明文。应通过签名 App 的 Settings 完成迁移并在本地确认
  marker，不要打印、记录或粘贴密钥
- **原始文件：** 默认从 AI 工具的原始日志文件流式读取；只有显式启用精确源归档 v2
  后，受支持的 Claude Code / Codex 单文件源才会进入本地不可变归档，并可进一步复制
  到两台自托管 Mac mini
- **隐私脱敏：** 导出和 Web 预览使用内置敏感内容脱敏；索引库按原始本地日志建立 FTS，不支持用户配置的索引时正则替换
- **数据不离本机（默认）：** `EngramService`、`EngramMCP` 和 macOS App 本地运行；
  配置远程 AI 摘要/标题、embedding provider、legacy remote offload 或精确源归档 v2
  后，相应功能才会发送数据。可用 embedding 配置会让初始/周期 backfill 把
  **session chunks and insight content** 发到配置的 `/embeddings`，semantic/hybrid
  查询会发送 **semantic/hybrid query text**；embedding key 未单独配置时可能复用
  `aiApiKey`。没有可用 embedding provider 时不会发送这些内容，关键词搜索仍为本地路径
