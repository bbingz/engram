...
final class ObservabilityRetentionTests: XCTestCase {
    ...
    private func makeActiveRetentionSchema(_ db: Database) throws {
        try db.execute(sql: """
    ...
    private func makeDormantLegacyObservabilitySchema(_ db: Database) throws {
        try db.execute(sql: """
    ...
    func testPrunesOldKeepsRecentAndCounts() throws {
        ...
        try queue.write { db in
            try self.makeActiveRetentionSchema(db)
        ...
        XCTAssertEqual(deleted, 2, "one old row per active retention table")
    ...
    func testPruneLeavesDormantLegacyObservabilityTablesUntouched() throws {
        let path = tmpPath()
        let queue = try DatabaseQueue(path: path)
        let now = Date()
        ...
        try queue.write { db in
            try self.makeActiveRetentionSchema(db)
            try self.makeDormantLegacyObservabilitySchema(db)
            try db.execute(sql: "INSERT INTO metrics (name,type,value,tags,ts) VALUES ('m','counter',1,NULL,?)", arguments: [old])
            try db.execute(sql: "INSERT INTO traces (trace_id,span_id,name,module,start_ts,source) VALUES ('t','a','n','m',?,'daemon')", arguments: [old])
            try db.execute(sql: "INSERT INTO logs (ts,level,module,message,source) VALUES (?,'info','m','x','daemon')", arguments: [old])
        }
    ...
    func testPruneRespectsBatchLimitAndLoopDrains() throws {
        ...
        try queue.write { db in
            try self.makeActiveRetentionSchema(db)
            for _ in 0..<5 {
                try db.execute(sql: "INSERT INTO ai_audit_log (ts,caller,operation) VALUES (?,'c','o')", arguments: [old])
            }
        }
        ...
        try queue.read { db in
            XCTAssertEqual(try Int.fetchOne(db, sql: "SELECT COUNT(*) FROM ai_audit_log"), 0)
        }
    }
