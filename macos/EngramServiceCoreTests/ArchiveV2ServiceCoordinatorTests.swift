...
final class ArchiveV2ServiceCoordinatorTests: XCTestCase {
    ...
    func testBacklogPassUsesIndependentBudgetsAndBacklogReplication() async throws {
        ...
        XCTAssertEqual(
            recordedEvents,
            [
                "replicateBacklog:16", "backlogCapture", "targets", "historical",
                "snapshot",
            ]
        )
    }
    ...
    func testBacklogPassAlternatesRemoteAndLocalAdmissionOrder() async throws {
        ...
        let firstPass = await events.values()
        _ = try await coordinator.runBacklogPass(adapters: [])
        let allEvents = await events.values()
        let secondPass = Array(allEvents.dropFirst(firstPass.count))

        XCTAssertEqual(
            firstPass,
            ["replicate", "capture", "targets", "historical", "snapshot"]
        )
        XCTAssertEqual(
            secondPass,
            ["capture", "targets", "historical", "snapshot", "replicate"]
        )
    }

    func testRemoteFirstDeadlineYieldsLocalAdmissionOnNextPass() async throws {
        let harness = try makeHarness(remoteReady: true)
        let events = EventLog()
        ...
        operations.replicateBacklog = { _, _ in
            await events.append("replicate")
            if await events.count(of: "replicate") == 1 {
        ...
        XCTAssertEqual(firstPass, ["replicate"])
        XCTAssertEqual(
            secondPass,
            ["capture", "targets", "historical", "snapshot", "replicate"]
        )
    ...
    func testLocalFirstDeadlineYieldsRemoteAdmissionOnFollowingPass() async throws {
        let harness = try makeHarness(remoteReady: true)
        let events = EventLog()
        let clock = CoordinatorClock(Date(timeIntervalSince1970: 100))
        var operations = makeOperations(events: events)
        operations.backlogCapture = { _, _, _ in
            await events.append("capture")
            if await events.count(of: "capture") == 2 {
                clock.set(Date(timeIntervalSince1970: 111))
            }
            return ArchiveV2ServiceCaptureSummary(
                unsupported: 0,
        ...
        let firstPass = await events.values()
        _ = try await coordinator.runBacklogPass(adapters: [])
        let throughSecondPass = await events.values()
        let secondPass = Array(throughSecondPass.dropFirst(firstPass.count))
        _ = try await coordinator.runBacklogPass(adapters: [])
        let allEvents = await events.values()
        let thirdPass = Array(allEvents.dropFirst(throughSecondPass.count))

        XCTAssertEqual(
            firstPass,
            ["replicate", "capture", "targets", "historical", "snapshot"]
        )
        XCTAssertEqual(secondPass, ["capture"])
        XCTAssertEqual(
            thirdPass,
            ["replicate", "capture", "targets", "historical", "snapshot"]
        )
    ...
    func testThrownLocalStageDoesNotPinFollowingPassAwayFromRemote() async throws {
        let harness = try makeHarness(remoteReady: true)
        let events = EventLog()
        var operations = makeOperations(events: events)
        operations.backlogCapture = { _, _, _ in
        ...
        operations.bindingTargets = { _ in
            ...
            if await events.count(of: "bind") == 2 {
        ...
        do {
            _ = try await coordinator.runBacklogPass(adapters: [])
            XCTFail("expected local binding failure")
        ...
        XCTAssertEqual(
            firstPass,
            ["replicate", "capture", "bind", "historical", "snapshot"]
        )
        XCTAssertEqual(secondPass, ["capture", "bind"])
        XCTAssertEqual(
            thirdPass,
            ["replicate", "capture", "bind", "historical", "snapshot"]
        )
    ...
    func testDisabledDerivedLocatorIsParkedInsteadOfRetriedEveryFiveMinutes_repro() async throws {
        ...
        let excludedSources = CoordinatorSourceSetBox([.lobsterai])
        ...
        excludedSources.set([])
        let previousSettingsPath: String?
        if let pointer = getenv("ENGRAM_SETTINGS_PATH") {
            previousSettingsPath = String(cString: pointer)
        } else {
            previousSettingsPath = nil
        }
        setenv("ENGRAM_SETTINGS_PATH", harness.settingsURL.path, 1)
        defer {
            if let previousSettingsPath {
                setenv("ENGRAM_SETTINGS_PATH", previousSettingsPath, 1)
            } else {
        ...
        let enableResponse = await handler.handle(
        ...
        guard case .success = enableResponse else {
            return XCTFail("setSourceEnabled must resume the parked ArchiveV2 locator")
        }
        let third = try await coordinator.runBacklogPass(
            adapterProvider: { [adapter] },
            excludedSnapshotSourcesProvider: { excludedSources.value() }
        )
        let thirdIndexCount = await events.count(of: "backlogIndex")
        let indexedSource = try await DatabaseQueue(path: harness.database.path).read { db in
            try String.fetchOne(
                db,
                sql: "SELECT source FROM sessions WHERE id = ?",
                arguments: ["backlog-derived-disabled-parked-session"]
            )
        }

        XCTAssertEqual(thirdIndexCount, 2, "re-enable must reparse the parked locator once")
        XCTAssertEqual(indexedSource, SourceName.lobsterai.rawValue)
        XCTAssertNil(third.nextRetryAt)
        XCTAssertFalse(third.hasRunnableWork)
    }

    /// R2.P1 archive-disabled-locator-churn: if enable wins after the index
    /// writer gate but before actor parking, the locator must stay immediately
    /// runnable instead of being stranded in the stale disabled state.
    func testDerivedSourceEnableBetweenIndexAndParkingKeepsLocatorDue_repro() async throws {
        let harness = try makeHarness(remoteReady: false, batchSize: 3)
        _ = try await emptyIndexResult(gate: harness.gate)
        let events = EventLog()
        let sourceURL = temporaryRoot("backlog-derived-enable-park-race")
            .appendingPathComponent("session.jsonl")
        try FileManager.default.createDirectory(
            at: sourceURL.deletingLastPathComponent(),
            withIntermediateDirectories: true
        ...
        let adapter = BacklogIndexingAdapter(
            ...
            events: events,
            outputSource: .lobsterai,
            sessionID: "backlog-derived-enable-park-race-session"
        )
        let excludedSources = CoordinatorSourceSetSequence([
            [.lobsterai],
            [],
        ])
        var operations = makeOperations(events: events)
        ...
        let first = try await coordinator.runBacklogPass(
            adapterProvider: { [adapter] },
            excludedSnapshotSourcesProvider: { excludedSources.next() }
        )
        XCTAssertTrue(first.hasRunnableWork)
        XCTAssertNil(first.nextRetryAt)
        ...
        let indexedSource = try await DatabaseQueue(path: harness.database.path).read { db in
            try String.fetchOne(
                db,
                sql: "SELECT source FROM sessions WHERE id = ?",
                arguments: ["backlog-derived-enable-park-race-session"]
            )
        }
    ...
    func testRequestDuringInitialFullCycleRemainsPendingForBacklogRefresh() async throws {
        let harness = try makeHarness(remoteReady: false)
        let events = EventLog()
        let gate = CoordinatorCaptureGate()
        var operations = makeOperations(events: events)
        operations.capture = { _, _, scope in
            XCTAssertEqual(scope, .full)
            await gate.enterAndWait()
            return ArchiveV2ServiceCaptureSummary(
                unsupported: 0,
                unsafe: 0,
        ...
        await coordinator.requestFullCaptureSweep()
        await gate.release()
        _ = try await initialCycle.value
        _ = try await coordinator.runBacklogPass(adapters: [])

        let recorded = await events.values().filter { $0.hasPrefix("backlogCapture:") }
        XCTAssertEqual(recorded, ["backlogCapture:true"])
    }

    func testBacklogAdapterProviderResolvesAfterWaitingForPipeline() async throws {
        let harness = try makeHarness(remoteReady: false)
        let events = EventLog()
        let captureGate = CoordinatorCaptureGate()
        let profileGeneration = CoordinatorIntBox(0)
        let providerProbe = CoordinatorAdapterProviderProbe()
        var operations = makeOperations(events: events)
        ...
        await captureGate.waitUntilEntered()
        let backlogPass = Task {
            try await coordinator.runBacklogPass(adapterProvider: {
                providerProbe.record(profileGeneration.value())
                return []
            })
        }
        try await Task.sleep(for: .milliseconds(20))
        let valuesWhileWaiting = providerProbe.values()
        XCTAssertEqual(valuesWhileWaiting, [])

        profileGeneration.set(1)
        await coordinator.requestFullCaptureSweep()
        await captureGate.release()
        _ = try await initialCycle.value
        _ = try await backlogPass.value

        XCTAssertEqual(providerProbe.values(), [1])
        let recorded = await events.values().filter { $0.hasPrefix("backlogCapture:") }
        XCTAssertEqual(recorded, ["backlogCapture:true"])
    }
    ...
    func testBacklogPassStopsBeforeNextStageWhenResourceConditionsChange() async throws {
        ...
        XCTAssertEqual(summary.capturedFiles, 1)
        XCTAssertTrue(summary.hasRunnableWork)
        XCTAssertEqual(recorded, ["replicate", "backlogCapture"])
    }
    ...
    func testAttentionPausedReplicaDoesNotHideHealthyReplicaRetryDeadline() async throws {
        ...
        operations.replicateBacklog = { _, _ in
            ArchiveReplicationCycleResult(pausedReplicaIDs: ["hq"])
        }
        operations.status = {
            let hq = ArchiveReplicaStatusCounts(
                pending: 1,
        ...
        XCTAssertFalse(summary.hasRunnableWork)
        XCTAssertEqual(summary.nextRetryAt, healthyRetryDeadline)
    }

    func testStatusPublishesAndRefreshesEffectiveReplicaPausesAndNextPriority() async throws {
        let harness = try makeHarness(remoteReady: true)
        let events = EventLog()
        let clock = CoordinatorClock(Date(timeIntervalSince1970: 100))
        ...
        operations.replicateBacklog = { _, _ in await results.next() }
        let coordinator = ArchiveV2ServiceCoordinator(
            settings: harness.settings,
            writerGate: harness.gate,
            remoteReady: true,
        ...
        XCTAssertEqual(initial.nextPassPriority, "remote")

        _ = try await coordinator.runBacklogPass(adapters: [])
        let afterReplication = await coordinator.status()
        XCTAssertEqual(
            afterReplication.replicas[0].pauseReason,
            "transientInfrastructureBackoff"
        )
        XCTAssertEqual(
            afterReplication.replicas[0].pausedUntil,
            "2026-07-13T11:01:00.000Z"
        )
        XCTAssertEqual(afterReplication.replicas[1].pauseReason, "needsAttention")
        XCTAssertNil(afterReplication.replicas[1].pausedUntil)
        XCTAssertEqual(afterReplication.nextPassPriority, "local")

        _ = try await coordinator.runBacklogPass(adapters: [])
        let afterDeadlineSkippedReplication = await coordinator.status()
        XCTAssertEqual(
            afterDeadlineSkippedReplication.replicas.map(\.pauseReason),
            ["transientInfrastructureBackoff", "needsAttention"]
        )
        XCTAssertEqual(afterDeadlineSkippedReplication.nextPassPriority, "remote")

        _ = try await coordinator.runBacklogPass(adapters: [])
        let afterFreshReplication = await coordinator.status()
        XCTAssertTrue(afterFreshReplication.replicas.allSatisfy {
    ...
    func testAcceptedManualRetryClearsOnlyRequestedReplicaPause() async throws {
        ...
        _ = try await coordinator.runBacklogPass(adapters: [])
        let response = await coordinator.retryQuarantined(replicaID: "hq")
        let status = await coordinator.status()

        XCTAssertTrue(response.accepted)
        XCTAssertNil(status.replicas[0].pauseReason)
...
private final class CoordinatorIntBox: @unchecked Sendable {
    private let lock = NSLock()
...
private final class CoordinatorAdapterProviderProbe: @unchecked Sendable {
    private let lock = NSLock()
...
private final class CoordinatorSourceSetSequence: @unchecked Sendable {
    private let lock = NSLock()
    private var values: [Set<SourceName>]

    init(_ values: [Set<SourceName>]) {
        precondition(!values.isEmpty)
        self.values = values
    }

    func next() -> Set<SourceName> {
        lock.withLock {
            if values.count > 1 {
