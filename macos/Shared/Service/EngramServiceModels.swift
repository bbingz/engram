...
struct EngramServiceSearchResponse: Codable, Equatable, Sendable {
    ...
    enum CodingKeys: String, CodingKey {
        case items, searchModes, warning, warningCode, insightResults
    }

    init(from decoder: Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
        items = try c.decode([Item].self, forKey: .items)
        searchModes = try c.decodeIfPresent([String].self, forKey: .searchModes)
        warning = try c.decodeIfPresent(String.self, forKey: .warning)
        warningCode = try c.decodeIfPresent(String.self, forKey: .warningCode)
        insightResults = try c.decodeIfPresent([Insight].self, forKey: .insightResults) ?? []
    }

    func encode(to encoder: Encoder) throws {
        var c = encoder.container(keyedBy: CodingKeys.self)
        try c.encode(items, forKey: .items)
        try c.encodeIfPresent(searchModes, forKey: .searchModes)
