...
public enum ImplementationDigestExtractor {
    ...
    private static func isMachineUserTurn(_ text: String) -> Bool {
        ...
        return trimmed.hasPrefix("# AGENTS.md instructions for ")
            || (trimmed as NSString).range(of: "<INSTRUCTIONS>").location != NSNotFound
            ...
            || containsAny(lower, ["caveat: the messages below were generated by the user while running local commands"])
    }

    private static func completionScore(_ text: String) -> Int {
        let lower = text.lowercased()
        let searchable = lower as NSString
        let options: String.CompareOptions = lower.canBeConverted(to: .ascii) ? .literal : []
        var score = 0
        for marker in ["**结果**", "## 结果", "### 结果", "结果\n", "最终状态", "改了什么", "验证结果", "checks run", "check_run", "checks_run"] {
            if searchable.range(of: marker.lowercased(), options: options).location != NSNotFound { score += 2 }
        }
        for marker in ["已完成", "完成", "已实现", "已修复", "已提交", "已推送", "已合并", "全绿", "success"] {
            if searchable.range(of: marker.lowercased(), options: options).location != NSNotFound { score += 1 }
        }
        let prefix = lower.prefix(80)
        for marker in ["我先", "我会", "我再", "正在", "先快速", "后台 watch", "收到，我先"] {
            if prefix.contains(marker.lowercased()) { score -= 1 }
        }
        if containsAny(lower, ["要试试吗", "请确认", "是否"]) {
            score -= 1
        }
        return score
    }
    ...
    private static func isOperationOnlyDirective(
        ...
        if containsAny(normalized, ["跑完了没", "ci 绿了就合并"]) {
            return true
        }
        if events == [.merged] || events == [.merged, .ciGreen] || events == [.ciGreen] {
            return true
        }
        if containsAny(normalized, ["清理"]) && events.contains(.cleanupOnly) && !containsAny(outcome.lowercased(), ["备份", "归档", "磁盘"]) {
            return true
        }
        return false
    }
    ...
    private static func containsAny(_ text: String, _ needles: [String]) -> Bool {
        // ASCII text needs no canonical normalization. Keep canonical matching
        // for Unicode while avoiding repeated character scans of long logs.
        let searchable = text as NSString
        let options: String.CompareOptions = text.canBeConverted(to: .ascii) ? .literal : []
        return needles.contains { searchable.range(of: $0.lowercased(), options: options).location != NSNotFound }
    }
}
