...
enum ObservabilityRetention {
    ...
    /// writer). Returns the total number of rows deleted. Table/column names are
    /// fixed literals; only the cutoff is bound.
    @discardableResult
    static func prune(_ db: Database, now: Date = Date(), config: Config = Config()) throws -> Int {
        ...
        func delete(_ table: String, _ column: String, _ days: Int) throws -> Int {
            try db.execute(sql: "DELETE FROM \(table) WHERE \(column) < ?", arguments: [cutoff(days)])
