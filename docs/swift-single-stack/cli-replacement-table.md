...
| `engram project move` | `src/cli/project.ts` calls project move orchestrator directly | Yes | Replaced operationally by service-backed MCP `project_move`; Swift CLI replacement not implemented | MCP service-unavailable and golden/project tests cover service routing, not CLI |
| `engram project archive` | `src/cli/project.ts` calls archive + project move directly | Yes | Replaced operationally by service-backed MCP `project_archive`; Swift CLI replacement not implemented | MCP service-unavailable and golden/project tests cover service routing, not CLI |
...
| `engram project undo` | `src/cli/project.ts` calls undo directly | Yes | Replaced operationally by service-backed MCP `project_undo`; Swift CLI replacement not implemented | MCP service-unavailable and golden/project tests cover service routing, not CLI |
...
| `engram project move-batch` | `src/cli/project.ts` calls batch orchestrator directly | Yes | Replaced operationally by service-backed MCP `project_move_batch`; Swift CLI replacement not implemented | MCP service-unavailable and golden/project tests cover service routing, not CLI |
...
- Stage 4 App and MCP routing can be considered service-backed after the current boundary scans pass.
