...
| `macos/Engram/Core/DaemonClient.swift` | Generic `POST`, `postRaw`, `DELETE` | Conflict: write-capable raw transport | Replace with typed `EngramServiceClient` | Raw daemon endpoint scan clean |
| UI relationship/project callers | `/api/sessions/*`, `/api/project/*` POST/DELETE | Conflict: mutations via Node daemon | Service commands with fail-closed behavior | Relationship and project operation tests |
| `macos/Engram/Core/EngramLogger.swift` | `POST /api/log` | Conflict: observability write via raw Node URL | Route through service event/log API or OSLog-only policy | Raw URL scan |
