...
**Original baseline:** `fix/audit-2026-07-18-r4plus` @ `2940e6c9`

**Closeout update:** 2026-07-19 blind-audit documentation based on PR #203 tip `62247c9e`

**Scope:** PR #181–#187 (available review evidence), #196 (merged), and #197 (open); #188–#195 were folded into #196 and are not double-counted.

**Method:** per-PR review evidence plus adversarial majority-vote verification; this document reports verified conclusions without inventing findings.
...
| #183 | merged | ACCEPTABLE_WITH_RESIDUAL_RISKS | 维护内存边界扎实；idle embedding starvation was later closed by EMB-STARVE TDD |
...
| #186 | merged | SOLID_MERGE_LOW_RESIDUAL | MCP 两工具 object-root；`manage_project_alias` list remains confirmed MCP-001 for D1 |
...
| #197 | **open** | **FRESH_CODEX_PASS** | R4 taxonomy blocker was corrected under TDD before commit/push; PR remains unmerged |

**#197 outcome:** the R4 scope was narrowed to explicit input-local rejection, the R1 documentation status was corrected, focused/runtime validation passed, and fresh Codex returned `PASS` before commit/push. R4 dual-tx and recovery ordered-stop remain separate follow-ups.
...
**Summary:** 嵌入 batch 限、archive 节流、policy 跳过、stale-status 与自适应扫描对齐。合并质量总体好；其历史残留 **merge-only `scan.indexed` × 周期嵌入门控** 曾导致 idle embedding backlog 饿死，后由 `2a919e79` 的 EMB-STARVE TDD 关闭。

### Findings（合入后仍相关）

| ID | Severity | 位置 | 说明 |
|----|----------|------|------|
| F1 / EMB-STARVE | **High（PR #183 历史残留；后续已关闭）** | `EngramServiceRunner` periodic gate；`SwiftIndexer.writeBatchCountingSuccesses` merge-only | 原 `scan.indexed > 0` 门控会饿死 idle pending/retry；`2a919e79` 已用独立 backlog 信号关闭 |
...
**Residual risks**
- F1 是 PR #183 tip 的历史 High，后由 `2a919e79` 关闭；不再列为当前风险
...
**Verdict:** **FRESH_CODEX_PASS** (original Grok: APPROVE_WITH_NITS; Codex taxonomy blocker corrected before commit/push)

**Title:** R4 insight embed terminalization, R5 reclaim budget cursor, R10/R11

**Branch baseline:** `fix/audit-2026-07-18-r4plus` @ `2940e6c9`
...
| 动作 | 当前状态 |
|------|----------|
| 提交/推送 #197 | **已完成门禁**：fresh Codex `PASS` 在 commit/push 前取得；PR 当前 open、非 draft、未 merge |
| pre-submit docs | **已完成**：taxonomy、runtime evidence、R1/R11 状态与提交 diff 对齐 |
| 后续小 PR | R4 成功+失败同一事务；可选 recovery ordered stop |
...
### 2. 三读表面漂移（R1）是贯穿主线
#196 SessionVisibilityFilter 只切了 list/KPI 一刀；#197 不处理 Read SQL。Post-review follow-ups now close **READ-001** (MCP multi-term session-scoped AND), **READ-002** (keyword/LIKE/semantic activity-time `since`), and **READ-003** (exact project-or-alias filtering across list and search modes). **ARCH-001 remains open** because the three Read SQL stacks are not unified in CoreRead.
...
### 4. 维护调度指标语义收窄的副作用
#183 将 `indexed` 收窄为 merge-only 后暴露 idle 嵌入饿死（F1）；后续 EMB-STARVE TDD 已将 session/insight backlog 信号与 merge-only 计数解耦并关闭该项。

### 5. MCP 合同局部修复
#186 object-root 只覆盖有 schema 的工具。2026-07-19 blind closeout 依据 MCP 2025-11-25 再次确认 **MCP-001 是协议违规**：`manage_project_alias action=list` 的 array-root `structuredContent` 不因缺少 `outputSchema` 而获得豁免。MCP-001 进入 D1；get_context/analytics visibility remains MCP-002.
...
### 7. Blind-spot closeout does not mean audit closure
The 2026-07-19 workflow completed 15/15 discovery scopes, explicitly named all 17 Swift sources, and adjudicated 23 canonical candidates into 22 confirmed and 1 refuted. Two current IDs retain document-scoped legacy references, while one duplicate MCP submission maps to MCP-001. The findings use 11 ownership batches (A1–D1): 21 are TDD-ready, while CURSOR-CWD-001 remains design/evidence pending until deterministic workspace ownership is established. Proposed `_repro` tests are not executed evidence; adapters with no retained finding and low-finding backup/restore, export, MCP framing, and undo/batch surfaces are not certified healthy.
...
1. #197's pre-submit blockers were corrected and a fresh Codex `PASS` was obtained before commit/push. Keep the stacked PR chain open for human merge control; do not auto-merge.
2. Execute the 21 implementation-ready blind findings in ownership order: A1 Codex, A2 Kimi, A3 Qwen, A4 adapter guardrails, B1 composite invalidation, B2 OpenCode, B3 Cursor content, C1a archive source toggle, C1b locator relocation, C1c parent-backfill pagination, and D1 MCP-001. Keep CURSOR-CWD-001 design/evidence pending until a deterministic workspace-ownership contract is established.
3. For every implementation-ready finding, add and run the `_repro` first, record RED, apply the minimum fix, run focused/full GREEN, then obtain a fresh Codex `PASS` before commit/push/PR. Never derive authoritative Cursor project state from an unrelated file selection.
4. Keep ARCH-001, MCP-002, SEC-001, UI-001/UI-002, R4-dual-tx, F5-CI, and R6–R9 visible as separate existing inventory; `readyToImplementAll = false` for the blind set and the scoped 21-item readiness does not close them or the repository-wide audit.
