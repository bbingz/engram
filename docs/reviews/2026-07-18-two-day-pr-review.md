...
2. **下一波工程优先（跨 PR）：**  
   - Shared Read predicates / cross-surface parity (ARCH-001); READ-001/002/003 are closed by post-review MCP follow-ups, while the shared CoreRead query module remains open
   - idle embedding drain 与 merge-only indexed 解耦（EMB-STARVE；post-review follow-up 已关闭，session + insight backlog `_repro` RED→GREEN）
   - reclaim product-missing 页 cursor 推进（SVC-001；post-review follow-up 已关闭并有 full-page `_repro`）
   - skip 时删除 `semantic_chunks` 并收紧 availability probe（EMB-001；post-review follow-up 已关闭，5 个 selected `_repro` RED→GREEN）
