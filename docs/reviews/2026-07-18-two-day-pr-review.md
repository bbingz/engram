...
### 2. 三读表面漂移（R1）是贯穿主线
#196 SessionVisibilityFilter 只切了 list/KPI 一刀；#197 不处理 Read SQL。Post-review follow-up 已用 executable `_repro` 关闭 **READ-001**（MCP multi-term session-scoped AND）和 **READ-002**（MCP keyword/LIKE/semantic `since` COALESCE）；**READ-003 + ARCH-001** 仍 open，三套 Read SQL 尚未统一到 CoreRead。
...
2. **下一波工程优先（跨 PR）：**  
   - 共享 Read 谓词 / 跨表面 parity（ARCH-001 族）；其中 READ-001/002 已由 post-review MCP follow-up 关闭，READ-003 与共享 CoreRead 模块仍 open
