# Finding disposition inventory

| ID | Area | Status | Batch/PR | Notes |
|----|------|--------|----------|-------|
| H1 | full-high | pending-fix | C | |
| H2 | full-high | pending-fix | C | |
| M1 | full-medium | pending-fix | D | |
| M2 | full-medium | pending-fix | D | |
| M3 | full-medium | pending-fix | D | |
| M4 | full-medium | pending-fix | D | |
| M5 | full-medium | pending-fix | D | |
| M6 | full-medium | pending-fix | D | |
| M7 | full-medium | pending-fix | D | |
| M8 | full-medium | pending-fix | D | |
| M9 | full-medium | pending-fix | D | |
| M10 | full-medium | pending-fix | D | |
| M11 | full-medium | pending-fix | D | |
| M12 | full-medium | pending-fix | D | |
| M13 | full-medium | pending-fix | D | |
| M14 | full-medium | pending-fix | D | |
| M15 | full-medium | pending-fix | D | |
| M16 | full-medium | pending-fix | D | |
| M17 | full-medium | pending-fix | D | |
| M18 | full-medium | pending-fix | D | |
| M19 | full-medium | pending-fix | D | |
| M20 | full-medium | pending-fix | D | |
| M21 | full-medium | pending-fix | D | |
| M22 | full-medium | pending-fix | D | |
| M23 | full-medium | pending-fix | D | |
| M24 | full-medium | pending-fix | D | |
| M25 | full-medium | pending-fix | D | |
| L1 | full-low | pending-fix | E |  |
| L2 | full-low | pending-fix | E |  |
| L3 | full-low | pending-fix | E |  |
| L4 | full-low | pending-fix | E |  |
| L5 | full-low | pending-fix | E |  |
| L6 | full-low | pending-fix | E |  |
| L7 | full-low | pending-fix | E |  |
| L8 | full-low | accepted-residual | E | Dead facade methods — remove or accept dead code (will fix by remove) |
| L9 | full-low | pending-fix | E |  |
| L10 | full-low | pending-fix | E |  |
| L11 | full-low | pending-fix | E |  |
| L12 | full-low | pending-fix | E |  |
| L13 | full-low | pending-fix | E |  |
| L14 | full-low | pending-fix | E |  |
| L15 | full-low | pending-fix | E |  |
| L16 | full-low | pending-fix | E |  |
| L17 | full-low | pending-fix | E |  |
| L18 | full-low | pending-fix | E |  |
| L19 | full-low | accepted-residual | E | TS CLI for tables Swift never populates — accepted TS-ref residual or delete |
| L20 | full-low | pending-fix | E |  |
| L21 | full-low | accepted-residual | E | Notarization manual-only documented — accepted residual |
| L22 | full-low | pending-fix | E |  |
| L23 | full-low | accepted-residual | E | docs retention policy — docs-only residual |
| L24 | full-low | pending-fix | E |  |
| L25 | full-low | pending-fix | E |  |
| L26 | full-low | pending-fix | E |  |
| L27 | full-low | pending-fix | E |  |
| L28 | full-low | pending-fix | E |  |
| L29 | full-low | pending-fix | E |  |
| L30 | full-low | pending-fix | E |  |
| L31 | full-low | pending-fix | E |  |
| L32 | full-low | pending-fix | E |  |
| L33 | full-low | pending-fix | E |  |
| L34 | full-low | pending-fix | E |  |
| L35 | full-low | accepted-residual | E | EngramCoreSchemaTool unused target — accept residual or remove later |
| L36 | full-low | pending-fix | E |  |
| SEC-H1 | security | pending-fix | B |  |
| SEC-H2 | security | fixed | A |  |
| SEC-M1 | security | fixed | A |  |
| SEC-M2 | security | pending-fix | B |  |
| SEC-M3 | security | pending-fix | B |  |
| SEC-M4 | security | pending-fix | B |  |
| SEC-M5 | security | accepted-residual | E | Product same-user MCP data plane; document only |
| SEC-L1 | security | pending-fix | E |  |
| SEC-L2 | security | pending-fix | E |  |
| SEC-L3 | security | fixed | A |  |
| SEC-L4 | security | accepted-residual | E | Indirect prompt injection via stored sessions; design residual |
| SEC-L5 | security | pending-fix | E |  |
| SEC-I1 | security | accepted-residual | E | Same-user capability model intentional |
| SEC-I2 | security | accepted-residual | E | No cert pinning; system trust acceptable |

Totals: H=2 M=25 L=36 SEC=14 ALL=77
Accepted residual planned: ['L19', 'L21', 'L23', 'L35', 'L8', 'SEC-I1', 'SEC-I2', 'SEC-L4', 'SEC-M5']
Must fix: 68
