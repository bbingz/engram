...
| 109 | ADAPTER-PARSEINFO-CAP-001N | low | Cursor `parseSessionInfo` counts every visible bubble without the produced cap | implementer | Fail closed when user+assistant bubbles exceed `ParserLimits.maxMessages`; named `_repro`. Remaining JSON-array adapter: OpenCode. | **THIS PR** |
