...
| 103 | ADAPTER-STREAM-WHOLE-CAP-001G | low | Qoder `streamMessages` uses `windowedMessages`, which swallows the message cap when `limit == nil` | implementer | Fail closed on oversized whole-transcript streams; named `_repro`. | **THIS PR** — #419 |
