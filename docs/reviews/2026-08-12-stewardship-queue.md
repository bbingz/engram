...
| 106 | ADAPTER-PARSEINFO-CAP-001K | low | Cline `parseSessionInfo` uses the uncapped JSON array and can return prefix counts as complete | implementer | Fail closed when produced messages exceed `ParserLimits.maxMessages`; named `_repro`. Remaining JSON-array adapters stay later slices. | **THIS PR** — #421 |
