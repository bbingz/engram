...
| 100 | ADAPTER-STREAM-WHOLE-CAP-001D | low | Antigravity cache/CLI `streamMessages` omit fail-closed truncation and can stream a prefix as complete | implementer | Pass `reportFailures: true` on cache reads and fail closed on CLI whole-transcript reads; named `_repro`s. | **THIS PR** — #413 |
