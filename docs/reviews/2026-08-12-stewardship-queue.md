...
| 48 | BROWSE-SORT-INDEX-001 | low | Default updated-desc browse filesorts the list-visible session set (L9) | implementer | Fresh migration creates an activity-time expression index; representative list-visible `EXPLAIN QUERY PLAN` uses it without a temporary B-tree; named `_repro` passes | **IMPLEMENTED / VERIFIED — PR #335 OPEN** 2026-08-13 — `macos/EngramCoreWrite/Database/EngramMigrations.swift:63`; `macos/EngramCoreTests/Database/MigrationRunnerTests.swift:158`. |
...
| 48 | BROWSE-SORT-INDEX-001 | low | Default updated-desc browse filesorts activity-time ordering (L9) | **IMPLEMENTED / VERIFIED — PR #335 OPEN** |
