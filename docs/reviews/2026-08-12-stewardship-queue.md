...
| 86 | ADAPTER-MCP-FULLREAD-CAP-001 | low | MCP whole-transcript `readMessages` discards adapter truncation and returns a prefix as complete | implementer | `readWithAdapterRegistry` throws `.messageLimitExceeded` when truncated; named `_repro`. | **THIS PR** |
