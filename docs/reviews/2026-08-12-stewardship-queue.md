...
| 112 | ADAPTER-STREAM-WHOLE-CAP-001H | low | Cline `streamMessages` applyWindow-returns a prefix when `limit == nil` | implementer | Throw `.messageLimitExceeded` on oversized whole-transcript streams; named `_repro`. Remaining JSON-array streams stay later slices. | **THIS BRANCH** — staged pending parseinfo drain |
