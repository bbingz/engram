...
| 111 | ADAPTER-PARSEINFO-CAP-001P | low | Copilot `parseCheckpointSessionInfo` counts every `index.md` row without the produced cap | implementer | Fail closed when checkpoint entries exceed `ParserLimits.maxMessages`; named `_repro`. | **THIS PR** |
