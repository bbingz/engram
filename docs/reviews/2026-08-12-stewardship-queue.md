...
| 89 | ADAPTER-PARSEINFO-CAP-001C | low | Qwen `parseSessionInfo` uses `readObjects` without `reportFailures` and can return prefix counts as complete | implementer | Pass `reportFailures: true` so oversized transcripts return `.messageLimitExceeded`; named `_repro`. Remaining adapters stay later slices. | **THIS PR** — #391 |
