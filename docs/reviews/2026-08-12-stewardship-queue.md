...
| 94 | ADAPTER-PARSEINFO-CAP-001H | low | Windsurf `parseSessionInfo` uses `readCache` without `reportFailures` and can return prefix counts as complete | implementer | Pass `reportFailures: true` so oversized Cascade caches return `.messageLimitExceeded`; named `_repro`. Remaining adapters stay later slices. | **THIS PR** — #401 |
