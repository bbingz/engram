...
| 54 | ADAPTER-STREAM-INJECTION-001 | low | CommandCode, Qwen, Qoder, and Iflow stream paths emit injected wrappers as user despite batch classification (L10) | implementer | Reuse each adapter's existing injection predicate for streamed roles; real user counts and first-user summary stay aligned; four named `_repro`s pass | **IMPLEMENTED / VERIFIED — PR #341 OPEN** 2026-08-13 — stream role fixes at `CommandCodeAdapter.swift:167-180`, `QwenAdapter.swift:196-208`, `QoderAdapter.swift:178-200`, and `IflowAdapter.swift:156-172`; regressions in `AdapterMessageCountTests.swift:670-1128,3674-3711`. |
...
| 54 | ADAPTER-STREAM-INJECTION-001 | low | Four adapter stream paths mislabeled injected wrappers as user (L10) | **IMPLEMENTED / VERIFIED — PR #341 OPEN** |
