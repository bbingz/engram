...
| 90 | ADAPTER-PARSEINFO-CAP-001D | low | Iflow `parseSessionInfo` uses `readObjects` without `reportFailures` and can return prefix counts as complete | implementer | Pass `reportFailures: true` so oversized transcripts return `.messageLimitExceeded`; named `_repro`. Remaining adapters stay later slices. | **THIS PR** |
