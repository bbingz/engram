...
| 105 | MESSAGEPARSER-METADATA-001 | low | MessageParser uses `streamMessages`, so Iflow/CommandCode/Qoder cap throws fall back to uncapped `parseLegacy` | implementer | Call `streamMessagesWithMetadata` and return the bounded prefix; named `_repro`. Do not change Claude/Codex `streamMessages` (#39). | **THIS PR** — #420 |
