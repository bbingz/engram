...
| 102 | ADAPTER-STREAM-WHOLE-CAP-001F | low | CommandCode `streamMessages` uses `windowedMessages`, which swallows the message cap when `limit == nil` | implementer | Fail closed on oversized whole-transcript streams; named `_repro`. Remaining adapters stay later slices. | **THIS PR** — #417 |
