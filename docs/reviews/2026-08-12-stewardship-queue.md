...
| 84 | ADAPTER-INDEX-FTS-CAP-001 | low | FTS drain uses `streamMessages` and can complete on a truncated-and-marked prefix | implementer | `buildSearchContent` honors `streamMessagesWithMetadata` and throws `.messageLimitExceeded` when truncated; named `_repro`. | **THIS PR** — #381 |
