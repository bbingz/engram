...
| 78 | ADAPTER-TRUNCATION-METADATA-001B | low | VS Code inherits default `streamMessagesWithMetadata` and silently omits truncation on oversized whole-transcript reads | implementer | Override reports `truncatedAt` / `totalKnownComplete=false` when message count exceeds `ParserLimits.maxMessages`; named `_repro`. Remaining adapters stay later slices. | **DONE** 2026-08-14 — PR #371 at `main@ffec248e`. |
| 79 | ADAPTER-TRUNCATION-METADATA-001C | low | Cursor inherits default `streamMessagesWithMetadata` and silently omits truncation on oversized whole-transcript reads | implementer | Override reports `truncatedAt` / `totalKnownComplete=false` when message count exceeds `ParserLimits.maxMessages`; named `_repro`. Remaining adapters stay later slices. | **DONE** 2026-08-14 — PR #372 at `main@8bb95e56`. |
| 80 | ADAPTER-TRUNCATION-METADATA-001D | low | Gemini CLI inherits default `streamMessagesWithMetadata` and silently omits truncation on oversized whole-transcript reads | implementer | Override reports `truncatedAt` / `totalKnownComplete=false` when message count exceeds `ParserLimits.maxMessages`; named `_repro`. Remaining adapters stay later slices. | **DONE** 2026-08-14 — PR #373 at `main@eaecb430`. |
| 81 | ADAPTER-TRUNCATION-METADATA-001E | low | Copilot checkpoint indexes inherit default `streamMessagesWithMetadata` and silently omit truncation on oversized whole-transcript reads | implementer | Override reports `truncatedAt` / `totalKnownComplete=false` when checkpoint message count exceeds `ParserLimits.maxMessages`; named `_repro`. Remaining wrappers stay later slices. | **DONE** 2026-08-14 — PR #375 at `main@9cac0311`. |
| 82 | ADAPTER-TRUNCATION-METADATA-001F | low | `RecentlyModifiedSessionAdapter` inherits default `streamMessagesWithMetadata` and drops the base adapter's truncation marker | implementer | Wrapper forwards `streamMessagesWithMetadata` and `scanForIndexing`; named `_repro`. Truncation-metadata adapter series complete, including recent-scan wrappers. | **DONE** 2026-08-14 — PR #377 at `main@b0d399a0`. |
| 83 | ADAPTER-INDEX-CAP-001 | low | Default `scanForIndexing` indexes an oversized transcript prefix as complete | implementer | Default scan uses `streamMessagesWithMetadata` and returns `.messageLimitExceeded` when truncated; named `_repro`. | **DONE** 2026-08-14 — PR #379 at `main@11dfdaaa`. |
| 84 | ADAPTER-INDEX-FTS-CAP-001 | low | FTS drain uses `streamMessages` and can complete on a truncated-and-marked prefix | implementer | `buildSearchContent` honors `streamMessagesWithMetadata` and throws `.messageLimitExceeded` when truncated; named `_repro`. | **DONE** 2026-08-14 — PR #381 at `main@74f5dccf`. |
| 85 | ADAPTER-INDEX-BACKFILL-CAP-001 | low | Instruction/beat backfill uses `streamMessages` and can persist prefix signals from a truncated-and-marked transcript | implementer | Both backfill readers honor `streamMessagesWithMetadata` and treat truncation as `.messageLimitExceeded`; named `_repro`s. | **DONE** 2026-08-14 — PR #383 at `main@4a63385d`. |
| 86 | ADAPTER-MCP-FULLREAD-CAP-001 | low | MCP whole-transcript `readMessages` discards adapter truncation and returns a prefix as complete | implementer | `readWithAdapterRegistry` throws `.messageLimitExceeded` when truncated; named `_repro`. | **DONE** 2026-08-14 — PR #385 at `main@c52fb157`. |
| 87 | ADAPTER-PARSEINFO-CAP-001 | low | Claude `parseSessionInfo` uses `readObjects` without `reportFailures` and can return prefix counts as complete | implementer | Pass `reportFailures: true` so oversized transcripts return `.messageLimitExceeded`; named `_repro`. Remaining adapters stay later slices. | **DONE** 2026-08-14 — PR #387 at `main@eddec1df`. |
| 88 | ADAPTER-PARSEINFO-CAP-001B | low | Codex `parseSessionInfo` uses `readObjects` without `reportFailures` and can return prefix counts as complete | implementer | Pass `reportFailures: true` so oversized transcripts return `.messageLimitExceeded`; named `_repro`. Remaining adapters stay later slices. | **DONE** 2026-08-14 — PR #389 at `main@57979e46`. |
| 89 | ADAPTER-PARSEINFO-CAP-001C | low | Qwen `parseSessionInfo` uses `readObjects` without `reportFailures` and can return prefix counts as complete | implementer | Pass `reportFailures: true` so oversized transcripts return `.messageLimitExceeded`; named `_repro`. Remaining adapters stay later slices. | **DONE** 2026-08-14 — PR #391 at `main@f022de0b`. |
| 90 | ADAPTER-PARSEINFO-CAP-001D | low | Iflow `parseSessionInfo` uses `readObjects` without `reportFailures` and can return prefix counts as complete | implementer | Pass `reportFailures: true` so oversized transcripts return `.messageLimitExceeded`; named `_repro`. Remaining adapters stay later slices. | **DONE** 2026-08-14 — PR #393 at `main@24c96146`. |
| 91 | ADAPTER-PARSEINFO-CAP-001E | low | CommandCode `parseSessionInfo` uses `readObjects` without `reportFailures` and can return prefix counts as complete | implementer | Pass `reportFailures: true` so oversized transcripts return `.messageLimitExceeded`; named `_repro`. Remaining adapters stay later slices. | **DONE** 2026-08-14 — PR #395 at `main@4d85c959`. |
| 92 | ADAPTER-PARSEINFO-CAP-001F | low | Qoder `parseSessionInfo` uses `readObjects` without `reportFailures` and can return prefix counts as complete | implementer | Pass `reportFailures: true` so oversized transcripts return `.messageLimitExceeded`; named `_repro`. Remaining adapters stay later slices. | **DONE** 2026-08-14 — PR #397 at `main@d9f7acfe`. |
| 93 | ADAPTER-PARSEINFO-CAP-001G | low | Copilot `parseSessionInfo` uses `readObjects` without `reportFailures` and can return prefix counts as complete | implementer | Pass `reportFailures: true` so oversized transcripts return `.messageLimitExceeded`; named `_repro`. Remaining adapters stay later slices. | **DONE** 2026-08-14 — PR #399 at `main@a127abeb`. |
| 94 | ADAPTER-PARSEINFO-CAP-001H | low | Windsurf `parseSessionInfo` uses `readCache` without `reportFailures` and can return prefix counts as complete | implementer | Pass `reportFailures: true` so oversized Cascade caches return `.messageLimitExceeded`; named `_repro`. Remaining adapters stay later slices. | **DONE** 2026-08-14 — PR #401 at `main@844225de`. |
| 95 | ADAPTER-PARSEINFO-CAP-001I | low | Antigravity cache/CLI `parseSessionInfo` omit `reportFailures` and can return prefix counts as complete | implementer | Pass `reportFailures: true` so oversized cache and CLI transcripts return `.messageLimitExceeded`; named `_repro`s. Remaining adapters stay later slices. | **DONE** 2026-08-14 — PR #403 at `main@a157d21d`. |
| 96 | ADAPTER-PARSEINFO-CAP-001J | low | Kimi `parseSessionInfo` uses `readObjects` without `reportFailures` and can return prefix counts as complete | implementer | Pass `reportFailures: true` so oversized `context.jsonl` returns `.messageLimitExceeded`; named `_repro`. | **DONE** 2026-08-14 — PR #405 at `main@ab720151`. JSONL parseSessionInfo series complete. |
| 97 | ADAPTER-STREAM-WHOLE-CAP-001 | low | Copilot `streamMessages` uses `readObjects` without `reportFailures` when `limit == nil` and can stream a prefix as complete | implementer | Pass `reportFailures: true` so oversized whole-transcript streams throw `.messageLimitExceeded`; named `_repro`. Remaining adapters stay later slices. | **DONE** 2026-08-14 — PR #407 at `main@363e86f9`. |
| 98 | ADAPTER-STREAM-WHOLE-CAP-001B | low | Qwen `streamMessages` uses `readObjects` without `reportFailures` and can stream a prefix as complete | implementer | Pass `reportFailures: true` so oversized whole-transcript streams throw `.messageLimitExceeded`; named `_repro`. Remaining adapters stay later slices. | **DONE** 2026-08-14 — PR #409 at `main@543ea5b8`. |
| 99 | ADAPTER-STREAM-WHOLE-CAP-001C | low | Windsurf `streamMessages` uses `readCache` without `reportFailures` and can stream a prefix as complete | implementer | Pass `reportFailures: true` so oversized Cascade caches throw `.messageLimitExceeded`; named `_repro`. Remaining adapters stay later slices. | **DONE** 2026-08-14 — PR #411 at `main@f1ece6a6`. |
| 100 | ADAPTER-STREAM-WHOLE-CAP-001D | low | Antigravity cache/CLI `streamMessages` omit fail-closed truncation and can stream a prefix as complete | implementer | Pass `reportFailures: true` on cache reads and fail closed on CLI whole-transcript reads; named `_repro`s. | **DONE** 2026-08-14 — PR #413 at `main@4bd9b1f9`. |
| 101 | ADAPTER-STREAM-WHOLE-CAP-001E | low | Iflow `streamMessages` uses `windowedMessages`, which swallows the message cap when `limit == nil` | implementer | Fail closed on oversized whole-transcript streams; named `_repro`. Remaining adapters stay later slices. | **DONE** 2026-08-14 — PR #415 at `main@30b70bf6`. |
| 102 | ADAPTER-STREAM-WHOLE-CAP-001F | low | CommandCode `streamMessages` uses `windowedMessages`, which swallows the message cap when `limit == nil` | implementer | Fail closed on oversized whole-transcript streams; named `_repro`. Remaining adapters stay later slices. | **DONE** 2026-08-14 — PR #417 at `main@f803b103`. |
| 103 | ADAPTER-STREAM-WHOLE-CAP-001G | low | Qoder `streamMessages` uses `windowedMessages`, which swallows the message cap when `limit == nil` | implementer | Fail closed on oversized whole-transcript streams; named `_repro`. | **DONE** 2026-08-14 — PR #419 at `main@ad8dce65`. |
| 105 | MESSAGEPARSER-METADATA-001 | low | MessageParser uses `streamMessages`, so Iflow/CommandCode/Qoder cap throws fall back to uncapped `parseLegacy` | implementer | Call `streamMessagesWithMetadata` and return the bounded prefix; named `_repro`. Do not change Claude/Codex `streamMessages` (#39). | **DONE** 2026-08-14 — PR #420 at `main@caf02275`. |
| 106 | ADAPTER-PARSEINFO-CAP-001K | low | Cline `parseSessionInfo` uses the uncapped JSON array and can return prefix counts as complete | implementer | Fail closed when produced messages exceed `ParserLimits.maxMessages`; named `_repro`. Remaining JSON-array adapters stay later slices. | **DONE** 2026-08-14 — PR #421 at `main@5c21a00d`. |
| 107 | ADAPTER-PARSEINFO-CAP-001L | low | VS Code `parseSessionInfo` counts every request without the produced-message cap | implementer | Fail closed when user+assistant texts exceed `ParserLimits.maxMessages`; named `_repro`. Remaining JSON-array adapters stay later slices. | **DONE** 2026-08-14 — PR #422 at `main@2b7ab948`. |
| 108 | ADAPTER-PARSEINFO-CAP-001M | low | Gemini CLI `parseSessionInfo` counts flattened messages without the produced cap | implementer | Fail closed when produced messages exceed `ParserLimits.maxMessages`; named `_repro`. Remaining JSON-array adapters stay later slices. | **DONE** 2026-08-14 — PR #423 at `main@d23cd2b3`. |
...
| 110 | ADAPTER-PARSEINFO-CAP-001O | low | OpenCode `parseSessionInfo` counts every contentful message without the produced cap | implementer | Fail closed when user+assistant counts exceed `ParserLimits.maxMessages`; named `_repro`. JSON-array parseinfo series complete after this; Copilot checkpoint stays later. | **THIS PR** |
