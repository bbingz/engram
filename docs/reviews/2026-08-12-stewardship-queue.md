...
| 61 | MCP-SEARCH-MIN-LENGTH-001 | low | MCP search accepts 1-char LIKE hits the app/service reject (L-a) | implementer | Guard `normalizedQuery.count < 2` like `Database.swift:568` / `EngramServiceReadProvider.swift:530`; keep 2-char Latin on LIKE; named `_repro` | **IMPLEMENTED / VERIFIED — PR PENDING** 2026-08-13 — fix at `MCPDatabase.swift:1073-1082`; regression at `EngramMCPExecutableTests.swift`. |
...
| 61 | MCP-SEARCH-MIN-LENGTH-001 | low | MCP 1-char keyword search over-recalled vs app/service (L-a) | **IMPLEMENTED / VERIFIED — PR PENDING** |
