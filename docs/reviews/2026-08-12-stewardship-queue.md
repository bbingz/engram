...
| 69 | ADAPTER-EMPTY-SESSION-001D | low | Cursor composers with no visible bubbles still index as zero-count sessions (R184-3 remainder) | implementer | `parseSessionInfo` fails closed on zero visible user/assistant counts; named `_repro`. Remaining adapters stay later slices. | **THIS PR** — #360 |
