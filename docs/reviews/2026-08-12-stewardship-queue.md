...
| 75 | ADAPTER-EMPTY-SESSION-001J | low | Copilot events with a valid session id but no user/assistant turns report malformed JSON (R184-3 remainder) | implementer | Empty session id remains `.malformedJSON`; valid-id zero-turn events return `.noVisibleMessages`; checkpoint discovery/fallback stays green; named `_repro`. Remaining adapters stay later slices. | **THIS PR** — #366 |
