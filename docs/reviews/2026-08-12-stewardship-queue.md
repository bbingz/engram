...
| 108 | ADAPTER-PARSEINFO-CAP-001M | low | Gemini CLI `parseSessionInfo` counts flattened messages without the produced cap | implementer | Fail closed when produced messages exceed `ParserLimits.maxMessages`; named `_repro`. Remaining JSON-array adapters stay later slices. | **THIS PR** |
