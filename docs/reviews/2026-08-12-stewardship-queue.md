...
| 76 | ADAPTER-TAIL-TERMINAL-001 | low | Tail-parse treats `malformedJSON` / `invalidUtf8` / `fileMissing` as terminal while full parse retries them (R184-4) | implementer | `isTerminalTailFailure` delegates to `FileIndexState.isTerminalFailure`; malformed tail falls through to full scan; `noVisibleMessages` stays terminal; named `_repro`s | **THIS PR** — #368 |
