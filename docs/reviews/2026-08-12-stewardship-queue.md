...
| 85 | ADAPTER-INDEX-BACKFILL-CAP-001 | low | Instruction/beat backfill uses `streamMessages` and can persist prefix signals from a truncated-and-marked transcript | implementer | Both backfill readers honor `streamMessagesWithMetadata` and treat truncation as `.messageLimitExceeded`; named `_repro`s. | **THIS PR** |
