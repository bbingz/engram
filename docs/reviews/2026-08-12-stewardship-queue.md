...
| 42 | WORKGRAPH-FORMATTER-001 | low | WorkGraphView allocates ISO8601DateFormatter per row (L15) | implementer | Reuse EngramTimestampParser; source _repro | **DONE** 2026-08-12 — PR #324 merged at `main@53c171d1`; audit L15. |
...
| 42 | WORKGRAPH-FORMATTER-001 | low | WorkGraphView allocates ISO8601DateFormatter per row (L15) | **DONE** PR #324 `main@53c171d1` |
...
Evidence for WORKGRAPH-FORMATTER-001: `WorkGraphView.swift` `isIdle` / `WorkGraphRow.statusColor` previously called `ISO8601DateFormatter()` per repo. Fix routes through `EngramTimestampParser.date(from:)` with source `_repro` in `ContentSegmentAndGitRepoTests`. Audit L15 in `docs/reviews/2026-07-17-engram-full-audit.md`. Shipped in PR #324 at `main@53c171d1`.

Evidence for INDEXER-BACKPRESSURE-001: `SwiftIndexer.indexAll` now writes each
full 100-snapshot batch from the awaited `scanSnapshots` callback before the
producer may advance (`macos/EngramCoreWrite/Indexing/SwiftIndexer.swift:67-81,128-131`).
`collectSnapshots` remains a direct collection path (`:115-120`), while the
unbounded snapshot stream has been removed. The 305-session backpressure and
write-error propagation regressions are in
`macos/EngramCoreTests/IndexerParityTests.swift:518-570`. Audit L17 in
`docs/reviews/2026-07-17-engram-full-audit.md`.
