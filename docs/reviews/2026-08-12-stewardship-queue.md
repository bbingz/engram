...
| 98 | ADAPTER-STREAM-WHOLE-CAP-001B | low | Qwen `streamMessages` uses `readObjects` without `reportFailures` and can stream a prefix as complete | implementer | Pass `reportFailures: true` so oversized whole-transcript streams throw `.messageLimitExceeded`; named `_repro`. Remaining adapters stay later slices. | **THIS PR** — #409 |
