...
| 83 | ADAPTER-INDEX-CAP-001 | low | Default `scanForIndexing` indexes an oversized transcript prefix as complete | implementer | Default scan uses `streamMessagesWithMetadata` and returns `.messageLimitExceeded` when truncated; named `_repro`. | **THIS PR** — #379 |
