...
| 99 | ADAPTER-STREAM-WHOLE-CAP-001C | low | Windsurf `streamMessages` uses `readCache` without `reportFailures` and can stream a prefix as complete | implementer | Pass `reportFailures: true` so oversized Cascade caches throw `.messageLimitExceeded`; named `_repro`. Remaining adapters stay later slices. | **THIS PR** — #411 |
