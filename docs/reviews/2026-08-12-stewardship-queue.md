...
| 73 | ADAPTER-EMPTY-SESSION-001H | low | Windsurf metadata-only Cascade cache files still index as zero-count sessions (R184-3 remainder) | implementer | `parseSessionInfo` fails closed on zero visible user/assistant counts; named `_repro`. Remaining adapters stay later slices. | **THIS PR** — #364 |
