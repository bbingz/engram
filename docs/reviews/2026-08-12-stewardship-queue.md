...
| 82 | ADAPTER-TRUNCATION-METADATA-001F | low | `RecentlyModifiedSessionAdapter` inherits default `streamMessagesWithMetadata` and drops the base adapter's truncation marker | implementer | Wrapper forwards `streamMessagesWithMetadata` and `scanForIndexing`; named `_repro`. | **THIS PR** — #377 |
