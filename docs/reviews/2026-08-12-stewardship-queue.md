...
| 58 | GET-CONTEXT-COST-TODAY-001 | low | `get_context` "Cost today" uses a UTC day window while `get_costs` groups by local day (L-g) | implementer | Honor process-local / `TZ` calendar day via `contextTimeZone()`; Shanghai includes previous-UTC-evening spend; UTC still isolates the UTC day; named `_repro` passes | **IN PR** 2026-08-13 |
