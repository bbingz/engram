...
| 96 | ADAPTER-PARSEINFO-CAP-001J | low | Kimi `parseSessionInfo` uses `readObjects` without `reportFailures` and can return prefix counts as complete | implementer | Pass `reportFailures: true` so oversized `context.jsonl` returns `.messageLimitExceeded`; named `_repro`. | **THIS PR** — #405 |
