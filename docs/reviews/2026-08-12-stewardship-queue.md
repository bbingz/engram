...
| 107 | ADAPTER-PARSEINFO-CAP-001L | low | VS Code `parseSessionInfo` counts every request without the produced-message cap | implementer | Fail closed when user+assistant texts exceed `ParserLimits.maxMessages`; named `_repro`. Remaining JSON-array adapters stay later slices. | **THIS PR** |
