...
| ~~MCP search parity: multi-term AND, activity-time `since`, exact project-or-alias filtering~~ — post-audit follow-ups fixed with executable `_repro` RED→GREEN | READ-001/002/003 closed; ARCH-001 remains open |
...
5. **原始合成回合未跑完整 xcodebuild**；后续批次按 finding 记录 focused/full suite 与 Debug build 证据。READ-001/002/003 follow-ups have run the complete `EngramMCPTests` suite (161/161 at the READ-003 tip) and a successful Debug `Engram` build.
