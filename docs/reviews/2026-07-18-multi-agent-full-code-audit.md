...
Engram 在 **写路径完整性、Archive V2 CAS、Unix socket 跨用户隔离、单写者门控** 上仍是健康的 local-first 产品。2026-07-17 命名 High（H1/H2/SEC-H1/H2）与 #196 关闭；#197 补完 R4 insight 隔离终端化、R5 reclaim 预算游标、R10 HEAD 行为证据与 R11 账本。
...
**#197 裁决：** APPROVE_WITH_NITS — 可合并；合并后修 followups 对 R1 的 overclaim，并安排 dual-tx 小修。
...
| M3 insight | 不全/误标 | R4 open | **#197 关**；dual-tx Medium |
...
**已修复不再重复开单：** H1, H2, SEC-H1/H2 主路径, M1 核心策略, M12/R2 中流, M18/M19/M24 主路径, R4 隔离+permanent, R5 候选预算游标, R10 行为 HEAD, R11 证据列。
...
6. **嵌入** session 隔离 + insight R4 产品路径；circuit breaker；model/dim reconcile
...
| 字段 | 内容 |
|------|------|
| **Location** | App `Database.swift` ~464–466；Service ~1886–1888：`COALESCE(s.end_time,s.start_time)`；MCP ~1371–1373, 2162–2164, 2225–2227：仅 `s.start_time` |
...
| **Fix** | 解耦：pending job 存在检查 / 独立 backlog 信号 / 每 N 个 idle 周期 best-effort drain |
| **Effort** | M |
...
| **Fix** | 扩展 exact/prefix（`initial*`/`periodic*` 等）并 source-contract + 行为测试 |
| **Effort** | S |
...
| **Impact** | writeEmbeddings 提交后 recordFailures 崩溃/抛错 → 毒 insight 无 retry/status，永久再选（削弱 R4 耐久叙事；session 路径单事务更强） |
...
| R11-r1-overclaim | confirmed docs | followups 称 R1 closed | S |
...
- Insight R4：失败表、隔离、permanent、产品 runner 接线
...
## Now（合并 #197 后 1–2 个冲刺）

| 项 | 对应 |
|----|------|
| 合并 #197；followups 修正 R1 非 closed | R11-r1-overclaim |
| R4 dual-tx 单事务 | R4-dual-tx |
| Reclaim product-missing → blocked + cursor | SVC-001 |
| skip 清理 `semantic_chunks` + probe | EMB-001 |
| long-running 名与 call site 对齐 | CONC-001 |
| idle embedding drain 与 merge-only 解耦 | EMB-STARVE |
| MCP since COALESCE + multi-term CTE（或共享 builder 第一步） | READ-002, READ-001 |
...
5. **未跑完整 xcodebuild 本合成回合** — 依赖输入 PR 复审与源码对抗验证。
