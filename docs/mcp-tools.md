...
|------|------|----------|-------------|
| source | string | no | Filter by tool source. Enum: `codex`, `claude-code`, `grok`, `copilot`, `pi`, `gemini-cli`, `opencode`, `iflow`, `qwen`, `qoder`, `kimi`, `minimax`, `mimo`, `doubao`, `glm`, `deepseek`, `lobsterai`, `commandcode`, `cline`, `cursor`, `vscode`, `antigravity`, `windsurf` |
