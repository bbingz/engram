...
| **MCP-SEARCH-MIN-LENGTH-001** / L-a | implemented / verified; PR pending | `MCPDatabase.searchSessions` now returns empty keyword results when `normalizedQuery.count < 2`, matching app/service (`Database.swift:568`, `EngramServiceReadProvider.swift:530`). Two-character Latin still uses LIKE. Multi-word session-scoped AND remains READ-001. Named `_repro`: `testKeywordSearchRejectsSingleCharacterQuery_repro`. |
...
| **L-a…L-j** | residual | Remaining Low/Info rows from the full-project review: L-h, L-i, L-j. L-b/#342, L-e/#343, L-d/#344, L-g/#345, L-c/#346, L-f/#347 closed; L-a is MCP-SEARCH-MIN-LENGTH-001 on the pending PR. |
