...
| **SERVICE-CHECKPOINT-SHUTDOWN-001** / L18 | implemented / verified; PR pending | Graceful service shutdown cancels and awaits the periodic PASSIVE checkpoint before either remaining TRUNCATE operation. Final TRUNCATE runs in an awaited fresh cancellation context because orderly shutdown begins from a cancelled runner task; named regressions pin both behaviors. |
