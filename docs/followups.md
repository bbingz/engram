...
| **ADAPTER-STREAM-WHOLE-CAP-001** / export P1 remainder | this PR (#407) | Copilot `streamMessages` called `readObjects` without `reportFailures` when `limit == nil`, so an oversized `events.jsonl` streamed a prefix as complete. |
