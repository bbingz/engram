...
| **WORKGRAPH-FORMATTER-001** / L15 | closed (#324) | WorkGraph idle/status coloring uses `EngramTimestampParser` instead of per-row `ISO8601DateFormatter()`; source `_repro` pins the contract. Shipped at `main@53c171d1`. |
| **INDEXER-BACKPRESSURE-001** / L17 | implemented / verified; PR #330 open | `SwiftIndexer.indexAll` awaits each full 100-snapshot write inside the scan callback, bounding producer lead without a channel dependency. `collectSnapshots` remains available without the unbounded stream; `testIndexAllBackpressuresBeforeThirdBatch_repro` and `testIndexAllPropagatesWriteFailureWithoutContinuingScan_repro` pin backpressure and fail-fast propagation. |
| **POLYCLI-LINKED-DEAD-001** / L12 | in review (#326) | Polycli parent classification remains suggestion-only while its unused concurrent-link helper and permanently-zero linked result/event surface are removed; do not duplicate while CI runs. |
| **ANTIGRAVITY-CWD-REREAD-001** / L34 | in review (#327) | Antigravity CWD inference reads at most the first 50,000 transcript bytes with a file handle and preserves the existing path heuristic, including safe UTF-8 prefix-boundary handling. |
| **VSCODE-STRICT-REQUESTS-001** / L24 | implemented / verified; PR pending | VS Code parsing now ignores stable non-object request entries while retaining valid siblings, matching message streaming and avoiding malformed-JSON retry churn. |
