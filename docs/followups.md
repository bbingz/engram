...
| **ADAPTER-STREAM-WHOLE-CAP-001F** / export P1 remainder | this PR (#417) | CommandCode `streamMessages` used `windowedMessages`, which swallows `.messageLimitExceeded` when `limit == nil`, so an oversized transcript streamed a prefix as complete. |
