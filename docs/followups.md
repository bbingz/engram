...
| **SESSION-DETAIL-FILTER-001** / L32 | implemented / verified (PR pending) | `SessionDetailView.updateDisplayIndexed` now runs its full loaded-prefix visibility loop in cancellable detached work (`macos/Engram/Views/SessionDetailView.swift:102-166`); append paging still filters only the new slice, and session/toggle changes reject stale results. Named source regression: `TodayWorkbenchScopeTests.testSessionDetailFullFilterRunsOffMain_repro`. |
