...
| **ADAPTER-EMPTY-SESSION-001B** / R184-3 | closed (#358) | CommandCode and Iflow injection-only files return `.noVisibleMessages` after a valid session id, matching Qwen. Shipped at `main@237aa757`. |
| **ADAPTER-EMPTY-SESSION-001C** / R184-3 | this PR | Gemini empty/content-less files and Qoder injection-only files return `.noVisibleMessages` after a valid session id. Cursor and remaining adapters stay later slices. |
