...
| **BROWSE-SORT-INDEX-001** / L9 | implemented / verified; PR #335 open | The list-visible activity-time index in `macos/EngramCoreWrite/Database/EngramMigrations.swift:63` serves `ORDER BY COALESCE(end_time, start_time)` without a temporary B-tree; `MigrationRunnerTests.swift:158` pins the fresh-migration query plan. |
