...
| **ADAPTER-EMPTY-SESSION-001** / R184-3 | this PR | Codex metadata-only files (`session_meta` + non-response events) now return `.noVisibleMessages` like Claude/Qwen/VS Code (`CodexAdapter.swift` after the meta id/timestamp guard). Remaining adapters stay later slices. |
