...
| **REMOTE-CATALOG-MEM-001** / L29 | implemented / verified; PR #325 open | Legacy `/v1/catalog` stops after 1,024 matching peer manifests and limits both cumulative decoded input and the serialized response to 4 MiB. It uses bounded encrypted-blob reads and returns fail-closed 413 when either budget is exceeded; `testCatalogRejectsAggregateDecodedBytesOverBudget_repro` and `testCatalogRejectsPeerCountOverBudget_repro` cover both limits over a live socket. |
