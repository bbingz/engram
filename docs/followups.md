...
| **MCP-002** | implemented (B2 secondary visibility parity; unmerged) | `get_context`, `tool_analytics`, and `file_activity` now reuse shared list-visible and top-level predicates; shipped-binary `_repro` coverage includes `listContextSessions`, `topToolsSince`, and `fileHotspotsSince`. Existing READ-005 `orphan_status` behavior is unchanged. |
| **MCP-012** | implemented (B2 secondary visibility parity; unmerged) | `project_timeline` now excludes hidden, skip-tier, confirmed-child, and suggested-child sessions, with shipped-binary `_repro` coverage against `list_sessions` defaults. |
