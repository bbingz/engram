...
| **DERIVEDDATA-RELEASE-PLAINTEXT-001** / L-f | implemented / verified; PR pending | `KeychainHelper.shouldBypassKeychain` retains its DEBUG/DerivedData authorization-dialog policy, while `allowsPlaintextSettingsFallback` is now independently true only in DEBUG. Release builds, including DerivedData Release, therefore keep the existing SEC-M3 fail-closed `@keychain` marker path (`SettingsIO.swift:69-96`; `AISettingsSection.swift:377-392`). The expanded source-contract `_repro` pins both halves in `DeprecatedSettingsScrubTests.swift`. |
...
| **L-a…L-j** | residual | Remaining Low/Info rows from the full-project review: L-a, L-h, L-i, L-j. L-b/#342, L-e/#343, L-d/#344, L-g/#345, and L-c/#346 closed; L-f is DERIVEDDATA-RELEASE-PLAINTEXT-001 on the pending PR. |
