...
| **ADAPTER-STREAM-WHOLE-CAP-001F** / export P1 remainder | closed (#417) | CommandCode `streamMessages` fails closed on oversized whole-transcript reads. Shipped at `main@f803b103`. Remaining windowed-message stream: Qoder. |
| **ADAPTER-STREAM-WHOLE-CAP-001G** / export P1 remainder | this PR (#419) | Qoder `streamMessages` used `windowedMessages`, which swallows `.messageLimitExceeded` when `limit == nil`, so an oversized transcript streamed a prefix as complete. |
