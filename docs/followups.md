...
| **GEMINI-JSON-FSYNC-001** / L26 | implemented / verified; PR pending | `GeminiProjectsJSON.writeAtomic` fsyncs the temp file before rename and the parent directory after, matching `JsonlPatch`. `testAtomicWriterFsyncsTempAndParentDirectory_repro` pins the durable write order. |
