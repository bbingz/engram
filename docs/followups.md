...
| **REMOTE-STATUS-PERSIST-001** / L27 | implemented / verified; PR pending | `/v2/archive/status` returns the live snapshot with `forcePersist: false` so polling no longer rewrites `status-v1.json` every request; disk flush stays on the 60s throttle. |
