...
| **MCP-HYBRID-ZIP-001** / L30 | implemented / verified; PR #321 open | MCP hybrid fusion indexes semantic hits by `session.id` via `MCPSearchResultIndex` instead of positional `zip(ids, items)` when deleted sessions drop out of `searchResultItems`. |
