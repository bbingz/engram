...
| **ADAPTER-PARSEINFO-CAP-001N** / export P1 remainder | this PR | Cursor `parseSessionInfo` counted every visible bubble without the produced cap, so an oversized composer returned prefix counts as complete. Remaining JSON-array adapter: OpenCode. |
