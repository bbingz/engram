...
| **REMOTE-MANIFEST-SCHEMA-001** / L36 | implemented / verified; PR #317 open | `ManifestCodec.decode` now throws `schemaVersionUnsupported` for an unknown version, `decodeCatalog` rejects an unsupported envelope version or type, and its per-peer isolation preserves compatible manifests while skipping a future-version peer (`macos/EngramCoreWrite/RemoteSync/ManifestCodec.swift`). Four named schema-version regression tests plus the full `SessionSyncTests` class pass on `feat/remote-manifest-schema-001`; merge status remains pending. |
