...
| **ADAPTER-PARSEINFO-CAP-001L** / export P1 remainder | this PR (#422) | VS Code `parseSessionInfo` counted every request without the produced-message cap, so an oversized chat session returned prefix counts as complete. Remaining JSON-array adapters stay later slices. |
