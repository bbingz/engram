...
| **ADAPTER-STREAM-WHOLE-CAP-001D** / export P1 remainder | this PR (#413) | Antigravity cache `streamMessages` called `readCache` without `reportFailures`, and CLI `streamMessages` swallowed the message cap when `limit == nil`, so oversized transcripts streamed a prefix as complete. |
