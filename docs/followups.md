...
|----|--------|------------------|
| **R1** / ARCH-001 | open (structural investment; A/B/C shipped; D implemented / PR pending) | ARCH-001A shared search predicates (#311), ARCH-001B list/aggregate visibility convergence (#312), and ARCH-001C residual list visibility (#313) are on main. ARCH-001D adds the first executable cross-surface keyword parity contract: one `mcp-contract.sqlite` copy and the same trimmed Latin query return identical non-empty session-ID sets from the App, Service, and native MCP readers. The full CoreRead pool/DTO migration remains deferred, so ARCH-001 stays open. Evidence is recorded at rank 62 in `docs/reviews/2026-08-12-stewardship-queue.md`. |
