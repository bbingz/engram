...
| **ADAPTER-PARSEINFO-CAP-001M** / export P1 remainder | this PR | Gemini CLI `parseSessionInfo` counted every flattened message without the produced cap, so an oversized session returned prefix counts as complete. Remaining JSON-array adapters stay later slices. |
