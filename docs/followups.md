...
| **REMOTE-TELEMETRY-001** / L28 | implemented / verified; PR pending | Archive routes use HTTP 503 for `storage_unavailable`, while telemetry previously mapped only 507 to that category and mislabeled 503 as `internal_error` (`macos/EngramRemoteServer/Core/ArchiveRoutes.swift:43,124,201,453`; `macos/EngramRemoteServer/Core/ArchiveRemoteTelemetryStore.swift:161-180`). The branch maps both storage statuses consistently and adds a named regression in `macos/EngramRemoteServerCoreTests/ArchiveRemoteTelemetryStoreTests.swift`; merge remains pending. |
