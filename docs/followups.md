...
| **MCP-STDIO-DEAD-001** / L13 | implemented / verified; PR #340 open | `MCPStdioServer.swift:109-113` remains the sole live `tools/call` dispatch and the unreachable `handle()` switch case is removed. `EngramMCPExecutableTests.swift:132-180` pins the source structure and a correlated executable response. |
