...
| **ADAPTER-PARSEINFO-CAP-001O** / export P1 remainder | this PR | OpenCode `parseSessionInfo` counted every contentful message without the produced cap, so an oversized session returned prefix counts as complete. JSON-array parseinfo series complete after this; Copilot checkpoint stays a later slice. |
