...
1. **本机上 `~/.codex/` 下真实的磁盘文件**(截至 2026-07-02 为 2,659 个 active rollout
   `.jsonl` 文件 + 5 个已归档;本文更深处的旧语料统计来自 2026-06-21 的 2,505 + 5 样本)。当格式与
...
## Claude Code Provider-Root 变体（`cc-codex`）

本文主体描述的是 `~/.codex/` 下的原生 Codex CLI rollout 文件。本机
`cc-codex` 会话不同：它们是用户 `cc-codex` wrapper（`_cc_with openai`）产生的
Claude Code JSONL 文件，位于 `~/.claude-openai/projects`；Engram 根据
provider-root 路径把它们映射为 `codex` source。

2026-07-02 对 `~/.claude-openai/projects` 的本机 smoke：

| Listed JSONL | Parsed conversations | Subagents | Parent links | Source |
|---:|---:|---:|---:|---|
| 2,823 | 2,693 | 2,659 | 2,659 | `codex` |

model metadata 是后端元数据,不用于 source ownership;`codex` source 由 provider root
路径拥有。跳过文件包括 workflow `journal.jsonl` 状态日志以及过滤后没有可展示
conversation turn 的 local-command/system-injection side-channel 会话。

新的字段级 smoke 发现 192,351 条 raw records、0 条 malformed lines。保留 TS parser
列出 2,823 个 locator，解析 2,693 个 conversation；在与 Swift 对齐 visible-tool-result
语义后，parser/stream count mismatch 为 0。

同次 DB/runtime 检查：

- 已安装 `/Applications/Engram.app` build `20260701074505` 在
  `/Users/bing/.claude-openai/%` 下有 2,526 个 `codex` 行,并有 2,654 个该 root
  的 `file_index_state` 行（2,526 个 `ok`,128 个 `retry`），全部仍是 schema version 1。
- 新的 parser 证据看到 167 个可解析 `cc-codex` locator 不在 `sessions` 中：166 个没有
  `file_index_state`，1 个已在 `file_index_state` 中记录为 `retry/malformedJSON`。
  当前 delta 是 active-write/retry frontier,不是缺少 provider-root 支持。
- 修正后的 visible-tool-result parser 报告 0 个字段陈旧的当前 provider-root 行。
  此前 2,433 行 stale-count 结论是 retained TS 审计工具误报：TS 当时会计入 Swift 产品已丢弃的非可见
  Claude `tool_result` 行。本次审计没有修改 `/Users/bing/.engram/index.sqlite`。

不要把原生 `codex` 行数当成 `cc-codex` 已索引的证据。它们共享 Engram source id，但磁盘格式和 root 不同。
...
**2267 threads**;现行处于 **40** / **2664 threads**)。只有 `codex-dev.db` 是 `sqlite/`
子目录独有的。
...
`archived=1` 的 `rollout_path` 均为 `~/.codex/archived_sessions/rollout-...jsonl`;截至
2026-07-02,全部 2659 行 `archived=0` 都指向 `sessions/YYYY/MM/DD` 树。磁盘计数
(2659 树内 + 5 归档)== Codex `state_5.sqlite` thread 数(2664)完全相等。Engram 自己的
`~/.engram/index.sqlite` 当前在 `/Users/bing/.codex/%` 下有 2662 个原生 `codex` 行,
另有 2 个可解析 rollout 文件缺失且 0 个 stale session extras;最新原生 `codex` 索引时间为
`2026-07-01T04:11:11Z`。现有 Engram DB 的 model 值也已过时:1808 行仍是 provider 名
`openai`,838 行为空/NULL,15 行为 `gpt-5.5`,1 行为 `custom`;2026-07-01 的
`turn_context.payload.model` 修复后,当前源码能从 JSONL 为 2610/2664 个会话解析出真实 model。
...
| **L1 记录类型** | 信封的种类 | `.type` | `session_meta`、`response_item`、`event_msg`、`turn_context`、`compacted`、`world_state` |
...
1 个 `session_meta`、1 个 `turn_context`。它们在磁盘上占主导,但磁盘记录集不是封闭集合。
官方已确认(2026-06-21):权威的 `RolloutItem` 枚举
...
加入下表。2026-07-02 的本机 corpus 还发现 200 条顶层 `world_state` 记录,出现在当前
desktop/Responses 会话中;应把它视作环境状态快照,不要把较早的官方 enum 检查当成穷尽集合。

| L1 `type` | 角色 | 用途 | 何时发出 | Engram 是否消费? |
|---|---|---|---|---|
| `session_meta` | 每个 rollout 的**第一行**(头部) | 会话身份 + 环境 | 每文件恰好 1 个(第一个生效) | **是** — 身份 |
| `turn_context` | 每回合运行时配置快照 | 该回合的 model/sandbox/approval/effort/mode | 每回合 ≥1(仅较新 CLI;0.60.1 中缺失) | **是** — model fallback |
...
| `world_state` | 环境/context 快照 | desktop/runtime resume 的完整 world-state/context 包 | 当前桌面会话中少量出现 | **否**(被忽略) |

> Engram 在 `session_meta`、`turn_context`、`response_item` 和 `event_msg` 上分支。它只把
> `turn_context.payload.model` 用作 session model fallback;`compacted`、
> `inter_agent_communication`、`world_state` 以及所有非 `token_count` 的 `event_msg` 仍被忽略。
...
| `model` | string | meta 中很少出现;仅当未见到 `response_item.payload.model` 或 `turn_context.payload.model` 时 Engram 才使用它 | 当前 corpus 中罕见/不存在 | `"gpt-5.5"` |
...
- *文本键:* 块的判别符是 `type:"input_text"`/`"output_text"`,但实际字符串**始终**在键
  ...
  数据**。
- *多块文本:* Swift 和保留的 TS 现在都会在跳过非文本块后,用 `\n\n` 连接所有保留的
  text block;多段 user/assistant 内容不再被截断为第一个块。
- *缺失每项用量:* 两个适配器都读取 assistant 的 `payload.usage` 作为每条消息的用量来源,
  但对现代文件的扫描发现**零**个
  `response_item.payload.usage`。每回合用量完全来自 `event_msg/token_count`。这是一条潜伏的
  代码路径,如果未来 Codex 重新加入内联用量,它会悄然取得优先权。
- *项上缺失 model:* 现代文件不携带 `response_item.payload.model`(2026-07-01 本机 corpus
  中 0 次);真实 model 名存在于 `turn_context.payload.model`(48,834 条记录)。2026-07-01 的
  adapter 修复让 TS 和 Swift 都在保留旧 `response_item` fallback 的同时读取
  `turn_context.payload.model`。当前源码能为本机 2610/2664 个 Codex 会话解析出真实 model;
  剩余 54 个旧/稀疏会话没有可用 model 字段。
...
消息发出,用于 transcript display。因此完整 stream 的 transcript 长度可以合理地大于
`SessionInfo.messageCount`:2026-07-02 的保留 TS smoke 解析出 701,043 条 session-counted
messages,但流式输出 1,195,657 条 display messages;2,564 个会话因 `function_call_output`
可展示但不作为额外 tool use 计数而存在差异。`custom_tool_call*`、`web_search_call`、
`tool_search_*` 以及所有 `event_msg` 工具遥测都被丢弃。
...
| `source` | 计数 | 备注 |
|---|---|---|
| `{"subagent":{"thread_spawn":{...}}}` | **1623** | **最大的单一类别** — 原生派生的 subagent;每个 blob 仅在嵌套的 parent/depth/role 上不同,但都共享此前缀。**占全部 2664 个 thread 的 61%。** 恰好等于 `thread_spawn_edges` 行数(1623)。 |
| `cli` | 481 | 交互式终端 |
| `vscode` | 471 | Codex VS Code / 桌面应用 |
| `exec` | 65 | 无头 `codex exec` |
| `{"subagent":"review"}` | 18 | review subagent(简单字符串形式;不在 `thread_spawn_edges` 中) |
| `unknown` | 6 | 未分类 |

> 此存储**以 subagent 为主**:`{subagent:{thread_spawn}}` 形式是占主导的单一类别(1623),
> 而非分散的长尾。任何把 `source` 当作小型字符串枚举的天真消费者都会错误分类多数行。(证据:
> `sqlite3 state_5.sqlite` 对 `source` 前缀 GROUP BY;`thread_spawn_edges` COUNT = 1623。)
...
| 路径 | 大小 | 状态 | 角色 |
|---|---|---|---|
| `~/.codex/state_5.sqlite` | 16 MB | **现行** | 线程目录 + agent-job + spawn 图(迁移 **40**,2664 threads) |
...
(最新;遗留 DB 缺少 `recency_at`/`recency_at_ms` 列以及迁移 40 的 thread-history-mode
增量)。这就是*为什么*有四个 DB 文件:
...
#### `threads` — rollout 会话索引(2,664 行;每个 rollout 一行)
...
#### `thread_spawn_edges` — subagent 父→子图(1,623 行)
...
| `status` | TEXT | `closed`(884) / `open`(739) — 推断的存活 vs 完成的派生关系 |

**已验证引用完整性:** 全部 1623 条边的两个端点都在 `threads` 中。顶层父级扇出很广(一个父级
→ 129 个子级)— 派发 subagent 集群的编排者。`threads.source` JSON 内部的 `parent_thread_id`
...
| `response_item.payload.model` | `model`(旧/遗留 item model;首次出现,最高优先级) | L92-96 | L311-313 |
| `turn_context.payload.model` | 现代 rollout 的 `model` fallback | L83-88, L149-152 | L298-303, L355 |
| `session_meta.payload.model` | 若无 item/turn-context model 则作最终 `model` 回退 | L152 | L355 |
...
| `reasoning`、`custom_tool_call*`、`web_search_call`、`tool_search_*`、`compacted`、`world_state`、所有非 `token_count` 的 `event_msg` | **丢弃**(默认分支) | (缺口) | `message(from:)` `default` L535-536 |
...
- **官方已确认:** 没有任何 `ResponseItem` 变体带有 `model` 字段;model 位于
  ...
  `response_item.payload.model` —— model 来自 `turn_context`。Engram 现在消费该字段用于
  session model 归因。
  ([models.rs](https://github.com/openai/codex/blob/main/codex-rs/protocol/src/models.rs),
  [protocol.rs](https://github.com/openai/codex/blob/main/codex-rs/protocol/src/protocol.rs))
...
- **Engram 内部设计 —— 无法通过 web 验证:** Swift 产品是否有自己的定价路径,以及
  `turn_context` 只被用于 model fallback、`compacted`/`inter_agent_communication`/
  `world_state` 被 Engram 忽略,都是 Engram 内部事实,超出 web 范围。
