...
> 1. **磁盘上的实时存储** `~/.commandcode/` —— **11 个项目目录**、**29 个会话 `.jsonl` 文件**(不含检查点)、**21 个 `.checkpoints.jsonl` 文件**、**18 个 `.meta.json` 文件**、**1,385 条消息记录**(全部为 `metadata.version: 2`),外加 CLI 全局文件(`config.json`、`auth.json`、`history.jsonl`、`updates.json`、`trusted-hooks.json`、`plans/`、`file-history/`)。
...
>
> 下文中的每一项 schema/字段断言都已在 2026-06-21 针对实时存储重新核验(顶层键集、metadata 键集、内容块类型计数、工具输入/输出类型、角色分布)。实时数据、fixture 与适配器之间的差异在文中就地标注。

---

## 1. 概述与 TL;DR

**CommandCode**(provider 字符串 `command-code`)是一个**多提供方的 CLI AI 编码 agent** —— agent 外壳是"提供方",而底层 LLM 是可配置的(实时 `config.json` 中携带 `deepseek/deepseek-v4-pro`;按会话的 `.meta.json` 文件携带 `Qwen/Qwen3.7-Max`、`deepseek/deepseek-v4-pro`)。Engram 将其视为名为 `commandcode` 的单一来源。
...
- **如何:** 行分隔 JSON(JSONL);在当前的 CommandCode(v0.40.0)中,每次保存都是整文件原子重写(整个数组重新序列化、id 重新生成),而非追加 —— 见 §3。无滚动切分。按会话的边车文件(`.meta.json` 标题/模型、`.checkpoints.jsonl` 文件历史快照)以及独立的 `file-history/<sessionId>/` blob 存储保存 UX/恢复状态。
...
| `<projectSlug>/<sessionId>.jsonl` | **JSONL transcript** | **是 —— 会话本体** | 仅追加的消息日志 |
...
**cwd 解码**(`decodeCwd`,Swift `:226-233` / `decodeCwdFromLocator`,TS `:183-190`):将 `--`→`\0`、`-`→`/`、`\0`→`-`。**仅在**没有记录携带 `cwd` 时作为回退使用。在实时数据中,记录上从不存在 `cwd`(0/1,385),因此解码出的 slug 永远是 cwd 来源 —— 而它是近似的,并非忠实的路径(例如 `users-bing-net-work-safeline` → `users/bing/net/work/safeline`,丢失了前导斜杠以及任何字面连字符)。没有任何实时项目目录包含 `--`,因此双连字符转义路径只在合成测试中被覆盖。
...
- **写入模型:整文件原子重写,而非追加。** 已订正(官方):当前 CommandCode(v0.40.0)在每次保存时都会重新序列化内存中**整个**消息数组,写入 `<file>.<pid>.tmp`,再 rename 覆盖会话 `.jsonl`(整文件原子重写)。每次保存都会重新生成**所有**记录 `id`(全新 `crypto.randomUUID()`),并从 `this.lastMessageId` 重新计算 `parentId`,而 `lastMessageId` 初始为 `null` 且从不从被恢复的数据中播种 —— 因此记录 `id`/`parentId` 在多次保存间**不**稳定,且第一条记录的 `parentId` 为 `null`。`appendFile` 仅用于日志、hook 审计与全局 `history.jsonl` —— 从不用于会话转录。实时存储所呈现的仅追加 + 单调 id + 首条非空 `parentId` 行为反映的是一个**更早的** CommandCode 版本;磁盘上的**结果**不变(每会话一个 JSONL 文件、相同的 8 键信封),因此下游解析不受影响。([source](https://registry.npmjs.org/command-code/-/command-code-0.40.0.tgz))
...
- **恢复 / 链表式续接:** 记录通过 `parentId` 链构成(链式 DAG),且**仅在文件内部**。实时存储中**第一条记录的 `parentId` 非空且指向该文件中不存在的 id**,反映的是更早的追加式版本;在当前源码中 `parentId` 仅为文件内链,且第一条记录的 `parentId` 为 `null`(如 **fixture** 根记录所示)。CommandCode **不**跨被恢复的文件串接 `parentId`。(见 §15,已解决。)
...
|---|---|---|---|---|
| user message | `role: "user"` | 80 | 人类提示,或注入的系统包装 | 计为 user,若匹配 `isSystemInjection` 则重分类为 system |
| assistant message | `role: "assistant"` | 940 | 模型回复(text / reasoning / tool-call) | 计为 assistant |
| tool message | `role: "tool"` | 365 | 工具执行结果 | 计为 tool |
...
| 文件 | 记录种类 | 判别符 | 用途 |
|---|---|---|---|
| `.checkpoints.jsonl` | 文件历史快照 | `type: "file-history-snapshot"`(仅见此值,65/65) | 撤销/恢复锚点 → `file-history/` 中的备份 blob |
...
每条转录记录都恰好有 **8 个顶层键** —— 在全部 1,385 条实时记录中验证为一致(`["content","gitBranch","id","metadata","parentId","role","sessionId","timestamp"]`)。该信封对三种角色完全相同。实时数据中**没有可选的顶层键**。
...
| `parentId` | string \| null | 同一文件内前一条消息的 id(文件内链);tool 记录的 `parentId` = assistant 记录的 `id`。已订正(官方):在 v0.40.0 中写入器从 `lastMessageId` 重新计算它(初始 `null`,从不从恢复中播种),因此第一条记录的 `parentId` 为 `null` ([source](https://registry.npmjs.org/command-code/-/command-code-0.40.0.tgz)) | 否 | 当前/fixture root: `null`;更早版本的实时存储: `"410a7034-…"`(非空) |
...
| `content` | array \| string | 消息负载(见 §6);1,371/1,385 为数组,14 条为裸字符串(仅 user) | 否 | `[ {type:"text",…} ]` |
| `metadata` | object | 每条记录的来源信封(见下) | 否 | `{source,timestamp,version,…}` |
| `cwd` | string | 工作目录 | **是 —— 在所有实时记录中缺失(0/1,385)**;在 fixture 中存在 | (live: never) |
| `model` | string | 模型 id | **是 —— 在所有实时记录中缺失(0/1,385)**;在 fixture 中存在(仅 assistant) | (live: never) |
...
`metadata` 始终存在。观察到 5 种键集变体(实时);所有键的并集:

| 字段 | 类型 | 含义 | 可选(在 1,385 中的频次) | 示例 |
|---|---|---|---|---|
| `source` | string | 来源通道;常量 `"cli"` | 始终(1385) | `"cli"` |
| `timestamp` | string (ISO) | 顶层 `timestamp` 的镜像(始终相等);适配器回退 | 始终(1385) | `"2026-05-25T01:56:44.064Z"` |
| `version` | int | 记录 schema 版本;常量 `2` | 始终(1385) | `2` |
| `messageId` | string (UUID) | 提供方 / UI 消息 id(该 schema 字段一路传递);检查点通过 `snapshot.messageId` 锚定到它。**始终不同于顶层 `id`**(66/66) | 可选(66) | `"685c1246-b555-4593-b068-4be3f4d72303"` |
| `entrypoint` | string | 调用模式;源码定义的**唯一**值是 `"print"`(常量 `Oh="print"`),仅由 `resolvePrintSession()` 在无头一次性 `--print`/`-p` 时赋值。交互式会话**不**写入 `entrypoint` 键。已订正(官方)([source](https://registry.npmjs.org/command-code/-/command-code-0.40.0.tgz)) | 可选(16) | `"print"` |
| `isAutomated` | bool | 机器生成 / 注入的回合;CLI 在自动化斜杠命令提示(`isAutomatedSlashCommandPrompt`)及类似非人类回合上设置它。已确认(官方)([source](https://registry.npmjs.org/command-code/-/command-code-0.40.0.tgz)) | 罕见(2) | `true` |
| `isMeta` | bool | meta 回合(`createMessageWithMeta`/`sanitizeMessage` 路径,如内容为空的 meta 回合)。已确认(官方)([source](https://registry.npmjs.org/command-code/-/command-code-0.40.0.tgz)) | 罕见(1) | `true` |
...
`entrypoint:"print"` 记录成对出现:一条 `content` 为**裸字符串**(一次性提示)的 `user` 记录,后跟一条带 `["reasoning","text"]` 的 `assistant` 记录 —— 即无头一次性查询。**整个实时存储中不存在任何 content 为空数组的 user 记录(0/80 user 记录)。** 全部 8 条实时 `entrypoint:"print"` user 记录都携带裸字符串内容(长度:5× ≈34 字节 + 3 条长内容:40,111 / 41,768 / 70,358 字节);这 8 条是 14 条字符串内容 user 记录的子集(8 条 `print` + 6 条无 `entrypoint`)。

---

## 6. 消息与内容 schema

`content` 是**内容块层**,嵌套在一条记录之下。它要么是**类型化块的数组**(1,371 条记录),要么是**裸字符串**(14 条记录,仅 user)。块由 `.type` 判别。
...
|---|---|---|---|---|
| `text` | `type`、`text` | 自然语言正文 | 814 | 逐字提取(Swift `:189-190` / TS `:198`) |
| `reasoning` | `type`、`text` | 模型思维链 | 170 | **丢弃** —— `extractContent` switch 中无对应分支(Swift `:188-201` default → nil / TS `:196-208`) |
| `tool-call` | `type`、`toolName`、`toolCallId`、`input` | 工具调用 | 584 | 在内容中渲染为 `` `<toolName>` ``;以 `NormalizedToolCall` 形式发出(名称 + 截断后的 input JSON) |
| `tool-result` | `type`、`toolName`、`toolCallId`、`output` | 工具输出(类型化包装) | 584 | `output` 折叠进内容(字符串逐字保留,否则 JSON 序列化并截断至 2000) |
...
| `text` | string | req | 思维链文本(170 条实时记录中全部非空) |
```json
...
| `input` | object (582) \| string (2) | req | 工具参数;形状因工具而异 | `{ "command": "<CMD>" }` |
```json
...
| 错误 | `tool-result.output.type == "error-text"` | 实时 2/584;否则为 `"text"` |

**不同 `toolName` 值 + 计数(实时):** `shell_command` 184、`read_file` 123、`explore` 99、`grep` 59、`read_directory` 29、`edit_file` 23、`glob` 22、`todo_write` 20、`read_multiple_files` 9、`write_file` 9、`enter_plan_mode` 5、`exit_plan_mode` 1、`think` 1。
...
**Engram 处理。** 两个适配器实际上都不做调用↔结果的连接。Swift `toolCalls`(`:206-219`) / TS `toolCalls`(`:213-228`)只把 `tool-call` 块提取为 `NormalizedToolCall{name, input(JSON, 截断 500B), output: nil}` —— `output` 始终为 nil(Swift `:217`)。结果文本仅通过 `extractContent` 折叠进扁平的内容摘要。Swift 接受 `input ?? args`(`:215`);TS 只读 `input`(`:224`)—— 这是一处一致性差异(见 §15 divergence 6)。实时数据使用对象 `input`,因此对象 JSON 序列化路径在生产中运行。

---

## 8. 推理 / 思考

**磁盘上存储:是。** `reasoning` 块(实时 170 条,仅 assistant)以纯文本携带模型的思维链(`{type:"reasoning", text:"…"}`),存在时位于块数组之首。
...
**在观察到的数据中缺失;schema 支持它,但未被填充。** 用户的实时存储不携带任何用量数据(已验证:0/1,385 条记录携带 `usage`;`jq paths | grep -iE 'usage|token|cost'` → 空),因此 Engram 无法读取。两个适配器都反映这一点:Swift `message()` 设 `usage: nil`(`:155`);TS 完全省略 usage。
...
已订正(官方):该边车并不限于 `{title, model?}`。`saveSessionMeta` 合并任意键;`saveSessionTitle` 写 `{title}`;重命名设置 `userRenamed`;fork 路径(`copyForkSessionFiles`,由 `--fork-session` 触发)写入额外的键。完整可能 schema 见下 ([source](https://registry.npmjs.org/command-code/-/command-code-0.40.0.tgz))。
| 字段 | 类型 | 可选 | 含义 | 示例 |
|---|---|---|---|---|
| `title` | string | req(18/18) | 人类可读会话标题(自动生成的摘要) | `"Review recent commits with sub-agents"` |
| `model` | string | optional(3/18) | 所用模型;**唯一的按会话模型记录** | `"deepseek/deepseek-v4-pro"`、`"Qwen/Qwen3.7-Max"` |
...
| 字段 | 类型 | 含义 | 示例 |
|---|---|---|---|
| `type` | string | 常量 `"file-history-snapshot"`(65/65) | `"file-history-snapshot"` |
...
| `backupTime` | string (ISO datetime) | 备份采集时间 —— 已确认(官方):类型化为 `string.datetime()`,故按 schema **不可能是数字**(58/58 非空条目;未观察到 `number`) |
| `version` | number | 备份版本 —— 已确认(官方):始终为正整数(`number.int().positive()`,58/58) |
...
2. `reasoning` 内容块(实时 170 条)—— 思维链被丢弃。
...
4. **GOTCHA(丢弃 reasoning/图像):** 实时数据中 170 个 reasoning + 4 个 image 块从转录与搜索文本中被静默丢弃。
...
7. **DIVERGENCE(`args` 回退):** Swift 对 tool-call input 读取 `input ?? args`(`:215`);TS 只读 `input`(`:224`)。Swift 一致性测试 `testCommandCodeAdapterAcceptsArgsForToolCallInput`(`macos/EngramTests/AdapterParityTests.swift:382`)覆盖 `args` 路径,但实时数据使用对象 `input`(582/584)—— `args` 与字符串输入都不是常见情形。

### 边界情况与版本漂移

8. **EDGE(字符串内容):** 14/1,371 条实时记录的 `content` 是裸字符串(非数组);两个适配器都逐字返回(Swift `:181-183` / TS `:193-194`)。
9. **EDGE(对象 output 强制 JSON 序列化):** 实时数据中 `tool-result.output` 为对象 584/584,因此逐字字符串路径从不运行;output 始终被 JSON 序列化并截断至 2000 字符 —— 大的工具输出在索引转录中被截断。
10. **EDGE(字符串 tool-call input):** 2/584 条实时 `tool-call` 块携带裸字符串 `input`(两个都是 `grep`);`jsonString`/`truncateJSON` 处理它们。
...
14. **FIXTURE 与 LIVE 的差异:** fixture `sample.jsonl`(3 条记录)被精心制作以覆盖本机实时数据中不存在的防御性路径 —— 它携带顶层 `cwd`/`model` 以及 `parentId: null` 根,分布在两种不同的记录变体上:**user/root** 记录是 **9 键**变体 `["content","cwd","gitBranch","id","metadata","parentId","role","sessionId","timestamp"]`(有顶层 `cwd`、**无** `model`)且 `parentId: null`;**assistant** 记录是加上 `model` 的 **10 键**变体(同时有顶层 `cwd` 与 `model`),`parentId: "msg-001"`(tool 记录是 9 键变体,`parentId: "msg-002"`)。实时存储既无顶层 `cwd`/`model`,也无空的首个 `parentId`(每条实时记录都是 8 键信封,且首个 `parentId` 非空)。注(web 已确认 2026-06-21):fixture 的 `parentId: null` 根实际上**与**当前 CommandCode(v0.40.0)**一致**,其写入器始终将第一条记录的 `parentId` 置为 `null`;实时存储的首个非空 `parentId` 反映的是更早的追加式版本,因此在这一点上偏离当前源码的是**实时数据**而非 fixture ([source](https://registry.npmjs.org/command-code/-/command-code-0.40.0.tgz))。fixture 的 `tool-call` input 是对象(`{path: …}`),与实时一致;其 `tool-result` output 是裸字符串(`"file contents omitted"`),与实时**不**一致(实时始终是 `{type,value}` 对象)。因此一致性测试通过,而实时行为丢弃了 model、cwd,并对每个 tool-result 做 JSON 序列化。

### 已解决问题(web 已确认 2026-06-21)
...
- **跨会话 `parentId`。** 已确认(官方):在 v0.40.0 中写入器(`SessionManager.writeMessages`)**不**追加 —— 它在每次保存时从内存数组重建整个消息列表,为每条记录 `id` 生成全新 `crypto.randomUUID()`,并设 `parentId = this.lastMessageId`,而 `lastMessageId` 在构造时为 `null` 且仅在重写循环内部更新(从不从加载/恢复的会话中播种)。因此第一条记录的 `parentId` 为 `null`,且链纯属文件内。恢复/续接(`loadMessages` → `resolvePrintSession`/`loadResumed`)会把先前消息重新加载进内存,但下一次保存仍从 `parentId: null` 重写整个文件。CommandCode **不**跨被恢复的文件串接 `parentId`;不会向单独文件写入前导/系统记录;文件名始终等于该文件自身的 `sessionId`(写入器设 `sessionFilePath = <sessionId>.jsonl` 并将每条记录的 `sessionId = this.sessionId`)。实时存储观察到的首条记录非空 `parentId`/稳定 id 最符合一个**更早的**追加式版本(早于 v0.40)。([source](https://registry.npmjs.org/command-code/-/command-code-0.40.0.tgz))
...
该 `user` 记录的 `content` 是**裸字符串**(一次性提示),而非空数组 —— 在全部 8 条实时 `print` user 记录中验证(长度 5× ≈34 字节 + 3 条长内容:40,111 / 41,768 / 70,358 字节);磁盘上不存在任何 content 为空数组的 user 记录。
```json
...
Web 确认于 2026-06-21 执行(`web_access_ok=true`)。npm bundle `dist/index.mjs` 是磁盘格式的权威来源。

- [command-code npm package (v0.40.0) tarball](https://registry.npmjs.org/command-code/-/command-code-0.40.0.tgz) —— 发布的 CLI bundle(`dist/index.mjs`),读写会话存储;磁盘格式的权威来源(zod schema + `SessionManager`)。
