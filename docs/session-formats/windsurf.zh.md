...
`WindsurfAdapter(enableLiveSync: false)`(`SessionAdapterFactory.swift:25,70`),因此
...
`defaultPath` 列为 `~/.codeium/windsurf/daemon`(`SourceCatalog.swift:38`),即发现
...
| **产品中的实时同步状态** | **关闭。** `WindsurfAdapter(enableLiveSync: false)`(`SessionAdapterFactory.swift:25,70`)。适配器*默认*为 `true`(`WindsurfAdapter.swift:116`)但工厂覆盖了它。`sync()` 提前返回;产品严格仅缓存。仅缓存状态在 `LiveSyncDisabledSources.ids = ["windsurf", "antigravity"]`(`macos/Shared/Service/LiveSyncDisabledSources.swift:15`)中被**规范化**,并在应用 UI 中作为 **"Cache only" 徽章**呈现给用户,因此实时同步关闭的状态被诚实展示,而不是被暗示为损坏/活跃的同步。 |
...
1. **产品中实时同步关闭(最大的坑)。** `enableLiveSync:false`
   (`SessionAdapterFactory.swift:25,70`)。适配器默认为 `true`
