...
> 1. **本机磁盘实存(LIVE on-disk store)** — `~/.local/share/opencode/opencode.db`(224.2 MB,WAL 模式;`-shm` 32 KB,`-wal` 0 B = 已 checkpoint)。22 张表,OpenCode 版本 `1.2.6`–`1.17.11`。计数:**22 个 project、391 个 session(全部 active,0 个 archived;170 个 root + 221 个 child)、7,455 行 `message`、36,356 行 `part`、190 行 `todo`、6 行 `session_message`、58 行 `event`**。已应用 21 个 Drizzle 迁移。
...
> 3. **Engram adapter(已编码)** — Swift 产品解析器 `macos/Shared/EngramCore/Adapters/Sources/OpenCodeAdapter.swift`(权威);TS 参考 `src/adapters/opencode.ts`(parity 镜像)。
>
> 2026-07-02 只读刷新:TS adapter 仍列出/解析 391/391 个 active 虚拟 locator,
> stream 出 3,153 条消息(647 user / 2,506 assistant),发出 2,496 条带 usage 的
> assistant 消息,parser/stream count mismatch 为 0。实时 Engram DB 仍有
> 391 条 `opencode` 行、0 条 `opencode` `file_index_state` 行、当前 id 缺失 0、
> DB-only id 0,以及 165 条来自旧 parser 语义的 count/end-time 陈旧行;
> `size_bytes` 已对齐。若把 downstream 的 `parent_session_id` 回填值也拿来和
> adapter 的 `nil` parent 输出比较,广义 DB-vs-parser diff 是 187 行;这些
> parent 值不是 parser-owned 陈旧字段。
...
- **存储技术:DB 而非文件,但 `message` 行会被例行性地反复触碰。** 内容在一个回合内作为新的 `message` + `part` 行追加写入,随后随着该回合/part 流定型,message 行的 `time_updated` 推进。实存证据:**7,449 / 7,455** 条 message 的 `time_updated > time_created`;只有 **6** 条 `time_updated == time_created`(且 0 条更早)。所以 `time_updated` 几乎在**每条** message 上都会推进——它跟踪的是回合/part 流的完成,**而非**罕见的重写。这些差值大多很小(6 个相等、31 个 <1s、6,358 个在 1s 至 <1min、1,060 个 ≥1min),符合定型而非后续编辑的特征。这里没有 JSONL 追加;持久性来自 SQLite WAL(`opencode.db-wal`)。
- **恢复(Resume)。** 恢复一个会话会在**同一个 `session.id`** 下追加更多 `message`/`part` 行;`session.time_updated` 推进。不会创建新文件(与 JSONL 工具相反,后者可能开启一份全新 transcript)。子 agent / 续接会话会获得一个非 NULL 的 `parent_id`(此处 391 中有 221 个)以及一个 `slug`。
...
|---|---:|---|---|
| `session` | 391 | ✅(5 列) | 每个对话一行;元数据、cwd、title、parent_id、time_*、model/cost/tokens 汇总 |
| `message` | 7,455 | ✅(envelope) | 每个回合一行;`data` JSON = role + 元数据;FK → session |
| `part` | 36,356 | ✅(仅 text) | 每个内容块一行;`data` JSON 按 `type` 区分;FK → message 与 session |
| `project` | 22 | ❌ | PK `id`;`worktree, vcs, name, icon_url, icon_color, time_* {created, updated, initialized}, sandboxes, commands, icon_url_override`(`time_initialized` 是一个独立的可空列) |
| `todo` | 190 | ❌ | 按会话的 todo 列表;PK `(session_id, position)`;`content, status, priority, time_*` |
| `session_message` | 6 | ❌ | **更新的**事件式表;`id, session_id, type, time_*, data, seq`;实存中确认只有 2 种 type:`agent-switched` / `model-switched` |
| `event` | 58 | ❌ | 通用事件溯源日志;`id, aggregate_id, seq, type, data`;实存 `type` 枚举完全可观测——6 种 type:`message.part.updated.1`、`message.updated.1`、`session.created.1`、`session.next.agent.switched.1`、`session.next.model.switched.1`、`session.updated.1` |
...
> **schema 处于迁移中的信号。** 已被完全填充的 `message`/`part`(7,455/36,356)与稀疏、较新的 `session_message`(6)共存,表明在较新的 OpenCode 构建中存在一项进行中的事件溯源迁移。Engram 正确地停留在被填充的传统 `message`/`part` 路径上。如果未来某个 OpenCode 版本把 transcript 文本移入 `session_message`,adapter 的 `message ⋈ part` JOIN 将需要更新——目前还不是问题。
...
| `parent_id` | text? FK | 父会话(子 agent 链接;索引 `session_parent_idx`) | 是 | **忽略**(未映射——见 §10) | `null`(root)/ `ses_…`(221/391) |
...
| `version` | text NN | 写入它的 OpenCode 版本 | 否 | 忽略 | `1.17.8` 样本;实存跨 `1.2.6`–`1.17.11` |
...
| `agent` | text? | 活动 agent 模式 | 是 | 忽略 | `build`(111/391 设置) |
| `model` | text?(JSON) | model id blob | 是 | **忽略**(Engram model=nil) | `{"id":"deepseek-v4-pro","providerID":"opencode-go","variant":"default"}`(111/391) |
| `cost` | real NN(默认 0) | 汇总会话成本(USD) | 否 | **忽略**(按 message 重新推导) | `0.03949974`(232/391 > 0) |
...
| `time_updated` | int NN(ms) | 定型标记——随回合/part 流完成而推进(7,449/7,455 > created;只有 6 == created) | 忽略 | 通常 > created |
...
**Part `type` 分布(实存,36,356 个 part):**

| `type` | 计数 | data key(实存) | Engram 使用? |
|---|---:|---|---|
| `tool` | 12,147 | `type, callID, tool, state{status,input,output,title,metadata,time}` | ❌ |
| `step-start` | 6,780 | `type`(其中 5,530 个有 `snapshot`) | ❌ |
| `step-finish` | 6,743 | `type, reason, cost, tokens`(其中 5,499 个有 `snapshot`) | ❌ |
| `reasoning` | 6,095 | `type, text, time` | ❌ |
| **`text`** | **3,519** | **`type, text`**(+ 可选 `time`、`synthetic`) | ✅ **仅此** |
...
形状与 `text` 相同但 `type=="reasoning"` → 被排除。**推理轨迹(6,095)虽被存储,但从不进入 Engram 的 transcript 或搜索索引。**
...
**已存储但不索引。** OpenCode 把模型的思维链持久化为 `type:"reasoning"` part(本存储中有 6,095 个)——形状与 `text` 相同 `{type, text, time}`。Engram 丢弃它们(只有 `type=="text"` 存活)。推理也以数值形式被记录在 `message.data.tokens.reasoning` 中,这一项 Engram **确实**会读取(折入 `outputTokens`)。所以 Engram 捕获推理 token 的*数量*,却从不捕获推理轨迹的*内容*。
...
1. `session.parent_id` — 已确认的父链接(FK,索引 `session_parent_idx`)。本存储中 **391 个会话里有 221 个是 child**。
...
|---|---|---|---|
| `id` | `session.id` | 直传;回退到定位符的 sessionId | `:182`(`:189`) |
| `source` | 常量 | `.opencode` | `:88,:183`(`:67,:190`) |
| `summary` | `session.title` | 空 → `nil` | `:196-199`(`:199`) |
| `cwd` | `session.directory` | 直传;NULL 则 `""` | `:188`(`:193`) |
| `project` | — | **永远 `nil`**(不从 `project.worktree`/`name` 推导) | `:189`(TS 省略该字段) |
| `model` | — | **永远 `nil`**(尽管 `session.model` 存在) | `:190`(TS 省略) |
| `startTime` | 首个 `message.time_created`,否则 `session.time_created` | epoch ms → ISO8601(÷1000) | `:175-178`(`:139-147`) |
| `endTime` | 末个 `message.time_created` | Swift:仅当 `messages.count > 1`;TS 参考:当 `messages.length > 0`(§15) | `:185-187`(`:140-147`) |
| `messageCount` | 派生 | `userCount + assistantCount`(仅 text-part——见 §15) | `:191`(`:194`) |
| `userMessageCount` | 派生 | role=user 且有 ≥1 个非空 text part 的不同 `msg.id` | `:158-172,:192`(`:149-166,:195`) |
| `assistantMessageCount` | 派生 | role=assistant 且有 ≥1 个非空 text part 的不同 `msg.id` | `:158-173,:193`(`:149-167,:196`) |
| `toolMessageCount` | — | 硬编码 `0` | `:194`(`:197`) |
| `systemMessageCount` | — | 硬编码 `0` | `:195`(`:198`) |
| `sizeBytes` | 本会话的 `Σ length(message.data) + Σ length(part.data)` | 按会话的 SQL 字节求和 | `:201,:269-298`(`:169-186,:201`) |
| `filePath` | 虚拟定位符 | `"{dbPath}::{id}"` | `:200`(`:200`) |
...
|---|---|---|---|
| `role` | `message.data.role` | 只有 `user`/`assistant` 存活 → `.user`/`.assistant` | `:361-363,:378`(`:325-337,:246`) |
| `content` | `part.data.text`(回退 `part.data.value`) | **仅 `type=="text"` part**;空的丢弃;多个 part 用 `\n` 拼接 | `:322-324,:336-337,:368-370`(`:327-333,:242,:247`) |
| `timestamp` | `message.time_created` | epoch ms → ISO8601 | `:374-375`(`:339-342,:248`) |
| `toolCalls` | — | 永远 `nil`(tool part 被丢弃) | `:345`(TS 省略) |
| `usage` | `message.data.tokens`(仅 assistant) | 见 §9 token 映射 | `:381,:385-402`(`:249,:282-303,:343`) |

### 定位符辅助

| 关注点 | Swift:line | TS:line |
|---|---|---|
| 默认 db 路径 | `:93-95` | `:70-72` |
| `detect()` = fileExists | `:100-102` | `:75-77` |
| 列表查询 | `:108-114` | `:85-92` |
| 虚拟定位符构建 `"{dbPath}::{id}"` | `:116` | `:91-92` |
| 从右拆分(`lastIndexOf`/`.backwards`) | `:261-267` | `:101-112` |
| 只读打开 | `:8,14` | `:84,:123,:223,:270` |
| `isAccessible`(缓存 actor / 重开) | `:61-85,:249-259` | `:264-280` |
| 按会话字节求和 | `:269-298` | `:169-186` |
...
1. **消息计数大幅低估实际值(text-only 谓词)。** 只有当一条 message 拥有 ≥1 个非空 `text` part 时才计入(Swift `contentfulRole` `:311-328`,注释 `:147-148`;TS `messagePart` `:315-345`)。纯工具的 assistant 回合消失。2026-07-02 live DB 的真实影响:6,793 条 assistant `message` row 中只有 2,506 条 assistant message 带非空 text part → Engram 报告约 37% 的实际 assistant row。这是有意的(计数必须等于流式 transcript),但 Engram 的 OpenCode `messageCount` 是"可见文本回合"计数,而非回合计数。

2. **曾经的 TS ↔ Swift 计数分歧已修复。** 当前 TS `parseSessionInfo` 已对齐 Swift:只有 `messagePart()` 产生非空 text part 时才按不同 user/assistant message id 计数(`ts:149-167`),且 `streamMessages()` 会先合并同一 message 的所有 text part 再分页(`ts:235-256`)。旧的宽松 role-count 行为是 live DB 中 165 条 stale row 的来源,需要后续 reindex/cleanup 刷新。
...
7. **按会话体积,而非整文件体积(CLAUDE.md 注记)。** 全部 391 个会话共享同一个 224 MB 文件;`statSync(dbPath)` 会把 224 MB 归到**每个**会话(391× 过计,约 88 GB 幻影总量)。修复方案是对范围限定在 `session_id` 的 `length(message.data) + length(part.data)` 求和(Swift `sessionPayloadSize:269-298`;TS `:169-186`,注释 `:169-172`)。两种实现按构造逐字节相同。由 fixture 验证:`sizeBytes: 276` = message + part 字节之和。

8. **Live Engram DB 计数字段新鲜度落后于 parser 修复。** 2026-07-02 字段级复核发现 locator 覆盖仍闭合,但 `/Users/bing/.engram/index.sqlite` 中 165 条既有 `opencode` row 仍保留旧 message-count 语义。在这些 stale row 中,DB 总计为 6,032 message / 424 user / 5,608 assistant,当前 parser 对应为 2,217 / 409 / 1,808;当前 parser 总计仍为 3,153 / 647 / 2,506。`size_bytes` 已对齐;stale 字段是 count/end-time 字段,需要 reindex/cleanup。广义比较还会看到 145 个 `parent_session_id` 差异,原因是 downstream parent backfill 已填充 DB 行,而 adapter 有意返回 `parentSessionId=nil`;不要把这些计入 OpenCode parser 陈旧。

9. **单个 DB 内版本跨度大,但 message JSON 稳定。** 会话跨越 OpenCode **1.2.6 → 1.17.11**(头部:`1.3.13`×70、`1.15.13`×61)。`message.data`/`part.data` JSON 形状(role/time/tokens/parts)在此范围内**稳定**——v1.2.6 的 assistant message 与 v1.17.x 拥有相同的 `tokens.{input,output,reasoning,cache.{read,write}}` 嵌套。迁移只改了 `session`/`project` 列。adapter 在实践中对版本漂移健壮。

10. **以只读方式对活跃 WAL DB。** 可能在 checkpoint 前漏掉未提交写入(不会损坏)。未观察到索引端的 `wal_checkpoint`/重试;下一次扫描时的最终索引是事实上的处理方式。

11. **时间戳是 epoch 毫秒(13 位),ISO 前 ÷1000**(Swift `isoFromMilliseconds`;TS `new Date(ms)`)。`1782005887047` → `2026-06-21T01:38:07.047Z`。两个 adapter 都正确处理 ms。

### Engram 不消费的数据(明确的数据丢失清单)

1. **`session.parent_id`**(221/391 已填充)→ 原生子 agent 谱系不可见(§10)。
2. **会话级 `cost` + `tokens_*` 汇总** → 忽略;用量按 assistant message 重新推导。
3. **`session.model` / `message.data.modelID`**(111/391 + 按 message)→ `model` 永远 `nil`;Engram 不记录任何 OpenCode model。
4. **`session.agent`、`mode`、`slug`、`version`、`workspace_id`、`share_url`、summary/diff 统计、`revert`、`permission`、`metadata`。**
5. **所有非 text part** — tool(12,147)、reasoning(6,095)、patch(1,032)、file(25)、compaction(14)、step-start/finish(13,523)、subtask(1):36k 个 part 中约 33k 被丢弃。
