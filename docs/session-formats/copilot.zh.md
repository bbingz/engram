...
| 基础 | 详情 |
|---|---|
| **磁盘上的实时存储** | `~/.copilot/session-state/` —— 本机有 **465 个会话目录**。其中 **222** 个带有 `events.jsonl`；全部 465 个都带有 `workspace.yaml` + `checkpoints/`。此外还有存储级的 `~/.copilot/session-store.db`（5.1 MB SQLite，`schema_version = 4`）。事件类型普查是对多个大型 `events.jsonl` 文件聚合统计得到的；深入采样了一个 8643 事件的会话和一个 1990 事件的会话（`67b3717b-…`）。 |
...
**Engram 的视角（有损）。** Engram 只呈现 `session.start`、`user.message`、`assistant.message` 和 `session.shutdown`（用于 token 合计）。所有工具 I/O、hook、子代理、skill、reasoning、压缩、按消息的 tokens/model，以及 SQLite 镜像都被**丢弃**。在一个 465 目录的存储中，Engram 呈现 **222 个会话**（那些带有真实事件日志的）；其余 243 个无事件目录（空的检查点模板）被静默跳过。
...
**所用证据基础：** 实时存储（465 个目录，222 个带事件）+ 仓库 fixture（1 个合成会话 + 黄金 parity JSON）+ 两个适配器。实时数据具有权威性。
...
**每会话子项名称频率**（在全部 465 个目录上聚合统计，已实时核实）：

| 子项 | 类型 | 计数 / 465 | Engram 使用？ | 含义 |
|---|---|---|---|---|
| `workspace.yaml` | 文件 | 465 | **是**（元数据） | 会话元数据：id、cwd、repo、branch、时间戳、summary |
| `checkpoints/` | 目录 | 465 | **是**（回退 + 索引） | 检查点历史（`index.md` + 编号 `.md` 正文） |
| `files/` | 目录 | 465（大多为空） | 否 | 用户粘贴/附加的负载（`paste-<epochms>.txt`） |
| `research/` | 目录 | 465（观测到为空） | 否 | Web/研究产物（本存储中为空） |
| `events.jsonl` | 文件 | **222** | **是**（主日志） | 仅追加的事件流 —— 即转录文本 |
| `session.db` | 文件 | 150 | 否 | 每会话 SQLite：`todos`、`todo_deps`、`inbox_entries` |
...
**关键事实：** 在 465 个目录中，仅 **222 个带有 `events.jsonl`**。其余 **243** 个只有一个空的 `checkpoints/index.md` 模板（零数据行），会被 Engram **静默跳过**（参见 [§9 发现](#9-engram-如何发现并枚举会话)）。
...
每一行是一个 JSON 对象，**恰好含 5 个信封键**
（`type`、`id`、`parentId`、`timestamp`、`data`）。下面是实时观测到的**完整事件类型词汇表**（约 28 种不同类型；计数来自采样的大型会话，用以传达相对体量）。**Engram** 列标记了适配器解析的那 4 种类型。
...
扁平 YAML（`key: value`）。两个适配器都使用**朴素的逐行解析器**（不是真正的 YAML 解析器）：Swift `readWorkspace` 在每行的第一个 `:` 处分割，要求键匹配 `^\w+$`（`CopilotAdapter.swift:361-375`）；TS `readWorkspace` 使用 `/^(\w+):\s*(.+)$/`，并额外剥离匹配到的外层引号（`copilot.ts:364-378`、`stripYamlQuotes:438-446`）。两者都能挺过 ISO 时间戳中的冒号；嵌套/多行 YAML 会被静默丢弃。频率是在全部 465 个文件上的精确计数。

| 键 | 频率 /465 | 类型 | 含义 | Engram 使用 |
|---|---:|---|---|---|
| `id` | 465 | string(uuid) | 会话 UUID（覆盖目录名） | **是** → `Session.id` |
| `cwd` | 465 | string(path) | 工作目录 | **是** → `cwd`（回退：`session.start.context.cwd`） |
| `created_at` | 465 | ISO-8601 | 会话开始 | **是** → `startTime` |
| `updated_at` | 465 | ISO-8601 | 最后活动 | **是** → `endTime` 种子 |
| `summary_count` | 465 | int | summary/压缩的数量 | 否 |
| `git_root` | 219 | string(path) | 仓库根 | 否 |
| `branch` | 219 | string | Git 分支 | 否 |
| `repository` | 166 | string | `owner/repo` | 否 |
| `summary` | 159 | string | 预先生成的会话摘要 | **是** → `summary`（第 1 优先级） |
| `host_type` | 148 | string | Forge 类型（例如 `github`） | 否 |
| `user_named` | 141 | bool | `name` 是否由用户设置？ | 否 |
| `name` | 133 | string | 显示**标题**（常为 AI 生成） | **否** —— 见 [§14](#14-engram-映射) 数据丢失 |
| `client_name` | 14 | string | 客户端（`github/cli`） | 否 |
...
> **对 Dim 报告的更正。** `summary` 并**非** "罕见/缺失" —— 它出现在 **159/465** 个实时文件中，且 Engram 优先使用它而非首条用户文本回退。在深入采样的会话中，`summary` + `summary_count` 都存在。
...
| `copilotVersion` | string | CLI 版本 | `"1.0.63"` |
...
1. **`workspace.yaml summary:`** —— 预先生成的会话摘要（实时 159/465）。**Engram 使用它**作为第一优先级的 `summary`（`CopilotAdapter.swift:110`；`copilot.ts:129`），回退为首条用户消息的前 200 字符。
...
`workspace.yaml summary_count` 记录 summary/压缩的数量（全部 465 个实时文件都有取值；采样到的全部为 `0`）。
...
WAL 模式 SQLite（+`-shm`/`-wal`）。实时行计数：sessions=135、turns=236、checkpoints=9、session_refs=27、session_files=0、forge_trajectory_events=0、dynamic_context_items=0。**行计数滞后于文件系统**（135 个会话 vs 465 个目录）—— 它是一个派生/近期活动缓存，**不是**真相来源。
...
### 12.2 `~/.copilot/session-state/<uuid>/session.db` —— 每会话（150 个目录）
...
| 产物 | 位置 | 类型 | Engram | 含义 |
|---|---|---|---|---|
| `session.db` | `<uuid>/session.db` | SQLite | 否 | TODO + 代理间收件箱（[§12.2](#122-copilotsession-stateuuidsessiondb--每会话150-个目录)） |
...
1. **`workspace.name`** —— 一个真实的、常为 AI 生成的**标题**（实时 133/465）。Engram 转而使用首条用户提示的前 200 字符。最大的 UX 缺失。
...
- **Swift ≠ TS 的 YAML 引号处理（潜在 parity bug）。** TS 通过 `stripYamlQuotes` 剥离匹配的外层引号对（`copilot.ts:438-446`）；**Swift 不会**（`readWorkspace` 中没有等价逻辑）。实时：6 个带引号的 `name:` 值（无论如何都被忽略），今天有 0 个带引号的 `cwd`/`id` → 差异是**潜在的，而非活动的**。如果 Copilot 将来给 `cwd:`/`id:` 加引号，Swift 会保留字面引号而 TS 会剥离 → 漂移。由 TS 测试 `tests/adapters/copilot.test.ts:162-199` 覆盖；**不存在 Swift 对应测试。**
...
- **仅含表头的 `index.md`**（实时 243/465）→ 0 个条目 → 目录被**静默跳过**（无报错，会话从 Engram 中消失）。
- **`cwd: /` 的检查点会话**会成为根作用域、近乎空的 Session 行（常 `created_at == updated_at`）。
- **检查点回退未在实时数据中验证。** events 缺失 → 检查点索引这条路径在两个适配器中都已实现，但在本实时存储上**从未被触发**（每个已填充检查点的目录也都有 `events.jsonl`；243 个无事件目录的模板是空的）。仅凭磁盘无法证明它对真实回退数据的行为。
- **`copilotVersion` 漂移 / 无版本化的 schema。** 实时显示 `1.0.63`（以及存储中较旧的 `1.0.18`/`1.0.63`）。适配器很宽容 —— 它会忽略未知的 `type` 值，因此新类型（`skill.invoked`、`subagent.*`、`session.model_change`，这些都不在合成 fixture 中）都被容忍。对 `user.message`/`assistant.message`/`session.shutdown` 的破坏性重命名会**静默地把**一个会话**清零**。
...
- **SQLite 镜像滞后。** `session-store.db`（135 会话 / 236 回合 / 9 检查点）滞后于文件系统（465 目录）。它是派生/近期活动的；Engram 不读取它。
...
- **`forge_trajectory_events` / `session_files`** 带有工具/文件列却存在，但在本存储中为 **0 行**。**Confirmed (official, partial)：** `session_files` 是一个真实、有文档的表，其用途是记录 "every file touched during the session"（由两份独立的逆向工程文章佐证），所以它**确实**被设计为可填充；0 行的观测是存储特定的，并非死列。没有任何公开来源提及 `forge_trajectory_events` 或 `dynamic_context_items`（Copilot CLI 闭源；公开逆向工程只枚举了 6 张表），因此较新版本是否会填充 `forge_trajectory_events` **无法从公开来源确认**
  （[jonmagic](https://jonmagic.com/posts/github-copilot-session-search-and-resume-cli/)、
  [dfberry](https://dfberry.github.io/2026-04-16-session-storage-decision-guide)）。
- **243/465 个无事件目录**（约 52%）。**Confirmed (official)：** 它们是中止/从未输入提示的启动，而非清理产物。GitHub
