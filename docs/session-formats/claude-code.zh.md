...
**当前 native Engram 状态（2026-07-02）：** `~/.claude/projects` 是活跃移动语料。
2026-07-02 的全量 TS smoke（`2026-07-01T19:11:45Z`）列出 12,168 个 JSONL
文件，解析出 11,275 个 conversation；语料包含 1,527,155 条非空 JSONL 行，
其中 4 条 malformed；来源分布为 11,270 个 native `claude-code` 加 5 个
native MiniMax model-hint session。该两阶段扫描最初出现 5 个 stream/count
mismatch，因为 5 个活跃 `mediahub` workflow subagent 文件在 parse pass 与
stream pass 之间继续增长；随后对这 5 个文件即时逐文件 parse+stream 复查，
计数全部匹配。稍后的 identity-only 复查（`2026-07-01T19:13:36Z`）已经移动到
11,279 个 parsed conversation 和 11,148 个 unique id。live DB freshness 并不
干净：99 个当前 `mediahub` id 仍没有 `sessions` 行或 `file_index_state`；142 个
当前 locator 不是 DB locator-current，但其中 43 个是 duplicate-id
`coding-memory` locator，其 id 已在另一个 locator 上存在。严格的当前
parser-vs-DB 比较仍发现 3,224 个 native Claude Code 行有 7,570 个字段差异：
主要是旧的 non-visible tool-result 计数（`message_count` x2,924、
`tool_message_count` x2,921），以及 `agent_role` x1,035、`parent_session_id`
x417、`size_bytes` x137、`cwd` x131、3 个 `assistant_message_count` 和 2 个
`user_message_count`。这应视为 live DB reindex/cleanup debt，而不是当前 parser 的格式不匹配。
...
通过不同 `CLAUDE_CONFIG_DIR` 启动 Claude Code 时,磁盘上的 **文件格式不变**。用户的
`cc-*` wrapper 设置 `CLAUDE_CONFIG_DIR="$HOME/.claude-$name"`,因此同一套
`projects/*` 语法也会出现在:
...
当前源码通过带 provider root 和固定 provider `source` 的 `ClaudeCodeAdapter` 处理这些
目录,同时保留 `originator: "Claude Code"`。2026-07-02 对 11 个 root 的 live smoke
列出 **8,623** 个 JSONL 文件,解析出 **8,409** 个 conversation 文件,发现 **8,289**
个 subagent transcript。已安装 `/Applications/Engram.app` build `20260701074505`
有 **8,015** 个 provider-root session 行；剩余 provider-root delta 是 active-write/retry
frontier（227 个 `.claude-glmc` 文件和 167 个 `.claude-openai` 文件，其中 1 个已标记为
`retry/malformedJSON`），不是缺少 provider-root 支持。TS 和 Swift 都会在应用
offset/count 分页之前从 stream 中跳过 user 形态的 system injection 和非可见
`tool_result` 记录。
...
| `<uuid>/subagents/workflows/wf_*/agent-*.{jsonl,meta.json}` | 嵌套工作流 subagent | Claude Code / polycli | 当前源码中 JSONL **Yes**;`.meta.json` No |
...
- 是目录 → 查找其 `subagents/` 子目录,并递归产出其中的每个 `*.jsonl`,
  包括 `subagents/workflows/wf_*/agent-*.jsonl`。
...
`/subagents/` 的文件路径** 推断“subagent”(TS `claude-code.ts:166`;Swift
`ClaudeCodeAdapter.swift:153`)。

### 10.5 Parent linkage derivation (Engram "Layer 1")

两个适配器都从 `subagents/` **紧前** 的路径段(以父会话命名的目录)提取父 UUID。
确定性,无启发式:
- **TS**(`claude-code.ts:245-248`):按 `/`
  切分,找到 `subagents` 索引,返回 `parts[subagentsIndex - 1]`。
- **Swift**(`ClaudeCodeAdapter.swift:568-575`,`parentSessionId(from:)`):按 `/`
  切分,找到 `subagents` 索引,返回 `parts[subagentsIndex - 1]`。

两者都设置 `agentRole:"subagent"` 和 `parentSessionId = <parent-uuid>`。

> **当前源码 vs 已安装运行时状态(2026-07-01)。** `subagents/workflows/wf_<id>/agent-*.jsonl`
> 的历史 discovery 缺口已在当前 Swift 和 TS 源码中通过递归扫描 `subagents/` 修复。
> 当前 live DB 在 `~/.claude/projects` 下已有 5,258 条 nested workflow subagent 行,
> 因此 nested workflow 可见性缺口已关闭。`workflows/<wf>/journal.jsonl` 仍是非
> conversation 日志,没有 user/assistant turn 时会被跳过。
...
3. **是 `Agent` 不是 `Task`。** subagent dispatch 工具是 `Agent`。`Task*` 是后台任务
   ...
   ([sub-agents docs](https://code.claude.com/docs/en/sub-agents))。
4. **工作流嵌套 subagent 源码已覆盖且已索引。**
   `subagents/workflows/wf_*/agent-*.jsonl` 现在会被两个适配器递归发现。当前 live
   DB 在 `~/.claude/projects` 下已有 5,258 条 nested workflow subagent 行。`journal.jsonl`
   仍是非 conversation 日志并会被跳过。
