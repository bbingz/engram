...
>
> 2026-07-02 只读刷新:TS adapter 仍列出 59 个虚拟 locator,
> 解析 8 个真实对话,拒绝 51 个 metadata-only composer,stream 出 345 条消息
> (86 user / 259 assistant),发出 46 条带 usage 的 assistant 消息,且
> parser/stream count mismatch 为 0。实时 Engram DB 仍有 59 条 `cursor` 行,
> 没有 `cursor` `file_index_state` 行,51 条历史零消息行,以及 6 条非空行仍带有
> 陈旧的整库 `size_bytes=29581312`。
...
- **滚动:无。** 没有按天 / 按大小的文件切分。所有时间段的所有 composer
  ...
  (`CursorAdapter.swift:71-76,94`、`cursor.ts:99-113,160`)。
...
适配器都用回退链来处理(`CursorAdapter.swift:190-218`、
...
3. **空 / 草稿** —— 既没有非空的 `conversation`,也没有匹配的
   `bubbleId:` 行 → 当前适配器把它拒绝为非对话。**实时:51 / 64 (~80%)**,
   现在都会解析为 null/failure,不再产生 0-message session。
...
| `latestConversationSummary` | object | `{ summary, lastBubbleId }` → Engram `summary`(≤200 字符) | yes (6/64) | **yes** | 2026-07-01 修复后支持字符串和嵌套对象 |
| `context` | object | 附加的文件/文件夹/终端/git/docs/规则 + `mentions` | no | yes(cwd) | 第一个 folderSelection,否则第一个 fileSelection 的 dirname |
...
(`CursorAdapter.swift:64-67`)。TS 适配器现在也镜像这条回退链
(`cursor.ts:136-140`),再通过 `new Date(createdAt).toISOString()` 转换。
Swift 侧时间戳通过 `isoFromMilliseconds = isoFromSeconds(ms / 1000.0)`
转换(`GeminiCliAdapter.swift:49-51`)。

**`latestConversationSummary` —— 现代嵌套摘要形态(2026-07-01 已修复)。**
...
```
旧适配器只把 `latestConversationSummary.summary` 当作 String 读取,所以会
丢掉现代对象形态。2026-07-01 修复后,两个适配器都会递归提取 `summary`
路径上的第一个字符串:
- Swift `CursorAdapter.swift:70,252-258` → `summaryText(from:)`,并在 `:92`
  截断到 200 字符。
- TS `cursor.ts:142,286-298` → `summary()` / `summaryText()`,并截断到
  200 字符。

**在所有 6 个带摘要的实时 composer 中,`summary.summary` 都是 OBJECT
(0 个字符串)。** 修复后,Engram 会摄入现代 Cursor 摘要,而不是为了预览
回退到第一条可见 user 消息。
...
存在三个用量面;Engram 只消费按消息的那个。

| 面 | 位置 | 类型 | 是否消费 |
|---|---|---|---|
| 按消息 token | `bubble.tokenCount = {inputTokens, outputTokens}` | object | **Swift + TS** → message `usage` |
...
- Swift 为 assistant 消息把按 bubble 的 `tokenCount → TokenUsage` 映射
  (`CursorAdapter.swift:149-150,288-298`),其中 `cacheReadTokens=0`、
  `cacheCreationTokens=0`,并有一个**丢弃零 token 用量**的防护
  (`inputTokens>0 || outputTokens>0`)。TS 现在镜像同一行为
  (`cursor.ts:233-239,305-318`)。最新 2026-07-02 live TS smoke 发出 46 条
  带 usage 的 assistant 消息;实时样本中 `{"inputTokens":0,"outputTokens":0}`
  的 bubble 仍不发出用量。
...
Engram 会把 `latestConversationSummary.summary`(≤200 字符)摄入
`summary`。自 2026-07-01 起,两个适配器都支持现代嵌套对象形态
(`summary.summary`)和旧字符串形态,因此 6 个实时现代摘要 composer 不再
丢失会话摘要。
...
**适配器注册。** 源枚举 `case cursor`；由
`SessionAdapterFactory.defaultAdapters()` / `recentActiveAdapters()` 构造，并由
`MessageParser` 通过 adapter-registry fallback 路径使用。TS `CursorAdapter`
为保留工具链镜像 Swift product adapter。

### 会话级(`NormalizedSessionInfo`)

| Engram 字段 | Cursor 来源 | Swift file:line | TS file:line | 备注 / 差异 |
|---|---|---|---|---|
| `id` | `composerData.composerId`(回退:locator composerId) | `CursorAdapter.swift:80` | `cursor.ts:145` | UUID |
| `source` | 常量 `.cursor` / `'cursor'` | `:81` | `:146` | — |
| `startTime` | `createdAt` → 否则第一个可见 bubble 的 `timingInfo.clientStartTime` → 否则 `lastUpdatedAt` → 否则 0 | `:64-68,82` | `:136-140,147` | Swift 和 TS 回退链已对齐 |
| `endTime` | `lastUpdatedAt`(仅当 ≠ createdAt,否则 `nil`) | `:69,83` | `:141,148-151` | — |
| `cwd` | 第一个 `folderSelections[].uri.fsPath`,否则 `dirname(第一个 fileSelections[].uri.fsPath)`,否则 `""` | `:84,260-286` | `:152,249-254` | 仅 best-effort:Cursor 不把 composer 绑定到 workspace;实时 folderSelections 0/64,fileSelections 8/64 |
| `project` | 始终 `nil` | `:85` | (TS 形态中无) | composer 不绑定到 workspace |
| `model` | 始终 `nil` | `:86` | (缺失) | 不提取 |
| `messageCount` | userCount + assistantCount(可见 bubble) | `:87` | `:153` | — |
| `userMessageCount` | 可见 `type==1` 的计数 | `:62,88` | `:122-132,154` | — |
| `assistantMessageCount` | 可见 `type==2` 的计数 | `:63,89` | `:122-132,155` | — |
| `toolMessageCount` | **硬编码 0** | `:90` | `:156` | `toolFormerData` 不计数 |
| `systemMessageCount` | **硬编码 0** | `:91` | `:157` | Cursor 没有 system bubble |
| `summary` | `latestConversationSummary.summary` / 嵌套 `summary.summary`,截断为 200 字符 | `:70,92,252-258` | `:142,158,291-303` | 支持字符串和现代嵌套对象形态 |
| `filePath` | 虚拟 locator `<db>?composer=<id>` | `:93` | `:159` | — |
| `sizeBytes` | 按会话 = `len(composerValue)` + Σ `len(bubble rows)` | `:71-76,94` | `:99-113,160` | 不是整个 28 MB 文件(对等注释) |
| `indexedAt` | `nil` | `:95` | (缺失) | 下游设置 |
| `agentRole`/`originator`/`origin`/`parentSessionId`/`suggestedParentId`/`tier`/`qualityScore`/`summaryMessageCount` | 全部 `nil` | `:96-103` | (缺失) | 由下游管线设置 |
...
| `content` | `text` ‖ `rawText`(非空) | `:233-235` | `:225-226` |
| `timestamp` | `timingInfo.clientStartTime`(ms→ISO) | `:140-143` | `:232,238` |
| `usage` | 仅 assistant:来自 `tokenCount` 的 `{inputTokens,outputTokens}`(cache=0;零用量被丢弃) | `:149-150,288-298` | `:233-239,305-318` |
| `toolCalls` | 始终 `nil` | `:148` | (缺失) |
...
3. `parseSessionInfo(locator)` —— 在 `?composer=` 处分割(`parseVirtualLocator`,
   `CursorAdapter.swift:174-180`;`parsePath`,`cursor.ts:321-331`),取
...
**被 Engram 完全丢弃:** `name`(聊天标题)、`toolFormerData`(所有工具调用/结果)、
`codeBlocks`/`codeBlockData`/`codeBlockDiff`(所有编辑/diff)、
...
1. **消息顺序依赖 `rowid`,而非头部清单。** Engram 按 SQLite
   `rowid ASC` 对分离 bubble 排序(`CursorAdapter.swift:205`、
   `cursor.ts:203`),而非按 `fullConversationHeadersOnly[]`。该清单是
   ...
   (而非清单)是一个可行但未验证的风险
   ([source](https://github.com/vltansky/cursor-chat-history-mcp/blob/main/docs/research.md))。
2. **摘要嵌套漂移已修复。** 现代存储把 `summary` 嵌套为
   对象(`{summary, truncationLastBubbleIdInclusive, …}`);2026-07-01 后的
   适配器同时读取旧字符串形态和现代嵌套形态,并有 TS/Swift 回归保护。
3. **标题(`name`)被忽略**,尽管它存在(8/64)。现在嵌套摘要已摄入,
   所以紧急程度降低;但 `name` 仍是现代 Cursor composer 数据中唯一显式
   标题字段。
4. **格式三分 + NULL。** 实时:4 内联 / 4 仅头部 / 51 空 /
   5 NULL(共 64)。当前适配器会拒绝 metadata-only composer,但实时
   Engram DB 仍有 51 条历史零消息 `cursor` 行,需要后续 cleanup/reindex。
   2026-07-02 字段级复核还发现 6 条非空 live DB row 仍带有旧的整库
   `size_bytes` 值;当前解析器已改为按 composer payload 计算。
5. **cwd 是 best-effort。** Swift 和 TS 都优先从 folderSelections 推断,
   然后用 fileSelections。大多数实时 composer 仍会发出 `""`:
   folderSelections 为 0/64;8 条 raw composer 记录带 fileSelections,但当前
   live 解析器只为 2 条可解析 session 推出 cwd,因为 metadata-only composer
   会先被拒绝,不会成为 session。
...
7. **稀疏时间戳。** 只有 assistant bubble 携带 `timingInfo`(恰好
   ...
   时间戳。
8. **Token 用量对等已修复。** Swift 和 TS 现在都会从 `tokenCount` 发出
   按消息的 assistant usage,并使用相同的零用量防护。Focused TS test 覆盖
   非零 `{inputTokens, outputTokens}`,live TS smoke 发出 46 条带 usage 的
   assistant 消息。
...
10. **Locator 耦合。** Locator 为 `<dbPath>?composer=<id>`;解析在
    字面 `?composer=` 处分割(`CursorAdapter.swift:174-180`、`cursor.ts:321-331`)。一个
...
- **现代嵌套摘要形式。** Confirmed (official):对象嵌套是真实的产品行为,
  ...
  处读取文本;Engram 现在跟随这个嵌套路径,同时保留旧字符串形态。Cursor 把这种嵌套视为 bug 还是
...
- **cwd 推断来源。** (Engram-internal design - not web-verifiable.) 格式前提
  已验证:`context.fileSelections[].uri.fsPath/path` 存在于 composer 和
  bubble 上,所以 Engram 用这些字段进行 best-effort cwd 推断。是否纳入更多
  context 字段仍是产品设计决策
  ([source](https://github.com/vltansky/cursor-chat-history-mcp/blob/main/src/database/types.ts))。
...
### `composerData.latestConversationSummary`(modern,嵌套对象)
```json
{
  "summary": {
