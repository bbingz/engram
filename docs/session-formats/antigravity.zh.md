...
> - CLI brain:**168** 个会话目录,其中 **160** 个有 `transcript.jsonl`、**145** 个有 `transcript_full.jsonl`
> - 扫描的 CLI 记录总量:跨全部 160 个 transcript 共 **9 038** 行
> - 扫描的缓存消息行总量:**2 303**
...
**当前 Engram 状态（2026-07-02）：** 只读复核仍列出 61 个 raw IDE `.pb` 文件、
58 个 Engram cache JSONL 文件、160 个 canonical CLI `transcript.jsonl` 文件，以及
145 个被忽略的 `transcript_full.jsonl` 辅助文件。解析 58 个 cache 文件加 160 个
canonical CLI transcript 得到 218/218 个 session、8,237 条 streamed message、0 个
parser/stream mismatch。live DB 有 218 个 `antigravity` 行和 218 个
`file_index_state ok/v1` 行，当前 id 中 0 missing、0 DB-only。剩余 DB freshness debt
不变：130 个当前行相对当前 parser 有 stale 字段，主要是 128 个旧 cwd 快照，另有
3 个旧 broad-tool count 行和 1 个指向 `transcript_full.jsonl` 的 stale `source_locator`。
...
**最重要的血统事实:** 在已发布的 Swift 产品中,`AntigravityAdapter` 在 factory 入口
以 **`enableLiveSync: false`** 构造
(`SessionAdapterFactory.swift:33` 和 `:96`)。UI transcript 读取通过
`MessageParser.swift:113` 进入 `SessionAdapterFactory.defaultAdapters()`,不是单独的构造点。因此该产品
...
- ⚠ **在已发布产品中,sync 是关闭的**(在 factory 构造点
  `AntigravityAdapter(enableLiveSync: false)` — `SessionAdapterFactory.swift:33,96`;
  `MessageParser.swift:113` 使用 factory-backed registry)。`sync()` 提前返回(`:131`),因此 Engram 只读取
...
- 空/中止的会话会留下一个**没有** transcript 的 `brain/<uuid>/` 目录(168 个目录、160 个 transcript
  → 8 个目录没有)。Engram 跳过缺少 transcript 的目录。
- Engram **按需直接**读取 transcript;Branch B **没有** Engram 侧缓存。
- `transcript_full.jsonl`(145 个目录)是一个并行的超集 transcript(同 schema)。Engram 从不
  枚举它。
...
跨 9 038 条实时记录发现的全部 16 个 `type` 值,以及生产者(`source`)和适配器分配的角色:

| `type` | n (live, all transcripts) | Purpose | Adapter role | Note |
|---|---:|---|---|---|
| `PLANNER_RESPONSE` | 4 015 | 模型回合:文本和/或 `thinking` 和/或 `tool_calls` | **assistant** | 携带 `thinking`、`tool_calls` |
| `VIEW_FILE` | 1 747 | 文件读取工具**结果** | **tool** | 两个适配器唯一保留的实时工具结果类型 |
| `GREP_SEARCH` | 1 251 | grep 工具**结果** | DROPPED | 见 §7 |
| `EPHEMERAL_MESSAGE` | 384 | 瞬态 UI 文本 | DROPPED | |
| `SYSTEM_MESSAGE` | 362 | 系统/控制文本 | DROPPED | |
| `LIST_DIRECTORY` | 292 | 目录列表**结果** | DROPPED | |
| `GENERIC` | 212 | 杂项事件 | DROPPED | |
| `USER_INPUT` | 172 | 人类提示词 | **user** | |
| `RUN_COMMAND` | 152 | shell 命令**结果** | DROPPED | |
| `FIND` | 102 | 文件查找**结果** | DROPPED | |
| `SEARCH_WEB` | 91 | 网页搜索**结果** | DROPPED | |
| `CONVERSATION_HISTORY` | 84 | 历史边界标记 | DROPPED | **无 `content`** |
| `CODE_ACTION` | 82 | 代码编辑/补丁动作 | DROPPED | |
| `ERROR_MESSAGE` | 40 | 错误事件 | DROPPED | 额外 `error` 键;`content` 缺失(12)或为字符串(28),从不为 null |
| `INVOKE_SUBAGENT` | 27 | 子智能体派发标记 | DROPPED | 见 §10 |
| `CHECKPOINT` | 25 | 检查点标记 | DROPPED | |

> **适配器映射**(`cliMessage`,Swift `:345-368` / TS `:491-507`):`USER_INPUT`→user;
> `PLANNER_RESPONSE`→assistant(使用 `content`,回退到 `thinking`,附加 `toolCalls`);一个
> 固定的工具结果白名单 → tool。Swift 与 TS 现在使用相同白名单 — 见 §7。
...
| `source` | string enum | 生产者:`MODEL`(7 971)/ `SYSTEM`(895)/ `USER_EXPLICIT`(172) | **required** | `"MODEL"` |
| `status` | string enum | 生命周期:`DONE`(8 875)/ `RUNNING`(78)/ **`ERROR`(85)** | **required** | `"DONE"` |
...
`streamMessages`(`:376-404`)*能容忍* 可选的 `timestamp`,但写入器从不产生它 →
...
- **`PLANNER_RESPONSE`** → assistant 回合。`content` 要么是非空字符串,要么键**缺失**
  (在实时数据中从不为 JSON `null` 或 `""` — 3 324 条记录省略它,691 条有字符串)。适配器
  通过 `typeof obj.content === 'string' ? … : ''` 将缺失/非字符串的 `content` 归并为 `""`
  (`antigravity.ts:494`),然后回退到 `thinking`。可能携带 `tool_calls`。
...
- **`ERROR_MESSAGE`** → `error` 字符串;`content` 要么是非空字符串(28 条记录),要么键
  **缺失**(12 条记录)— 从不为 `null`。
...
被硬编码为 `0`(`AntigravityAdapter.swift:84`;TS `:332`)。

### Branch B — 调用和结果是 SEPARATE 记录,仅由顺序关联

- **调用** = `PLANNER_RESPONSE` 内的一个 `tool_calls[]` 条目。
  - 观察到的对象 key 集合**始终**为 `{args, name}`(3 998/3 998)。
...
### 工具结果映射:Swift↔TS 已对齐,但实时词汇表已超过 allowlist

Swift 适配器的工具结果白名单(`AntigravityAdapter.swift:362`)匹配:
`VIEW_FILE`、`TOOL_OUTPUT`、`COMMAND_OUTPUT`、`SHELL_OUTPUT`、`APPLY_PATCH`。

TS 参考现在使用相同白名单(`antigravity.ts:28-34`、`:506-507`)。其中**只有 `VIEW_FILE` 实际出现**在
160 个实时 transcript 中。`TOOL_OUTPUT` /
`COMMAND_OUTPUT` / `SHELL_OUTPUT` / `APPLY_PATCH` 匹配**零**条实时记录(死分支 — 一个较旧或
推测性的 schema 词汇表)。与此同时,真实的结果类型(`RUN_COMMAND`、`GREP_SEARCH`、
`LIST_DIRECTORY`、`FIND`、`SEARCH_WEB`、`CODE_ACTION` — 1 970 条记录)命中 Swift `default: return nil`
路径以及共享 TS allowlist miss 路径 → **被丢弃**。

**后果:** Swift 与 TS 现在对同一文件一致,但两者都只保留当前实时词汇表里的 `VIEW_FILE`
工具结果子集。对当前本机语料,160 个 CLI transcript 映射为 172 条 user、4 015 条 assistant、
1 747 条 tool(共 5 934 条);加上 2 303 条 Cascade 缓存消息行后,TS live smoke 在 218 个
Antigravity locator 上流式输出 **8 237** 条消息,stream/count mismatch 为 0。
...
- **Branch B:** 作为 `PLANNER_RESPONSE` 上的 `thinking` 字符串存储。适配器**仅在 `content` 缺失时
  ...
  (3 324 条记录),适配器将其归并为 `""` 然后替换为 `thinking`(`:354`,TS `:497`)。当 planner 步骤
...
- Branch B 发出子智能体记录:`INVOKE_SUBAGENT`(27),以及工具调用 `invoke_subagent`(30)、
  `define_subagent`(22)、`manage_subagents`(4)。这些标记 Antigravity 内部的子智能体派发。
...
- **Branch A:** meta 行中的 `summary` 字段(AI 生成,来自 gRPC;存在于 58 个文件中的 18 个)。
  ...
  (`:71,86`;TS `:334-336`)。
- **Branch B:** 无显式摘要字段。CLI 发出 `CONVERSATION_HISTORY` 边界标记(84)和
  `CHECKPOINT` 标记(25),它们 *形似* 压缩边界,但被适配器**丢弃**。
  会话 `summary` = 首条用户文本截断至 200 字符(`:324`;TS `:482`)。
...
| `~/.gemini/antigravity-cli/brain/<uuid>/transcript_full.jsonl` | 超集 transcript(同 schema),145 个目录 | **No**(从不枚举) |
...
|---|---|---|---|---|
| `id` | A | `:55,73` | `:258,324` | `meta.id`(UUID);为空则失败 |
| `id` | B | `:305` `cliSessionId` `:384-414` | `:469,528-544` | 从路径解析的 brain `<uuid>` 目录名;回退 = 文件名主干 |
| `source` | both | `:75`(`.antigravity`) | `:325`(`"antigravity"`) | 常量 |
| `startTime` | A | `:76` | `:326` | `meta.createdAt` |
| `startTime` | B | `:286-288,314` | `:460,474` | 首条记录的 `created_at` |
| `endTime` | A | `:77` | `:327` | `meta.updatedAt`,**仅当 ≠ createdAt**,否则 `nil` |
| `endTime` | B | `:289-291,315` | `:461,475` | 末条记录的 `created_at`,仅当 ≠ start |
| `cwd` | A | `:69,416-423,430-511` | `:303-306,549-611` | 存在则用 `meta.cwd`;否则从前 50 KB 中可信的本地绝对文件路径推断 |
| `cwd` | B | `:316,416-423,430-511` | `:462,540-611` | 始终从可信的本地绝对文件路径推断(transcript 中无 cwd) |
...
| `userMessageCount` | A | `:63,82` | `:291-293,330` | `role=="user"` 缓存行 |
| `userMessageCount` | B | `:293-295,321` | `:462-464,478` | `USER_INPUT` 计数 |
| `assistantMessageCount` | A | `:64,83` | `:294-295,331` | `role=="assistant"` 计数 |
| `assistantMessageCount` | B | `:296-297,322` | `:465,479` | `PLANNER_RESPONSE` 计数 |
| `toolMessageCount` | A | `:84`(`=0`) | `:332`(`=0`) | 始终 0 |
| `toolMessageCount` | B | `:298-299,323` | `:466,480` | 已映射的 allowlist 工具记录(实时只有 `VIEW_FILE` — §7) |
| `systemMessageCount` | both | `:85,323`(`=0`) | `:333,481`(`=0`) | 始终 0 |
| `summary` | A | `:71,86` | `:334-336` | `title` ?? `summary` ?? firstUserText,截断 200 |
| `summary` | B | `:324` | `:482` | firstUserText,截断 200 |
| `filePath` | both | `:87,326` | `:337,483` | `.jsonl` 定位路径 |
| `sizeBytes` | A | `:88,233-245` | `:267-275,338` | `meta.pbSizeBytes` 若 >0;否则 stat `.pb`;否则 stat 缓存文件 |
| `sizeBytes` | B | `:326` | `:447,484` | transcript 文件的 stat |
| `NormalizedMessage.usage` | both | `:360`(`nil`) | not set | 从不设置(§9) |
| `NormalizedToolCall` | B | `cliToolCalls` `:370-382` `{name, input=jsonString(args,500), output:nil}` | `:510-526` `{name, input=truncateJSON(args,500)}` | 仅调用;**无 output 关联** |
...
**发现与路由辅助:**
- `detect()`(`:36-40`;TS `:59-76`):若 `daemonDir`、`cacheDir`、`cliBrainDir` 任一存在则为 true。
...
- `isCLITranscript()`(`:262-270`;TS `:432-441`):路径位于 `brain/` 根之下,或匹配
...
2. **Swift↔TS Branch B 映射现在已对齐,但范围偏窄。** §7 — 两个适配器都只映射固定工具结果
   allowlist;在当前实时语料中,这意味着只有 `VIEW_FILE` 会作为 tool 消息保留。真实工具输出
   (`RUN_COMMAND`、`GREP_SEARCH`、`CODE_ACTION`、…)仍被两边丢弃。
3. **死的 allowlist case = 版本漂移。** `TOOL_OUTPUT` / `COMMAND_OUTPUT` / `SHELL_OUTPUT` /
   `APPLY_PATCH`(`:362`,TS `:30-33`)匹配**零**条实时记录。allowlist 仍反映较旧或推测性的
   记录类型词汇表。
4. **`cwd` 推断现在已对齐,但刻意保守。** Swift 与 TS 都使用通用的最频繁目录启发式
   (`inferCWDFromAbsolutePaths`,Swift `:430-511`;TS `:549-611`),不再使用旧的
   `/Users/<user>/-Code-/<project>` 硬编码模式。两个适配器都会拒绝 JSON 转义路径、
   类 markup 的 slash token、URL/localhost 片段、route/菜单式路径、`node_modules` 等依赖/缓存组件,
   以及没有文件式末尾组件的候选。这避免了 `/github.com/bbingz`、`/localhost:5173`、`/CI`、
   `/components/ui` 这类假 cwd;如果 transcript 只提到目录而没有可信本地文件路径,可能返回 `""`。
   2026-07-02 live smoke 确实返回了 3 个以 `/src/components` 结尾的合理本地源码目录;
   这些是有效的本地路径候选,不应与 route/菜单 false positive 混为一谈。
5. **实时 DB 仍有旧 cwd 快照。** 2026-07-02 live smoke 解析全部 218 个当前 Antigravity locator,
   stream/count mismatch 为 0,当前 id 中 0 missing、0 DB-only;但现有 DB 仍有
   128 条 Antigravity 行保存了旧的宽松启发式 cwd。需要用当前 worktree 重新索引才能刷新这些行。
6. **`createdAt === updatedAt` → 无 `endTime`。** 对短对话很常见;此时 Engram 只有一个
   开始时间(`:77`、TS `:327`)。
7. **`pbSizeBytes` 与缓存大小不匹配。** `sizeBytes` 报告的是 `.pb` 大小(KB → 数十 MB,一个
   实时文件 25.9 MB),而缓存 `.jsonl` 只有几 KB。基于大小的 UI/排序反映的是完整
   对话,而非存储的文本。≤ 200 字节的缓存文件被新鲜度闸门视为 "无内容"
   (`:188,220-223`;TS `:103,182`)。
8. **gRPC 列表只返回约 10 个最近的**(TS 注释 `:148,156`);`syncFromPbFiles` 回填其余 —
   但仅在 live-sync(非产品)路径中。
9. **`getTrajectoryMessages` 扁平化为仅 user/assistant 文本**(`CascadeClient.swift:55-90`):只有
   `USER_INPUT`/`PLANNER_RESPONSE`/`NOTIFY_USER` 步骤 → 消息;trajectory 中所有工具/文件步骤
   都在 gRPC 边界被丢弃。因此即使实时 sync 也会丢失 Branch A 工具调用。
10. **三级内容回退(Branch A sync):** trajectory 消息 → markdown(`## User`/`## Cascade`)
   → 对话摘要作为单条 assistant 消息(Swift `:153-161`;TS `:111-120`)。因此一个缓存文件
   可能只包含一条合成的 assistant 行。
11. **零 token/成本贡献**(§9)— Antigravity 会话在任何成本仪表板中显示为零成本。
12. **更丰富的每条消息 JSON 被忽略。** `.system_generated/messages/*.json`(基础键
...
13. **`status:"ERROR"` 存在**(85 条实时记录),超出 DONE/RUNNING — 适配器完全不在
...
- **死的 Swift 工具结果类型**(`TOOL_OUTPUT`/`COMMAND_OUTPUT`/`SHELL_OUTPUT`/`APPLY_PATCH`)— 是否
  ...
  存在过。)**
- **Antigravity Branch B 工具结果覆盖策略** — 两个适配器是否应该扩展到 `RUN_COMMAND`、
  `GREP_SEARCH` 等当前实时结果类型,还是继续丢弃这些嘈杂工具输出才是产品策略?当前两边已对齐,
  但产品策略仍是 Engram 内部设计问题,不可通过 web 验证。
