...
> - 参考/开发 RPC schema/client:`src/adapters/grpc/cascade-client.ts`;Swift 产品只读取既有缓存与
>   CLI transcript
>
...
|---|---|---|---|---|---|
| **A. Cascade IDE** | Antigravity IDE(基于 Codeium "Cascade" 引擎的 VS Code 分支) | 源 `~/.gemini/antigravity/conversations/<uuid>.pb`;Engram 缓存 `~/.engram/cache/antigravity/<uuid>.jsonl` | IDE 写入加密的 `.pb`;保留的参考/开发工具可通过 gRPC 与 IDE 通信写入 `.jsonl` 缓存 | 来自**缓存**的 meta 行 + `{role,content}` 消息行(从不读 `.pb`) |
...
**心智模型。** Branch A 是一个不透明加密 IDE 存储的 *Engram 自有文本投影*。保留的
参考/开发工具可通过 gRPC 拉取对话并写入有损的 `{role,content}` JSONL 缓存;Swift 产品
已不再包含该 live-sync 路径,也从不解码 `.pb`。Branch B 是一个 *原生丰富的智能体事件日志*
(步骤、工具、推理、时间戳),Engram 原样读取但随后会进行大量过滤。

**最重要的血统事实:** 已发布的 Swift 产品是 cache/transcript-only。Branch A 只暴露
早先由参考/开发工具写入的缓存 JSONL;Branch B 是实时读取的(它在磁盘上本就是 JSONL)。
在一台没有任何先前缓存生产运行的全新纯 Swift 机器上,Branch A 产出 **空**,只有 CLI brain
transcript 会出现。
...
### Branch A — IDE 缓存:衍生的、按对话整文件重写(Swift 产品无 live-sync)

- **唯一真相源** = 加密的 `<uuid>.pb`,由 IDE 拥有;它随用户聊天而增长/重写。
- **保留的参考/开发 sync**(`src/adapters/antigravity.ts`)可发现正在运行的 language server →
  `listConversations()` → 对每个对话**整文件重写**缓存 JSONL。这不是 append。
  - **新鲜度闸门:**当 `cache.mtime >= pb.mtime` **且** `cache.size > 200` 时跳过
    (size ≤ 200 = "仅 meta / 无内容")。
  - **`.pb` 扫描回填:**gRPC 列表只返回约 10 个最近的对话,因此参考/开发路径会扫描
    `conversations/*.pb` 找出 API 未返回的 id,并以文件 mtime 衍生的 `createdAt`/`updatedAt`
    和 `title:""` 写入缓存条目。
- **在已发布 Swift 产品中,live sync 不存在。** Engram 只读取**既有的**
  `~/.engram/cache/antigravity/*.jsonl`。上次参考/开发 sync 之后产生的新 IDE 对话**不会**
  被产品拾取。
- **发现(仅参考/开发 sync):** 先基于进程(`CascadeGrpcClient.fromProcess()`)— 找到
  `language_server` 进程、提取 `--csrf_token`、对 LISTEN 端口 `lsof`;回退到 `daemon/` 中
  带 `httpPort`+`csrfToken` 的 `<name>.json`。这里不存在 daemon `.json`,证实了基于进程的
  发现是实时机制。
...
**已验证:** 全部 2 303 条实时消息行**恰好**为 `{content, role}`。当前 Swift 产品读取这些
缓存记录,但不再写 Branch-A 缓存文件。Swift `normalizedMessages` 与 TS `streamMessages`
能容忍可选的 `timestamp`,所以遗留/参考缓存可能带有它;本次采样的 live cache 集合没有
Branch-A 每消息时间戳。
...
Swift 产品从不解码它们;对遗留缓存记录,它可能读取**字节大小**(→ `pbSizeBytes` fallback),
而保留的参考/开发 sync 路径会通过 gRPC 与正在运行的 language server 通信。嵌入在
`src/adapters/grpc/cascade-client.ts` 的参考 schema **极简** — 它只声明:
...
> `pbSizeBytes` 归因于 `cascade.proto`。**proto 不包含这些**。它们是保留的 TS 参考
> client 中的客户端实现细节,后者通过 HTTP/JSON 发起 `GetCascadeTrajectory`(未在
> 精简的 proto 中声明)并映射 JSON 响应:
> - `getTrajectoryMessages`读取 `trajectory.steps[]`,通过
...
**发现与路由辅助:**
- `detect()`:当前 Swift 产品在 legacy cache dir 或 CLI brain dir 存在时返回 true。
- `listSessionLocators()`:返回缓存 `.jsonl` 定位符(`CascadeCacheSupport.jsonlLocators`)与
  CLI transcript 定位符(`cliTranscriptLocators`)的排序并集,没有 live sync 步骤。
...
- **相同的上游 gRPC 服务** `exa.language_server_pb.LanguageServerService` 仍记录在保留的
  参考/开发工具中,相同的 RPC `GetAllCascadeTrajectories` / `ConvertTrajectoryToMarkdown`(+
  client 的 `GetCascadeTrajectory`)。
- **完全相同的缓存 JSONL 格式**(meta 行 + `{role,content}` 行),由共享的 Swift
  `CascadeCacheSupport` 缓存 helper 读取。
...
- **差异:** Antigravity 位于 `~/.gemini/antigravity/`,Windsurf 位于 `~/.codeium/windsurf/`。
  Antigravity cache metadata 带 `pbSizeBytes`;Windsurf cache 不带。两者在 Swift 产品中均为
  cache-only。缓存格式修复必须在两个 Swift parser 间镜像;参考/开发 RPC 修复保留在 TS 工具中。
...
1. **产品 = 仅缓存(Branch A)/ 实时(Branch B)。** Branch A 只暴露
...
7. **gRPC 列表只返回约 10 个最近的**(TS 注释 `:140,148`);`syncFromPbFiles` 回填其余 —
   但仅在参考/开发(非产品)路径中。
8. **`getTrajectoryMessages` 扁平化为仅 user/assistant 文本**(保留的 TS 参考 client):只有
...
9. **三级内容回退(Branch A sync):** trajectory 消息 → markdown(`## User`/`## Cascade`)
   → 对话摘要作为单条 assistant 消息(TS `:103-112`)。因此一个缓存文件可能只包含一条合成的
   assistant 行。
...
- **Fixture `timestamp`** 在缓存消息行上(`conv-001.jsonl`)— 没有任何实时缓存文件有它,且
  当前 Swift 产品不再写 Branch-A 缓存文件;该 fixture 是过期/愿景式、参考工具输出,
  还是反映了某种较旧的缓存格式,未经确认。**(Engram-internal design — not web-verifiable:
  这是 Engram 自有缓存文件,而非 Antigravity 拥有的源文件,因此没有官方 Antigravity 来源适用。)**
