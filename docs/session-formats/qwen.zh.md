...
1. **LIVE 磁盘存储** — 本机的 `~/.qwen/`(已确认存在)。在 **42 个项目目录** 下、
   `~/.qwen/projects/<encodedCwd>/chats/` 中共有 **744 个 `.jsonl` 会话转录文件**,
   外加 **16 个 `~/.qwen/tmp/<64-hex>/` 目录**,其中 **15 个含有一个 `logs.json`**
   (遥测数据,非转录)、**1 个不含任何文件**。**没有 `~/.qwen/projects.json`**
   (已确认缺失 —— 这是与 Gemini 的一个关键分歧)。LIVE `~/.qwen` 根目录还带有适配器忽略的
   **以 session 为键的非转录产物**:`debug/<sessionId>.txt`(**750 个** LIVE 的
   逐 session INFO/DEBUG 日志,文件名主干 == sessionId,与转录 1:1 对应)和
   `todos/<sessionId>.json`(`{sessionId, todos:[{content,id,status}]}`),以及全局的
   `memories/MEMORY.md`、`skills/<name>/` 和 `settings.json.orig`(见
   [§14](#14-auxiliary-files-present-live-not-consumed))。*(标注:此前一份研究笔记声称
   有 44 个项目目录 / 18 个 tmp 目录 / 3 个无 logs.json;本次 LIVE 存储分别显示
   **42 / 16 / 1** —— 以真实数据为准。744 个转录的数字和缺失的 `projects.json`
   在两者中完全一致。)* 磁盘上的 CLI 版本跨度:`0.10.5` → `0.18.4`(采样到的众数簇为
   `0.14.5`/`0.15.x`)。在采样的内容丰富的会话中,记录类型普查为:
   `system/ui_telemetry`、`tool_result`、`assistant`、`user`、`system/attribution_snapshot`、
   `system/slash_command` —— 六种类型全部存在。见到的模型取值:`qwen3.5-plus`、
   `qwen3.6-plus`(用量账本里还有 `qwen3.7-plus`)。
...
3. **Engram 适配器(已编码的知识)** — Swift 产品解析器
   `macos/Shared/EngramCore/Adapters/Sources/QwenAdapter.swift`(242 行);TS 参考解析器
   `src/adapters/qwen.ts`(211 行)。共享 I/O 辅助 `JSONLAdapterSupport` 位于
...
对象中。适配器在结构上处理这些 LIVE 形态(它按 `type` 过滤、只提取 `text` parts,并挖掘遥测),
所以大部分 LIVE 的丰富性是 **被解析但被丢弃** 而非被错误解析。见
...
(取自第一条符合条件的记录),**仅** 从 `user` + `assistant` 记录的 `message.parts[].text`
扁平化会话文本(Swift 中用 `\n\n` 连接,TS 中用 `\n` 连接 —— 一处分隔符漂移),统计 user 与 assistant
数量,把系统注入式 `user` 记录(文本以 `You are Qwen Code` 开头或含 `<INSTRUCTIONS>`)重新归类进
`systemMessageCount`,并从 `usageMetadata` 推导 token 用量,带一个 `system/ui_telemetry api_response`
兜底(**仅 Swift**)。它 **丢弃**:整个 `tool_result` 记录类型(`toolMessageCount` 硬编码为 `0`)、
除作为 token 旁路外的所有 `system` 行、`parts[].thought` 标志(文本会漏入)、
...
下有 744 个 `.jsonl`,42 个项目目录)。在每个项目目录内,转录位于 `chats/` 下
...
**但仅用于 token 遥测**(Swift `telemetryUsage` :174-196;TS 根本不检视它们)。因此 `toolMessageCount`
...
> 且不具备成为 summary 的资格)。—— `isSystemInjection` Swift:222-226 / TS:157-163。
...
`extractContent` 读取 `message.parts[]`,保留每个元素的非空 `.text`,然后连接 —— **`\n\n`**(Swift:240)/
**`\n`**(TS:203)。关于推理泄漏的注意事项见 §8。
...
| `message.parts` | array | 混合:推理(`thought:true`)、最终文本、工具调用(`functionCall`)(§6.5) | 必需 | ✅(text + thought-text 连接) |
| `usageMetadata` | object | 逐回合 token 用量(§9) | 可选 | ✅(Swift) |
...
| 块形态 | 在哪里 | 含义 | 是否消费? |
|---|---|---|---|
| `{ "text": "<string>" }` | user, assistant | 纯文本回合 / 答案 | ✅(连接;`\n\n` Swift,`\n` TS) |
| `{ "text": "<string>", "thought": true }` | assistant | 标记为 thought 的推理文本 | ✅ **文本被保留** —— Swift/TS 只检查是否有非空 `text` 键,所以推理文本被折入 assistant 内容(`thought:true` 标志被忽略)。 |
...
**抽取**(`extractContent` Swift:228-241 / TS:194-206):遍历 `message.parts`,保留每个元素的非空 `.text`,连接。
没有 `text` 键的块(`functionCall`/`functionResponse`)不贡献任何内容。

> **覆盖标记。** 由于唯一的过滤条件是「有非空 `text`」,一个 assistant `thought` part 的推理文本
> **会漏入归一化后的内容** —— Qwen **不** 像 Gemini 适配器那样剥离推理(后者丢弃其独立的 `thoughts[]` 数组)。
> 反过来,一个 `parts` *只有* `functionCall` 的 assistant 回合会被扁平化为 **空内容**(仍计为一个 assistant 回合
> —— 按 `type` 计数,而非按内容)。
...
**Engram 保留推理文本** —— `extractContent` 连接每个有非空 `.text` 的 part,且 **不** 检查 `thought`
(Swift:228-241,TS:194-206),因此一条 assistant 消息存储的 `content` = **推理文本与最终答案拼接而成**。

> **血统对照 / 行为不对称。** Gemini 把推理存于独立的顶层 `thoughts[]` 数组,Engram **丢弃** 它;
> Qwen 把推理内联进 `parts`,Engram **保留** 它。所以 Engram 的 Qwen assistant 内容是「思考 + 答案」,
> 而非仅答案 —— 这是一个纯粹由格式差异造成的真实行为差异(请向维护者标注:有意为之还是 bug? —— 见 §15 开放问题)。
...
Qwen 在 **两处** 暴露逐回合用量 —— Swift 适配器两者都读,优先用 `usageMetadata`。(**TS 适配器两者都不读**
—— 它丢弃所有 token 用量。)
...
### 9.2 兜底 —— `system/ui_telemetry` `api_response` 行(仅 Swift)

一行 `type:"system"`、`subtype:"ui_telemetry"`、其 `systemPayload.uiEvent["event.name"] == "qwen-code.api_response"`
的记录,**仅当其后的 assistant 记录缺少 `usageMetadata` 时**,才被挖掘 token(`telemetryUsage` Swift:174-196)。
...
**机制(Swift `messages(from:)` :140-153):** 遥测行出现在它们所描述的 assistant 记录 **之前**。解析器缓冲
`pendingTelemetryUsage`;一行遥测设置它并把自身丢弃(返回 `nil` → 不是消息);下一条 assistant 消息取
`metadataUsage ?? telemetryUsage`(Swift:170)并清空缓冲(Swift:148-150)。
...
> **差异标记。**
> 1. **TS 丢弃所有 token 用量** —— `qwen.ts` 没有 `usageMetadata`/遥测处理。Swift 是唯一产出 Qwen 成本/用量的路径。(与 Gemini 相同的 Swift-vs-TS 分裂。)
> 2. **parity 固件掩盖了它:** 陈旧固件没有 `usageMetadata`/遥测,所以 `usageTotals` 全零,从不演练抽取。真实 session 确实会填充用量。
...
| **逐消息** `content` | `extractContent(message.parts[].text)` 连接 | `:167,228-241` | `:150,194-206` | thought 文本漏入;`functionCall`/`functionResponse` 丢弃;分隔符 `\n\n`(Swift)vs `\n`(TS) |
| **逐消息** `timestamp` | `timestamp` | `:168` | `:151` | |
| **逐消息** `usage` | assistant `usageMetadata` ?? pending `ui_telemetry` | `:164,170,174-213` | **无** | **仅 Swift** |
| **逐消息** `toolCalls` | `nil` | `:169` | (无) | 丢弃 |

**Engram 不消费的内容:** 整个 `tool_result` 记录类型;除 `ui_telemetry/api_response` token 挖掘(Swift)外的
所有 `system` 行;`parts[].functionCall`/`functionResponse`(但 `thought` 文本漏入);`message.role`;
...
`memory/`;`tmp/*/logs.json`;`usage/*` 账本;以及(TS 路径)**所有** token 用量。磁盘上没有可消费的
...
| 推理 | 独立 `thoughts[]` 数组(文本被 Engram **丢弃**) | **带内 `{text,thought:true}` part(文本被 Engram 保留)** | ✗ 行为 |
...
1. **固件是陈旧的 `v0.10.5` schema(HIGH)。** `tests/fixtures/qwen/*` 与 parity 输入使用扁平 3 行 `user`/`assistant` 形态,带 `message.parts[].text`、顶层 `model`,**没有** 遥测/`tool_result`/`system`/`usageMetadata`/`thought` parts。LIVE `v0.14.5+` 要丰富得多。固件只验证了快乐文本路径,**不测试** token 抽取、遥测兜底、`tool_result` 跳过、注入过滤或 thought 泄漏。`schema_drift.jsonl` 只证明对未知键的容忍。
...
5. **推理文本漏入 assistant 内容(与 Gemini 相反)。** `extractContent` 保留任何带 `text` 键的 part,包括 `{text,thought:true}` 推理 —— Qwen 不像 Gemini 那样剥离推理。Engram 的 Qwen assistant 内容 = 推理 + 答案拼接。
...
8. **Token 仅在 Swift 产品路径。** TS 参考适配器对 Qwen 报告零用量;parity 固件(全零 `usageTotals`)掩盖了这一点。
...
16. **内容连接分隔符漂移。** Swift 用 `\n\n` 连接 parts,TS 用 `\n` —— 多 part 消息主体在两个解析器间以不同间距渲染。
...
- **行为选择(Engram-internal design —— 不可由 web 验证):** 是否剥离推理(`thought:true`)文本,以及是否从 `inputTokens` 减去缓存(vs Gemini)。约束该决定的格式事实(已确认):(a) 推理是 assistant 记录上带 `thought:true` 的带内 part(没有独立的 `thoughts[]` 数组),所以该标志是剥离它的唯一可用信号;(b) `promptTokenCount` 与 `cachedContentTokenCount` 是独立字段,所以如需可做缓存扣减。 — [`chatRecordingService.ts`](https://raw.githubusercontent.com/QwenLM/qwen-code/main/packages/core/src/services/chatRecordingService.ts)
