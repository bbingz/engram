...
| Basis | Detail |
|---|---|
| **Live store**（布局/生命周期的主要依据） | `~/Library/Application Support/Code/User/workspaceStorage/` ——**19 个 workspace 目录**（机器状态，非关键），其中**4 个**包含 `chatSessions/` 文件夹，共有**5 个 `*.jsonl` 聊天会话文件**。**全部 5 个 live 会话都是空桩**（`requests: []`、`isEmpty: true`）。其中一个文件 (`cea0313a…`) 有**2 行**（一条 `kind:0` 快照 + 一条 `kind:1` 补丁）；其余 4 个为单行。每个 workspace 下均有 `state.vscdb`（SQLite）。 |
...
| **Adapters**（已编码的知识） | Swift 产品解析器 `macos/Shared/EngramCore/Adapters/Sources/VsCodeAdapter.swift`（234 行）；TS 参考实现 `src/adapters/vscode.ts`（290 行）。行为上完全一致。测试 `tests/adapters/vscode.test.ts`。 |
...
text, first markdownContent block}` 对。**没有 model、没有 tokens、没有 tool 调用、
...
（[§15](#15-谱系陷阱版本漂移与边界情况)）。Insiders **在本机上物理存在**
（`~/Library/Application Support/Code - Insiders/User/workspaceStorage`
→ 6 个 workspace 目录，今日有 0 个聊天 `*.jsonl`），所以这个覆盖缺口
是真实但潜伏的，并非假设。
...
`.jsonl` 扩展名是一种**误称**：第 0 行就是整个会话对象。
两个适配器都**只读取第 0 行**（Swift `readSession` `:138-150`；TS
`readFirstLine`/`readSession` `:220-275`）。它是单文件单对象加一条
可选的补丁尾，**不是**流式 JSONL。
...
- **每个会话一个文件，就地重写（在概念层面**不是**追加式）。** 第 0 行是 `v` 的完整快照；VS Code 在保存时重新序列化它。
  第 1..N 行是它追加、稍后再折叠回快照的增量 `kind:1` 补丁。TS 注释指出文件"可达数 MB"（`:224-225,:261`），
  这正是两个适配器都只流式解析第 0 行的原因。
- **文件创建：** 当某个 workspace 中打开聊天面板时，会出现一个新的
  `chatSessions/<uuid>.jsonl`（加上配对的
  `chatEditingSessions/<uuid>/`）。空会话（`requests: []`、`isEmpty: true`）即使没有
  轮次也会持续存在——**全部 5 个 live 会话都处于此状态**。
...
3. **解析**（`parseSessionInfo`）：只读取第 0 行，要求 `kind === 0`，取出 `v`；
   若 `v.requests` 为空或 `v.creationDate` 缺失则拒绝（Swift `:40-44`；TS `:80`）。
4. **cwd 解析：** 向上爬两级目录（`<uuid>.jsonl` → `chatSessions/` →
   `<id>/`），读取 `workspace.json`，解码 `folder`/`configuration` URI。
...
`.jsonl` 文件是快照 + 变更日志。**Engram 只读取第 0 行**，所以
下面的补丁行对 Engram 而言仅供参考，但它们是真实 schema 的一部分。

| Line `kind` | Record type | Top-level fields | Meaning | Engram use |
|---|---|---|---|---|
| `0` | **Snapshot** | `kind:0`, `v:{…}`（完整会话对象——见 [§5](#5-共享信封--元数据字段)/[§6](#6-消息与内容-schema)） | 初始的完整会话序列化 | **Parsed**（整个 payload） |
| `1` | **Set/patch** | `kind:1`, `k:[…keypath…]`, `v:<value>` | 在 JSON keypath 数组 `k` 处设置值（例如替换 `inputState`） | **Ignored** |

> **相对于 DIM 报告的更正：** `k` 是一个由路径段组成的 **JSON 数组**
> （`["inputState"]`），已在 live 中验证——不是字符串。确切的补丁语义
> （merge 还是 replace、嵌套索引段）未经验证，因为两个适配器
> 都完全忽略这些行。

适配器对后续行垃圾内容的容忍度有明确测试：
`tests/adapters/vscode.test.ts:70-104` 写入
`<line0>\nNOT VALID JSON ON LINE 1\n{"kind":99}\n` 并断言只有第 0 行被
解析。Swift `readSession` 除非 `objects.first["kind"] == 0` 否则返回 `nil`
（`:144-149`）；TS 除非 `parsed.kind === 0 && parsed.v` 否则拒绝（`:228-229`）。
...
> 它**不**读取 `initialLocation`、`inputState`、`mode`、
> `responderUsername`、`requesterUsername`、`hasPendingEdits`、`pendingRequests`、
> `customTitle` 或 `isImported`。Model 和 token/usage 在会话级别**完全缺失**。
...
|---|---|---|---|
| `workspace.json` | JSON | Workspace 身份 → `cwd`（Engram 读取的唯一附属文件） | **READ** |
...
**`workspace.json` cwd 解析**（Swift `readWorkspaceCwd` `:152-201`；TS `:125-165`）：
...
| `cwd` | `workspace.json` `folder`/`configuration` → 解码的 `file://` | `VsCodeAdapter.swift:53,152-201` | `vscode.ts:93,125-165` | 若缺失/remote/格式错误/legacy-`workspace` 则为 `""` |
...
| `sizeBytes` | 磁盘上完整文件大小 | `VsCodeAdapter.swift:73` | `vscode.ts:78,111` | 整个 `.jsonl`（含 `kind:1` 补丁行），不只是第 0 行 payload |
...
**发现 / 读取内部：** 检测 `VsCodeAdapter.swift:18-20` / `vscode.ts:51-58`；
枚举 `:22-36` / `:60-74`；读取第 0 行 `readSession` `:138-150` / `readSession`+`readFirstLine` `:220-234,260-275`；
...
|---|---|---|---|---|
| **VS Code (Copilot/Gemini chat)** | `…/Code/User/workspaceStorage` | 每会话 `chatSessions/*.jsonl`（`kind:0`+`kind:1`） | `.jsonl` 第 0 行 | **baseline** |
| **VS Code Insiders** | `…/Code - Insiders/User/workspaceStorage` | 相同 `.jsonl` 格式 | 未覆盖——只扫描稳定版 `Code` 路径 | 格式相同，**未覆盖** ——**本机存在**（6 个 workspace 目录，今日 0 个聊天会话）；已确认默认适配器根目录会跳过它 |
...
1. **多行追加日志，单行读取。** Live `.jsonl` 文件为 1..N 行
   （`kind:0` 快照 + `kind:1` 补丁）。两个适配器都**只读取第 0 行**。
   风险：仅作为后续 `kind:1` 增量持久化的轮次会不可见。已 live
   验证：`cea0313a…jsonl` 有 2 行。
...
11. **`sizeBytes` 高估 payload。** 报告的大小是整个 `.jsonl`
    （含所有 `kind:1` 补丁行），而只有第 0 行内容被解析。
