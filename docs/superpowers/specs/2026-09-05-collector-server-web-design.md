...
- **Status**: Accepted for staged implementation. Draft PR #446 head `09de6304` has successful Tests, CodeQL and dependency review. The next local POSIX/T1 work-lease/Web-auth foundation candidate has independent slice and integration-seam gates passing, plus all six central targets: Core 1,596 and Service 875 (one existing skip each), App 1,175, MCP 270, Collector 108, Remote 292, zero failures. Final ten-script rerun is 205 passed/two existing conditional skips; typecheck, five invariant gates and staged project drift pass. The candidate is ready for authorized normal commit/push; new-head CI remains pending. N1 persisted root enrollment, T2 atomic parsed commit and A4 actual HTTP wiring are donor-only RED drafts, excluded from this candidate. The corrected GET contract is not yet real HTTP/browser acceptance. Runtime wiring, FSEvents/upload queues, parsed/read/FTS consumers, UI and W6 remain incomplete; W7 is separately authorized.
