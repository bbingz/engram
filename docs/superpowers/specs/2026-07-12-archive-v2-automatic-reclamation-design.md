...
6. There is no newer capture for the locator and no active capture, replication,
   recovery, transcript read, or reclamation operation for it.
...
```text
eligible -> source_quarantined -> source_deleted -> local_content_evicted
```
...
Local object residency must be explicit. Extend the local object catalog model
with a local-residency state or equivalent durable marker so integrity checks
can distinguish deliberate eviction from unexpected loss. Do not infer intent
solely from a missing CAS file.

The coordinator runs at most 10 source intents and evicts at most 256 MiB of CAS
content per cycle. These are fixed implementation limits in v1, not user-facing
settings.
...
3. Atomically rename the source to a unique hidden quarantine path in the same
   directory and record that path on the intent.
4. Open the quarantined file without following symlinks and verify its device,
   inode, size, modification time, regular-file mode, and whole-source SHA-256.
5. If every value matches the accepted capture, remove the quarantined file and
   commit `source_deleted` with released bytes.
6. If validation fails, restore the original name only when the destination is
...
Startup recovery inspects every `source_quarantined` intent. It validates and
finishes deletion only under the same gate, otherwise restores or pauses it.
Quarantine files are never treated as generic temporary garbage.
...
2. Perform a bounded read-back of the manifest from HQ, falling back to M1, and
   validate the canonical manifest identity.
...
The transcript resolver treats intentionally evicted objects as a normal remote
fallback condition. It tries HQ and then M1 using the existing verified archive
path. Unexpectedly missing local objects remain corruption and must not be
silently reclassified as eviction.
...
- deterministically select one eligible archived session for that replica;
...
only when both receipts and both recovery-drill leases are currently valid. The
dialog shows those four checks without exposing receipt internals.
...
- executing one bounded recovery drill per requested replica.
...
quarantine collision, remote unavailable, remote mismatch, and local I/O
failure. Repeated failures use the existing bounded retry style and do not spin
continuously.
...
4. Enable reclamation in Settings after the four checks are green.
