...
```
src/
  adapters/    # SessionAdapter implementations (15 sources: codex, Codex, cursor, etc.)
...
- MCP server: `idleTimeoutMs: 0` — Codex manages lifecycle via stdin close. Do NOT re-enable idle timeout (causes premature disconnect).
...
Parent-child session linking: agent sessions (dispatched by Codex to Gemini/Codex) are grouped under their parent.
...
- Four detection layers:
  1. **Layer 1 (path)**: Codex subagents — parse parent ID from `/subagents/` file path. Deterministic.
  2. **Layer 1b (originator)**: Codex `session_meta.originator === "Codex"` → auto `agentRole: 'dispatched'`. Deterministic.
...
- Session sources: `~/.Codex/projects/`, `~/.codex/sessions/`, `~/.gemini/`, etc.
...
- Don't add removed external indexing backends — use local sqlite-vec + FTS5 instead
