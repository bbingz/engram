...
export class CodexUsageProbe implements UsageProbe {
  ...
  private async tmuxProbe(): Promise<UsageSnapshot[]> {
    ...
    try {
      ...
      try { execSync(`tmux kill-session -t ${sessionName}`, { stdio: 'pipe' }) } catch { /* ok */ }
  ...
  private parseOutput(output: string): UsageSnapshot[] {
    ...
    for (const line of lines) {
      ...
      const percentMatch = line.match(/(\w[\w\s]*?)\s*[:=]\s*(\d+(?:\.\d+)?)\s*%/)
      if (percentMatch) {
        const metric = percentMatch[1].trim().toLowerCase().replace(/\s+/g, '_')
      ...
      const fractionMatch = line.match(/(\w[\w\s]*?)\s*[:=]\s*(\d+)\s*\/\s*(\d+)/)
      if (fractionMatch) {
        const metric = fractionMatch[1].trim().toLowerCase().replace(/\s+/g, '_')
