...
function getPort(): number {
  try {
    const settingsPath = join(process.env.HOME || '', '.engram', 'settings.json')
...
async function main() {
  ...
  try {
    ...
    sessions = (json.sessions || [])
      ...
      .map((s: any) => ({
        id: s.id,
        source: s.source,
        displayTitle: s.custom_name || s.generated_title || s.summary || s.id.slice(-8),
  ...
  sessions.forEach((s, i) => {
    ...
    console.log(`  ${i + 1}. ${s.source} · ${s.displayTitle} · ${time} · ${s.messageCount} msgs`)
  })

  // Prompt user to select
  const rl = createInterface({ input: process.stdin, output: process.stdout })
  const answer = await new Promise<string>(resolve => {
    rl.question(`\nSelect session (1-${sessions.length}) or q to quit: `, resolve)
  })
  rl.close()
  ...
  // Get resume command
  try {
    const res = await fetch(`${baseUrl}/api/session/${selected.id}/resume`, { method: 'POST' })
