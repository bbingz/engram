...
export function queryHealth(db: BetterSqlite3.Database): HealthResult {
  ...
  const tableNames = ['logs', 'traces', 'metrics', 'metrics_hourly', 'sessions']
...
export function diagnose(db: BetterSqlite3.Database, opts: { since?: string }): DiagnoseResult {
  ...
  ).all(...sinceParams) as Array<{ name: string, duration_ms: number, start_ts: string }>
  ...
  ).all(...sinceParams) as Array<{ ts: string, module: string, message: string }>
...
export function main(subcommand: string, args: string[]): void {
  ...
  try {
    if (subcommand === 'diagnose') {
      ...
      console.log(opts.json ? JSON.stringify(result, null, 2) : formatDiagnose(result))
    } else {
      const result = queryHealth(db.raw)
      console.log(opts.json ? JSON.stringify(result, null, 2) : formatHealth(result))
    }
  } finally {
    db.close()
  }
