  ...
  record(entry: AiAuditRecord): number {
    ...
    try {
      ...
      if (this.config.logBodies) {
        ...
        requestBody =
          rawReq != null
            ? truncate(applyPatterns(rawReq), this.config.maxBodySize)
            : null;
        responseBody =
          rawRes != null
            ? truncate(applyPatterns(rawRes), this.config.maxBodySize)
            : null;
      }
...
export class AiAuditQuery {
  ...
  list(
    ...
    if (filters.caller) {
      conditions.push('caller = @caller');
      params.caller = filters.caller;
    }
    if (filters.model) {
  ...
  stats(timeRange?: { from?: string; to?: string }): AiAuditStats {
    const from =
      timeRange?.from || new Date(Date.now() - 86400000).toISOString();
