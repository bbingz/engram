...
export class Indexer {
  ...
  private writeExtractedData(
    ...
    fileCounts?: Map<string, { action: string; count: number }>,
  ): void {
  ...
  private static accumulateFromStream(
    msg: Message,
    acc: {
      inputTokens: number;
      outputTokens: number;
      ...
      for (const tc of msg.toolCalls) {
        ...
        if (action && tc.input) {
          try {
            ...
            /* intentional: malformed tool_call input JSON, skip */
          }
        }
      }
    }
  ...
  private buildLocalAuthoritativeSnapshot(
    ...
    const toolCount = messages.filter(
      (m) => m.role === 'tool' || (m as { toolName?: string }).toolName,
    ).length;
      ...
      for (const adapter of this.adapters) {
        ...
        for await (const filePath of adapter.listSessionFiles()) {
          await runWithContext(
            { requestId: randomUUID(), source: 'indexer' },
            async () => {
              try {
                ...
                if (
                  ...
                  info.sizeBytes > 0 &&
                  this.db.isIndexed(filePath, info.sizeBytes)
                )
                  return;
                ...
                const sessionSpan = this.tracer?.startSpan(
                  'indexer.indexSession',
                  'indexer',
                  {
        ...
        try {
          ...
          if (info) {
            this.db.upsertSession({
              ...info,
              project: session.project ?? info.project,
            });
            count++;
            break;
          }
        } catch {
          /* intentional: try next adapter as fallback */
        }
      }
    }
    return count;
  ...
  async indexFile(
    ...
    try {
      ...
      const snapshot = this.buildLocalAuthoritativeSnapshot(
        current,
        { ...info, sizeBytes: info.sizeBytes || fileSize },
        filePath,
        messages,
      );
      this.writer.writeAuthoritativeSnapshot(snapshot);
