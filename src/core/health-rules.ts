...
export async function checkStaleBranches(cwd: string, shell: ShellExecutor): Promise<HealthIssue[]> {
  try {
    let mainBranch = 'main'
    try {
      const r = await shell('git', ['symbolic-ref', 'refs/remotes/origin/HEAD', '--short'], { cwd, timeout: 5000 })
      mainBranch = r.stdout.trim().replace('origin/', '') || 'main'
    } catch { /* use default */ }

    const { stdout } = await shell('git', ['branch', '--merged', mainBranch], { cwd, timeout: 5000 })
...
export async function checkLargeUncommitted(cwd: string, shell: ShellExecutor): Promise<HealthIssue[]> {
  try {
    const { stdout } = await shell('git', ['status', '--porcelain'], { cwd, timeout: 5000 })
    ...
    if (files.length > 20) {
      return [{
        ...
        detail: files.slice(0, 5).map(f => f.slice(3)).join(', '),
        repo: cwd,
...
export async function checkZombieDaemon(shell: ShellExecutor): Promise<HealthIssue[]> {
  try {
    const { stdout } = await shell('pgrep', ['-lf', 'engram.*daemon|daemon.*engram'], { timeout: 3000 })
...
export async function checkEnvSecurity(cwd: string, shell: ShellExecutor): Promise<HealthIssue[]> {
  try {
    // List .env* files in repo
    const { stdout: lsOut } = await shell('git', ['ls-files', '--others', '--exclude-standard', '-z'], { cwd, timeout: 5000 })
    const untrackedFiles = lsOut.split('\0').filter(Boolean)

    // Also check tracked .env files
    const { stdout: trackedOut } = await shell('git', ['ls-files', '-z'], { cwd, timeout: 5000 })
    const trackedFiles = trackedOut.split('\0').filter(Boolean)

    const envFiles = [...untrackedFiles, ...trackedFiles].filter(f => /^\.env/.test(f.split('/').pop() ?? ''))

    const exposed: string[] = []
    for (const file of envFiles) {
      try {
        await shell('git', ['check-ignore', '-q', file], { cwd, timeout: 3000 })
...
export function parseEtime(etime: string): number {
  ...
  if (parts.length === 3) {
    ...
    if (hourPart.includes('-')) {
      ...
      return parseInt(dd, 10) * 86400 + parseInt(hh, 10) * 3600 + parseInt(min, 10) * 60 + parseInt(sec, 10)
    }
    return parseInt(hourPart, 10) * 3600 + parseInt(min, 10) * 60 + parseInt(sec, 10)
  }
  return 0
}
...
export async function checkZombieProcesses(shell: ShellExecutor): Promise<HealthIssue[]> {
  try {
    const { stdout } = await shell('ps', ['axo', 'pid,etime,command', '--no-headers'], { timeout: 5000 })
...
export async function checkWorktreeHealth(cwd: string, shell: ShellExecutor): Promise<HealthIssue[]> {
  try {
    const { stdout } = await shell('git', ['worktree', 'list', '--porcelain'], { cwd, timeout: 5000 })
  ...
  } catch { /* not a git repo or old git */ }
  return []
}

/** Run npm audit on repos with recent sessions. Uses a 30-min per-repo cache. */
export async function checkDependencySecurity(repoPaths: string[], shell: ShellExecutor): Promise<HealthIssue[]> {
  ...
  for (const repoPath of repoPaths) {
    ...
    try {
      ...
      await shell('test', ['-f', `${repoPath}/package.json`], { timeout: 1000 })
    } catch {
    ...
    try {
      ...
      const { stdout } = await shell('npm', ['audit', '--json'], { cwd: repoPath, timeout: 30_000 })
...
export async function checkGitStash(cwd: string, shell: ShellExecutor): Promise<HealthIssue[]> {
  try {
    const { stdout } = await shell('git', ['stash', 'list'], { cwd, timeout: 5000 })
...
export async function checkBranchDivergence(cwd: string, shell: ShellExecutor): Promise<HealthIssue[]> {
  try {
    const { stdout } = await shell('git', ['rev-list', '--left-right', '--count', 'HEAD...@{u}'], { cwd, timeout: 5000 })
  ...
  } catch { /* no upstream tracking or not a git repo */ }
  return []
}

// ── Main entry point ─────────────────────────────────────────────────────────

export async function runAllHealthChecks(db: Database, options?: HealthCheckOptions): Promise<HealthCheckResult> {
  const now = Date.now()
  ...
  if (scope === 'project' && options?.cwd) {
    ...
    const rows = raw.prepare('SELECT path FROM git_repos ORDER BY probed_at DESC').all() as { path: string }[]
  ...
  await Promise.all(repoPaths.map(async (repoPath) => {
    const [staleBranches, largeUncommitted, worktreeHealth, gitStash, branchDivergence, envSecurity] =
      await Promise.all([
    ...
    issues.push(...staleBranches, ...largeUncommitted, ...worktreeHealth, ...gitStash, ...branchDivergence, ...envSecurity)
  }))
