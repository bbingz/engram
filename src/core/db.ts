...
export class Database {
   ...
   */
  private searchSessionsLike(query: string, limit: number, filters?: SearchFilters): FtsSearchResult[] {
    const conditions: string[] = ['f.content LIKE @pattern', 's.hidden_at IS NULL']
    ...
    const rows = this.db.prepare(`
      ...
      ORDER BY s.start_time DESC
      LIMIT @limit
    `).all({ ...params, query }) as { sessionId: string; context: string; rowid: number }[]
    ...
    return rows.map((r, i) => {
      ...
      return { sessionId: r.sessionId, snippet: `…${snippet}…`, rank: -(rows.length - i) }
    })
  }
