...
export class Database {
  ...
  private migrate(): void {
    ...
    if (cols.length > 0) {
      ...
      if (!colNames.has('hidden_at'))
        this.db.exec('ALTER TABLE sessions ADD COLUMN hidden_at TEXT');
      if (!colNames.has('custom_name'))
        this.db.exec('ALTER TABLE sessions ADD COLUMN custom_name TEXT');
      ...
      if (!colNames.has('summary_message_count'))
        this.db.exec(
          'ALTER TABLE sessions ADD COLUMN summary_message_count INTEGER',
        );
      if (!colNames.has('authoritative_node'))
        this.db.exec('ALTER TABLE sessions ADD COLUMN authoritative_node TEXT');
      if (!colNames.has('source_locator'))
        this.db.exec('ALTER TABLE sessions ADD COLUMN source_locator TEXT');
      ...
      if (!colNames.has('snapshot_hash'))
        this.db.exec('ALTER TABLE sessions ADD COLUMN snapshot_hash TEXT');
    ...
    const syncCols = this.db.prepare('PRAGMA table_info(sync_state)').all() as {
      name: string;
    }[];
    const syncColNames = new Set(syncCols.map((c) => c.name));
    if (syncCols.length > 0 && !syncColNames.has('last_sync_session_id')) {
      this.db.exec(
        'ALTER TABLE sync_state ADD COLUMN last_sync_session_id TEXT',
      );
    }
    ...
    if (ftsVersion !== FTS_VERSION) {
      ...
      try {
        this.db.exec('DELETE FROM session_embeddings');
      } catch {
        /* table may not exist yet */
      }
      try {
        this.db.exec('DELETE FROM vec_sessions');
      } catch {
        /* table may not exist yet */
      }
    ...
    const sessionCols = this.db.pragma('table_info(sessions)') as {
      name: string;
    }[];
    ...
    try {
      const logIndexes = this.db.prepare('PRAGMA index_list(logs)').all() as {
        name: string;
      }[];
      const hasOldIndex = logIndexes.some((i) => i.name === 'idx_logs_level');
      const hasNewIndex = logIndexes.some(
        (i) => i.name === 'idx_logs_level_ts',
      );
  ...
  searchSessions(
    ...
    if (containsCJK(query)) {
      return this.searchSessionsLike(query, limit, filters);
    }
  ...
  upsertAuthoritativeSnapshot(snapshot: AuthoritativeSessionSnapshot): void {
    const localReadablePath =
      this.getLocalState(snapshot.id)?.localReadablePath ?? '';
    this.db
      .prepare(`
  ...
  resolveProjectAliases(projects: string[]): string[] {
    ...
    const placeholders = projects.map((_, _i) => `?`).join(',');
    const rows = this.db
      .prepare(`
  ...
  removeProjectAlias(alias: string, canonical: string): void {
    this.db
      .prepare('DELETE FROM project_aliases WHERE alias = ? AND canonical = ?')
      .run(alias, canonical);
  }
  ...
  private applyFilters(
    ...
    const effectiveProjects =
      rawProjects.length > 0
        ? this.resolveProjectAliases(rawProjects)
        : rawProjects;
  ...
  vacuumIfNeeded(thresholdPct: number): boolean {
    const pageCount =
      (this.db.pragma('page_count') as { page_count: number }[])[0]
        ?.page_count ?? 0;
    const freeCount =
  ...
  private rowToAuthoritativeSnapshot(
    ...
    return {
      ...
      assistantMessageCount:
        (row.assistant_message_count as number | null) ?? 0,
