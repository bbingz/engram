    ...
    return [
      {
        ...
        check(): AlertResult | null {
          ...
          if (total > 50) {
            return { severity: 'critical', message: `Error rate critical: ${total} errors in last 10min`, value: total, threshold: 50 }
          }
          if (total > 10) {
            return { severity: 'warning', message: `Error rate elevated: ${total} errors in last 10min`, value: total, threshold: 10 }
          }
          return null
        },
      },
      {
        ...
        check(): AlertResult | null {
          ...
          const total = row?.total ?? 0
          if (total > 20) {
            return { severity: 'warning', message: `HTTP error rate elevated: ${total} errors in last 10min`, value: total, threshold: 20 }
          }
          return null
        },
      },
      {
        ...
        check(): AlertResult | null {
          ...
          const p95 = row.p95
          if (p95 > 15000) {
            return { severity: 'critical', message: `Search p95 latency critical: ${p95}ms`, value: p95, threshold: 15000 }
          }
          if (p95 > 5000) {
            return { severity: 'warning', message: `Search p95 latency elevated: ${p95}ms`, value: p95, threshold: 5000 }
          }
          return null
        },
      },
      {
        ...
        check(): AlertResult | null {
          ...
          const p95 = row.p95
          if (p95 > 2000) {
            return { severity: 'critical', message: `DB query p95 latency critical: ${p95}ms`, value: p95, threshold: 2000 }
          }
          if (p95 > 500) {
            return { severity: 'warning', message: `DB query p95 latency elevated: ${p95}ms`, value: p95, threshold: 500 }
          }
          return null
        },
      },
      {
        ...
        check(): AlertResult | null {
          ...
          if (growth > 100) {
            return { severity: 'warning', message: `Heap growing: +${growth.toFixed(1)}MB over ~3h`, value: growth, threshold: 100 }
          }
          return null
        },
      },
      {
        ...
        check(): AlertResult | null {
          ...
          const rss = row.value
          if (rss > 1024) {
            return { severity: 'critical', message: `RSS memory critical: ${rss}MB`, value: rss, threshold: 1024 }
          }
          return null
        },
      },
