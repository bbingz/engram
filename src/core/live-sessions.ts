...
export class LiveSessionMonitor {
  ...
  private parseSessionFile(
    ...
    try {
      ...
      if (meta.cwd) {
        ...
        // Try to extract from claude-code path: ~/.claude/projects/-Users-example--Code--foo/SESSION.jsonl
  ...
  /** Extract project name from claude-code path like ~/.claude/projects/-Users-example--Code--foo/SESSION.jsonl */
  private projectFromPath(filePath: string): string | undefined {
    ...
    // Decode: -Users-example--Code--foo → /Users/example/-Code-/foo → last segment = "foo"
