...
export async function handleHandoff(
  ...
  if (format === 'markdown') {
    ...
    lines.push(
      `**Last active**: ${relativeTime} via ${mostRecent.source} (${mostRecent.model || 'unknown'})`,
    );
    ...
    for (let i = 0; i < sessions.length; i++) {
      ...
      lines.push(
        `${i + 1}. [${s.source}] ${s.summary || 'No summary'} — ${s.messageCount} msgs${durationStr}${cost}`,
      );
    }
    lines.push('');
    if (lastTask) {
