...
import { getCostSuggestions, type CostSuggestion } from '../core/cost-advisor.js'
...
export const getInsightsDefinition = {
  ...
  inputSchema: {
    ...
    properties: {
      since: {
        type: 'string',
        description: 'ISO timestamp for start of analysis window (default: 7 days ago)',
      },
    },
...
export async function handleGetInsights(
  ...
  lines.push('')
  lines.push(`**Period summary:** Spent ${fmt(summary.totalSpent)} · Projected monthly ${fmt(summary.projectedMonthly)}`)
  ...
  if (suggestions.length === 0) {
    lines.push('No cost optimization suggestions for this period. Spending looks healthy!')
  } else {
    lines.push(`Found **${suggestions.length}** suggestion${suggestions.length > 1 ? 's' : ''}:`)
    lines.push('')
