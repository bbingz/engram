...
export async function handleGetContext(
  ...
  deps.log?.info('get_context invoked', {
    cwd: params.cwd,
  ...
  if (params.task && deps.vectorStore && deps.embed) {
    try {
      ...
      /* vector search failed, fall through */
    }
  }
...
async function gatherEnvironmentData(
  ...
  try {
    if (deps.liveMonitor) {
      ...
      if (live.length > 0) {
        const lines = live.map(
          (s) => `  ${s.source} ${s.project ?? '?'} [${s.activityLevel}]`,
        );
    ...
    if (!isNoSuchTableError(err))
      console.error('[get_context] liveSessions error:', err);
  }
  ...
  try {
    ...
    if (!isNoSuchTableError(err))
      console.error('[get_context] costToday error:', err);
  }

  // Active alerts
  try {
    if (deps.backgroundMonitor) {
      const alerts = deps.backgroundMonitor
        .getAlerts()
    ...
    if (!isNoSuchTableError(err))
      console.error('[get_context] alerts error:', err);
  }
  ...
  if (detail !== 'abstract') {
    ...
    try {
      ...
      if (!isNoSuchTableError(err))
        console.error('[get_context] recentTools error:', err);
    }
    ...
    try {
      ...
      const healthResult = await runAllHealthChecks(db, {
        scope: 'project',
        cwd,
      });
      if (healthResult.issues.length > 0) {
        const lines = healthResult.issues.map(
          (h) => `  [${h.severity}] ${h.message}`,
        );
        sections.push(
          `Health (score ${healthResult.score}/100, ${healthResult.issues.length} issues):\n${lines.join('\n')}`,
        );
      }
    } catch {
    ...
    try {
      ...
      if (rows.length > 0) {
        const lines = rows.map(
          (r) =>
            `  ${r.name}${r.branch ? ` (${r.branch})` : ''}: ${r.dirty_count} dirty, ${r.unpushed_count} unpushed`,
        );
      ...
      if (!isNoSuchTableError(err))
        console.error('[get_context] gitRepos error:', err);
    }
    ...
    try {
      ...
      if (rows.length > 0) {
        const lines = rows.map(
          (r) => `  ${r.file_path} (${r.total} edits, ${r.sessions} sessions)`,
        );
      ...
      if (!isNoSuchTableError(err))
        console.error('[get_context] fileHotspots error:', err);
    }
    ...
    try {
      ...
      if (!isNoSuchTableError(err))
        console.error('[get_context] recentErrors error:', err);
    }
    ...
    try {
      ...
      if (lintResult.score < 100 || lintResult.issues.length > 0) {
        ...
        const warnings = lintResult.issues.filter(
    ...
    try {
      ...
      const costResult = getCostSuggestions(db, config, {
        since: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString(),
      });
      if (costResult.suggestions.length > 0) {
        const lines = costResult.suggestions.slice(0, 5).map((s) => {
          const savingsText = s.savings
            ? ` (save ~$${(s.savings.current - s.savings.projected).toFixed(2)}/${s.savings.period})`
            : '';
          return `  [${s.severity}] ${s.title}${savingsText}`;
        });
        sections.push(
          `Cost suggestions (${costResult.suggestions.length}):\n${lines.join('\n')}`,
        );
      }
    } catch {
      /* cost advisor is best-effort */
    }
