...
describe('migration_log', () => {
  ...
  describe('hasPendingMigrationFor', () => {
    it('false when no migration covers path', () => {
      expect(db.hasPendingMigrationFor('/Users/bing/foo.jsonl')).toBe(false);
    });

    it('true while state = fs_pending and path is under old_path', () => {
      db.startMigration(
        basicInput('m1', '/Users/bing/-Code-/old', '/Users/bing/-Code-/new'),
      );
      expect(
        db.hasPendingMigrationFor('/Users/bing/-Code-/old/some/file.jsonl'),
      ).toBe(true);
    });
    ...
    it('true while state = fs_pending and path is under new_path', () => {
      ...
      expect(
        db.hasPendingMigrationFor('/Users/bing/-Code-/new/some/file.jsonl'),
      ).toBe(true);
    });
    ...
    it('true for exact old_path match', () => {
      ...
      expect(db.hasPendingMigrationFor('/Users/bing/-Code-/old')).toBe(true);
    });
