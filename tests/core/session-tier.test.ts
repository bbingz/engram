...
import { computeTier, TierInput } from '../../src/core/session-tier.js'
...
describe('computeTier()', () => {
  describe('skip tier', () => {
    ...
    it('returns skip for any non-null agentRole', () => {
      expect(computeTier(makeInput({ agentRole: 'orchestrator' }))).toBe('skip')
    })

    it('returns skip for subagent file path', () => {
      expect(computeTier(makeInput({ filePath: '/home/user/.claude/projects/abc/subagents/xyz.jsonl' }))).toBe('skip')
    })
  ...
  describe('premium tier', () => {
    ...
    it('returns premium for messageCount 25 with no project', () => {
      expect(computeTier(makeInput({ messageCount: 25, project: null }))).toBe('premium')
    })

    it('returns premium for messageCount 10 with project set', () => {
      expect(computeTier(makeInput({ messageCount: 10, project: 'my-project' }))).toBe('premium')
    })

    it('returns premium for messageCount 15 with project set', () => {
      expect(computeTier(makeInput({ messageCount: 15, project: 'my-project' }))).toBe('premium')
    })
    ...
    it('returns premium for 40-minute session', () => {
      ...
      expect(computeTier(makeInput({ startTime: start, endTime: end }))).toBe('premium')
    })

    it('premium takes priority over lite (/usage summary with 25 messages)', () => {
      expect(computeTier(makeInput({ messageCount: 25, summary: 'Check /usage stats' }))).toBe('premium')
    })
  })
  ...
  describe('lite tier', () => {
    ...
    it('returns lite for auto-summary noise pattern', () => {
      expect(computeTier(makeInput({ summary: 'Generate a short, clear title for this session' }))).toBe('lite')
    })
  })
  ...
  describe('normal tier', () => {
    it('returns normal for exactly 30-minute session (not > 30)', () => {
      ...
      expect(computeTier(makeInput({ startTime: start, endTime: end }))).toBe('normal')
    })

    it('returns normal for messageCount 3, no project, clean summary', () => {
      expect(computeTier(makeInput({ messageCount: 3, project: null, summary: 'Refactored auth module' }))).toBe('normal')
    })

    it('returns normal for messageCount 8 with project (below premium threshold of 10)', () => {
      expect(computeTier(makeInput({ messageCount: 8, project: 'my-project' }))).toBe('normal')
    })

    it('returns normal for messageCount 2, no summary', () => {
      expect(computeTier(makeInput({ messageCount: 2, summary: null }))).toBe('normal')
    })
  })
  ...
  describe('duration edge cases', () => {
    it('treats missing startTime as duration 0 (no premium from duration)', () => {
      const end = new Date('2024-01-01T10:40:00Z').toISOString()
      expect(computeTier(makeInput({ startTime: null, endTime: end }))).toBe('normal')
    })

    it('treats missing endTime as duration 0 (no premium from duration)', () => {
      const start = new Date('2024-01-01T10:00:00Z').toISOString()
      expect(computeTier(makeInput({ startTime: start, endTime: null }))).toBe('normal')
    })

    it('treats both timestamps missing as duration 0', () => {
      expect(computeTier(makeInput({ startTime: null, endTime: null }))).toBe('normal')
    })
  })
})
