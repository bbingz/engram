...
import { buildTitlePrompt, parseTitleResponse, TitleGenerator } from '../../src/core/title-generator.js'
...
describe('TitleGenerator audit', () => {
  ...
  function makeAudit() {
    return { record: vi.fn().mockReturnValue(1) } as unknown as AiAuditWriter & { record: ReturnType<typeof vi.fn> }
  }
  ...
  it('records audit with Ollama token fields', async () => {
    ...
    const gen = new TitleGenerator({
      ...
      model: 'qwen2.5:3b', autoGenerate: true, audit,
    })
  ...
  it('records audit with OpenAI usage fields', async () => {
    ...
    const gen = new TitleGenerator({
      ...
      model: 'gpt-4o-mini', apiKey: 'sk-test', autoGenerate: true, audit,
    })
    const title = await gen.generate(messages)
  ...
  it('does not crash when no audit is provided', async () => {
    ...
    const gen = new TitleGenerator({
      provider: 'ollama', baseUrl: 'http://localhost:11434',
      model: 'qwen2.5:3b', autoGenerate: true,
      // no audit
    })
    const title = await gen.generate(messages)
  ...
  it('records undefined totalTokens when both counts are missing', async () => {
    ...
    const gen = new TitleGenerator({
      provider: 'openai', baseUrl: 'https://api.openai.com',
      model: 'gpt-4o-mini', autoGenerate: true, audit,
    })
