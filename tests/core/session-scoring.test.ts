...
import { computeQualityScore, type ScoringInput } from '../../src/core/session-scoring.js'
...
describe('computeQualityScore()', () => {
  describe('turn ratio (0-30)', () => {
    ...
    it('scores 0 turn ratio when assistantCount is 0', () => {
      ...
      const withAssistant = computeQualityScore(makeInput({ assistantCount: 5 }))
    ...
    it('rewards balanced user/assistant pairs', () => {
      ...
      const balanced = computeQualityScore(makeInput({ userCount: 5, assistantCount: 5 }))
      // 1 user + 9 assistant = 1 pair out of 14 total → (1/14)*30 ≈ 2.1
      const unbalanced = computeQualityScore(makeInput({ userCount: 1, assistantCount: 9 }))
  ...
  describe('tool engagement (0-25)', () => {
    ...
    it('caps tool score at 25', () => {
      ...
      const extreme = computeQualityScore(makeInput({ toolCount: 100, assistantCount: 5 }))
      const high = computeQualityScore(makeInput({ toolCount: 50, assistantCount: 5 }))
    ...
    it('scores 0 tool engagement when assistantCount is 0', () => {
      const score = computeQualityScore(makeInput({ assistantCount: 0, toolCount: 10 }))
  ...
  describe('session density (0-20)', () => {
    it('scores 0 for sessions under 1 minute', () => {
      ...
      const longer = computeQualityScore(makeInput({
        startTime: '2024-01-01T10:00:00Z',
    ...
    it('scores max density for 5-60 min sessions', () => {
      const fiveMin = computeQualityScore(makeInput({
        startTime: '2024-01-01T10:00:00Z',
        endTime: '2024-01-01T10:05:00Z',
      }))
      const thirtyMin = computeQualityScore(makeInput({
        startTime: '2024-01-01T10:00:00Z',
        endTime: '2024-01-01T10:30:00Z',
      }))
      const sixtyMin = computeQualityScore(makeInput({
        startTime: '2024-01-01T10:00:00Z',
    ...
    it('tapers for sessions over 60 min', () => {
      ...
      const twoHours = computeQualityScore(makeInput({
        startTime: '2024-01-01T10:00:00Z',
    ...
    it('scores 10 density for sessions over 3 hours', () => {
      const fourHours = computeQualityScore(makeInput({
        startTime: '2024-01-01T10:00:00Z',
        endTime: '2024-01-01T14:00:00Z',
      }))
      const fiveHours = computeQualityScore(makeInput({
        startTime: '2024-01-01T10:00:00Z',
    ...
    it('scores 0 density when timestamps missing', () => {
      const noTimes = computeQualityScore(makeInput({
        startTime: null,
        endTime: null,
      }))
      const withTimes = computeQualityScore(makeInput({
        startTime: '2024-01-01T10:00:00Z',
        endTime: '2024-01-01T10:30:00Z',
      }))
  ...
  describe('project association (0-15)', () => {
    it('scores 15 for sessions with a project', () => {
      const withProject = computeQualityScore(makeInput({ project: 'my-project' }))
  ...
  describe('message volume (0-10)', () => {
    it('rewards more messages up to cap', () => {
      const few = computeQualityScore(makeInput({ userCount: 1, assistantCount: 1, toolCount: 0 }))
      const many = computeQualityScore(makeInput({ userCount: 10, assistantCount: 10, toolCount: 10 }))
      expect(many).toBeGreaterThan(few)
    })

    it('caps volume at 10', () => {
      // 50 messages / 5 = 10 (cap)
      const fifty = computeQualityScore(makeInput({ userCount: 25, assistantCount: 25, toolCount: 0 }))
      const hundred = computeQualityScore(makeInput({ userCount: 50, assistantCount: 50, toolCount: 0 }))
