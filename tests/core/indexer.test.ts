...
describe('Indexer', () => {
      ...
      lines.push(
        JSON.stringify({
          ...
          payload: {
            type: 'message',
            role,
            content: [{ type: contentType, text: `msg ${i}` }],
          },
        }),
      );
    }
      ...
      const adapter: SessionAdapter = {
        ...
        async parseSessionInfo(filePath: string) {
          ...
          return makeBaseSessionInfo({
            id: entry.id,
            filePath,
            sizeBytes: 100 + paths.indexOf(entry),
          });
        },
        async *streamMessages(_filePath: string): AsyncGenerator<Message> {
          yield { role: 'user', content: 'hello' };
  ...
  it('computes tier and upgrades on re-index', async () => {
        ...
        lines.push(
          JSON.stringify({
            ...
            payload: {
              type: 'message',
              role,
              content: [{ type: contentType, text: `message ${i}` }],
            },
          }),
        );
      }
    ...
    const filePath = makeMultiMessageFile(
      sessionsDir,
      'tier-001',
      '/Users/test',
      1,
