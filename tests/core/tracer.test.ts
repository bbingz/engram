...
describe('Tracer', () => {
  ...
  it('supports nested spans with parent', () => {
    ...
    parent.end();
    const rows = db.raw
      .prepare('SELECT * FROM traces ORDER BY id')
      .all() as any[];
  ...
  it('sets attributes on span', () => {
    const span = tracer.startSpan('test.attrs', 'test', {
...
describe('withSpan', () => {
  ...
  it('records error span and re-throws', async () => {
    await expect(
      withSpan(tracer, 'test.fail', 'test', async () => {
...
describe('withSpanSync', () => {
  ...
  it('records error sync span and re-throws', () => {
    expect(() =>
      withSpanSync(tracer, 'sync.fail', 'test', () => {
