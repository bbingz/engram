...
describe('getCostSuggestions', () => {
  ...
  it('detects opus overuse when opus is >70% of total and has short sessions', () => {
    ...
    insertCost(db, 's1', 'claude-opus-4-6', 50000, 5000, 0, 0, 4.0);
    insertCost(db, 's2', 'claude-opus-4-6', 50000, 5000, 0, 0, 4.0);
    insertCost(db, 's3', 'claude-sonnet-4-6', 50000, 5000, 0, 0, 0.5);
  ...
  it('detects low cache rate for Anthropic models', () => {
    ...
    insertCost(db, 's1', 'claude-sonnet-4-6', 95000, 5000, 5000, 0, 0.5);

    const result = getCostSuggestions(db, emptyConfig);
    const cacheRule = result.suggestions.find(
      (s) => s.rule === 'low-cache-rate',
    );
  ...
  it('does not flag low cache rate for non-Anthropic models', () => {
    ...
    insertCost(db, 's1', 'gpt-4o', 95000, 5000, 0, 0, 1.0);
  ...
  it('detects over-budget when daily avg exceeds configured budget', () => {
    ...
    insertCost(db, 's1', 'claude-sonnet-4-6', 1000000, 100000, 0, 0, 100.0); // $100 over 7 days = $14.28/day
  ...
  it('skips over-budget rule when no budget configured', () => {
    ...
    insertCost(db, 's1', 'claude-sonnet-4-6', 1000000, 100000, 0, 0, 100.0);
  ...
  it('detects project hotspot when single project >50% of total', () => {
    ...
    insertCost(db, 's1', 'claude-sonnet-4-6', 500000, 50000, 0, 0, 8.0);
    insertCost(db, 's2', 'claude-sonnet-4-6', 50000, 5000, 0, 0, 1.0);

    const result = getCostSuggestions(db, emptyConfig);
    const hotspotRule = result.suggestions.find(
      (s) => s.rule === 'project-hotspot',
    );
  ...
  it('ranks models by cost-per-session efficiency', () => {
    ...
    insertCost(db, 's1', 'claude-opus-4-6', 100000, 10000, 0, 0, 10.0); // $10/session
    insertCost(db, 's2', 'claude-sonnet-4-6', 100000, 10000, 0, 0, 0.5); // $0.25/session
    insertCost(db, 's3', 'claude-sonnet-4-6', 100000, 10000, 0, 0, 0.5);

    const result = getCostSuggestions(db, emptyConfig);
    const effRule = result.suggestions.find(
      (s) => s.rule === 'model-efficiency',
    );
  ...
  it('computes summary with projected monthly', () => {
    ...
    insertCost(db, 's1', 'claude-sonnet-4-6', 100000, 10000, 0, 0, 7.0); // $7 over 7 days → $30/month
  ...
  it('detects expensive sessions (>$5 and >200K tokens)', () => {
    ...
    insertCost(db, 's1', 'claude-sonnet-4-6', 200000, 100000, 0, 0, 6.0);

    const result = getCostSuggestions(db, emptyConfig);
    const expRule = result.suggestions.find(
      (s) => s.rule === 'expensive-sessions',
    );
  ...
  it('detects week-over-week spike', () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      insertCost(db, id, 'claude-sonnet-4-6', 100000, 10000, 0, 0, 1.0); // $3 last week
    }
    ...
    for (let i = 0; i < 3; i++) {
      ...
      insertCost(db, id, 'claude-sonnet-4-6', 1000000, 100000, 0, 0, 10.0); // $30 this week
    }
  ...
  it('detects output imbalance excluding heavy edit sessions', () => {
    ...
    insertCost(db, 's1', 'claude-sonnet-4-6', 10000, 50000, 0, 0, 1.0); // ratio = 5x
    ...
    const result = getCostSuggestions(db, emptyConfig);
    const imbalanceRule = result.suggestions.find(
      (s) => s.rule === 'output-imbalance',
    );
    expect(imbalanceRule).toBeDefined();
    // s1 should be flagged, s2 excluded
    expect(
      imbalanceRule?.topItems?.find((i) => i.label === 'Sessions affected')
        ?.value,
    ).toBe(1);
  });
});
