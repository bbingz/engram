...
describe('filterForViking', () => {
    ...
    const msgs = [
      {
        role: 'user',
        content:
          '# AGENTS.md instructions for /Users/bing/-Code-/project\n\n<INSTRUCTIONS>\nAct like a senior engineer...',
      },
  ...
  it('strips <command-name> / <command-message> messages', () => {
    const msgs = [
      {
        role: 'user',
        content: 'Some text <command-name>commit</command-name> more text',
      },
    ];
    expect(filterForViking(msgs)).toHaveLength(0);
  });
  ...
  it('preserves PGPASSWORD (no redaction)', () => {
    const msgs = [
      {
        role: 'assistant',
        content: 'Running: PGPASSWORD=TPmCa4FjQhRG psql -h 10.10.0.12',
      },
    ];
    const result = filterForViking(msgs);
  ...
  it('preserves API keys (no redaction)', () => {
    const msgs = [
      {
        role: 'user',
        content: 'sk-henhtN3lOMGKYoTkDX2PDFY0irmW8Rha14xO3OmAIolGipzJ',
      },
    ];
    expect(filterForViking(msgs)[0].content).toContain(
      'sk-henhtN3lOMGKYoTkDX2PDFY0irmW8Rha14xO3OmAIolGipzJ',
    );
  });
    ...
    const msgs = [
      {
        role: 'assistant',
        content:
          'curl -H "Authorization: Bearer engram-viking-2026" http://...',
      },
    ];
    const result = filterForViking(msgs);
  ...
  it('keeps messages that mix tools with natural language', () => {
    const msgs = [
      {
        role: 'assistant',
        content: 'The issue is in the Bash command `ls`. Let me fix it.',
      },
      {
        role: 'assistant',
        content:
  ...
  it('handles mixed content — keeps valuable, strips noise', () => {
    const msgs = [
      ...
      { role: 'assistant', content: '`Read`: /src/login.ts' },
      {
        role: 'assistant',
        content:
          'The bug is on line 42. The token validation skips expired tokens.',
      },
      {
        role: 'user',
        content: '<system-reminder>Task tools available</system-reminder>',
      },
    ];
    const result = filterForViking(msgs);
    expect(result).toHaveLength(2);
