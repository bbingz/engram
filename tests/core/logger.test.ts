...
import {
  createLogger,
  LogWriter,
  type LogWriter as LogWriterType,
...
describe('observability schema', () => {
  ...
  it('enforces level check constraint on logs', () => {
    expect(() => {
      db.raw
        .prepare(
          "INSERT INTO logs (level, module, message, source) VALUES ('invalid', 'test', 'msg', 'daemon')",
        )
        .run();
    }).toThrow();
  });
...
describe('logger error count', () => {
  ...
  it('increments error.count on log.error()', () => {
    ...
    const rows = metricsDb.raw
      .prepare("SELECT * FROM metrics WHERE name = 'error.count'")
      .all() as any[];
    expect(rows).toHaveLength(1);
...
describe('logger performance', () => {
  it('sanitize + stderr write stays under a broad regression budget', () => {
    ...
    expect(avgUs).toBeLessThan(process.env.CI ? 2_500 : 2_000);
