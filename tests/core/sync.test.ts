...
describe('SyncEngine', () => {
  ...
  it('pulls new sessions from a peer', async () => {
    ...
    const result = await engine.pullFromPeer({
      name: 'mac-mini',
      url: 'http://192.0.2.10:3457',
    });
  ...
  it('does not skip sessions that share the page-boundary indexed_at timestamp', async () => {
    ...
    const mockFetch = vi
      .fn()
      .mockResolvedValueOnce(
        jsonOk({ sessionCount: 101, nodeName: 'peer-a', timestamp: sharedTs }),
      )
  ...
  it('pulls newer metadata even when messageCount is unchanged', async () => {
    db.upsertAuthoritativeSnapshot(
      validSnapshot('sess-1', '2026-03-18T12:00:00Z'),
    );
    ...
    const mockFetch = vi
      ...
      .mockResolvedValueOnce(
        jsonOk({
          sessions: [
            validSnapshot('sess-1', '2026-03-18T12:05:00Z', {
              syncVersion: 2,
    ...
    expect(db.getAuthoritativeSnapshot('sess-1')?.indexedAt).toBe(
  ...
  it('exports agentRole in sync session list', async () => {
    db.upsertAuthoritativeSnapshot(
      validSnapshot('local-agent-1', '2026-03-19T10:00:00Z', {
        source: 'claude-code',
        agentRole: 'refactor',
      } as Partial<AuthoritativeSessionSnapshot>),
    );
