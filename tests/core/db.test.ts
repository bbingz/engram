...
describe('Database', () => {
  ...
  it('searches Chinese with filters', () => {
    ...
    db.indexSessionContent('session-001', [
      { role: 'user', content: '修复数据库连接问题' },
    ]);
    db.indexSessionContent('session-002', [
      { role: 'user', content: '修复网络请求超时' },
    ]);
  ...
  it('dual-writes indexSessionContent while an FTS rebuild is pending', () => {
    ...
    expect(
      db.raw
        .prepare(
          'SELECT content FROM sessions_fts_rebuild WHERE session_id = ? ORDER BY rowid',
        )
        .pluck()
        .all('session-001'),
  ...
  it('checks if file is already indexed', () => {
    db.upsertSession(mockSession);
    expect(db.isIndexed(mockSession.filePath, mockSession.sizeBytes)).toBe(
      true,
    );
  ...
  it('backfills local_readable_path from legacy file_path without losing machine-local readability', () => {
    ...
    expect(db.getLocalState('legacy-1')?.localReadablePath).toBe(
      '/Users/test/.codex/sessions/legacy.jsonl',
    );
    expect(db.getAuthoritativeSnapshot('legacy-1')?.sourceLocator).toBe(
      '/Users/test/.codex/sessions/legacy.jsonl',
    );
  });
  ...
  describe('tier migration', () => {
    it('backfills tier column for existing sessions', () => {
      ...
      expect(
        raw.prepare('SELECT tier FROM sessions WHERE id = ?').get('skip-1'),
      ).toHaveProperty('tier', 'skip');
      expect(
        raw.prepare('SELECT tier FROM sessions WHERE id = ?').get('premium-1'),
      ).toHaveProperty('tier', 'premium');
      expect(
        raw.prepare('SELECT tier FROM sessions WHERE id = ?').get('lite-1'),
      ).toHaveProperty('tier', 'lite');
      expect(
        raw.prepare('SELECT tier FROM sessions WHERE id = ?').get('normal-1'),
      ).toHaveProperty('tier', 'normal');
    });
  });
