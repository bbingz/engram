...
const testCases: DriftTestCase[] = [
  { name: 'claude-code', adapter: new ClaudeCodeAdapter(), fixtureDir: 'claude-code', expectedMinMessages: 2 },
  { name: 'codex', adapter: new CodexAdapter(), fixtureDir: 'codex', expectedMinMessages: 2 },
  { name: 'iflow', adapter: new IflowAdapter(), fixtureDir: 'iflow', expectedMinMessages: 2 },
  { name: 'kimi', adapter: new KimiAdapter(), fixtureDir: 'kimi', expectedMinMessages: 2 },
  { name: 'qwen', adapter: new QwenAdapter(), fixtureDir: 'qwen', expectedMinMessages: 2 },
]
...
describe('schema drift fixture coverage', () => {
  it('all JSONL adapters have schema_drift fixtures', () => {
    for (const tc of testCases) {
      ...
      expect(existsSync(path), `Missing fixture: ${tc.name}/schema_drift.jsonl`).toBe(true)
    }
  })
})
