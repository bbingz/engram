...
describe('VsCodeAdapter', () => {
  ...
  it('streamMessages yields user and assistant alternating', async () => {
    ...
    expect(msgs[0]).toMatchObject({ role: 'user', content: 'How do I use async/await in TypeScript?' })
    expect(msgs[1].role).toBe('assistant')
    expect(msgs[2]).toMatchObject({ role: 'user', content: 'Can you show an example?' })
