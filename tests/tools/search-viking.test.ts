...
describe('handleSearch with Viking', () => {
  ...
  it('uses Viking find results when available', async () => {
    ...
    const vikingResults: VikingSearchResult[] = [
      {
        uri: 'viking://session/claude-code/engram/session-1',
        score: 0.95,
        snippet: 'SSL fix found',
      },
    ];
    const mockViking = {
  ...
  it('falls back to FTS when Viking is not provided', async () => {
    ...
    db.indexSessionContent('session-2', [
      { role: 'user', content: 'SSL certificate error' },
    ]);
  ...
  it('returns Viking memory results that cannot map to sessions', async () => {
    ...
    const vikingResults: VikingSearchResult[] = [
      {
        uri: 'viking://user/default/memories/pref-1',
        score: 0.95,
        snippet: 'User prefers TypeScript strict mode',
      },
      {
        uri: 'viking://session/claude-code/engram/session-1',
        score: 0.8,
        snippet: 'Fixed auth bug',
      },
    ];
    const mockViking = {
      checkAvailable: vi.fn().mockResolvedValue(true),
    ...
    expect(result.vikingMemories).toContain(
  ...
  it('falls back to FTS when Viking find() throws', async () => {
    ...
    db.indexSessionContent('session-3', [
      { role: 'user', content: 'SSL cert renewal' },
    ]);
    const mockViking = {
      checkAvailable: vi.fn().mockResolvedValue(true),
