...
describe('link_sessions', () => {
  ...
  beforeEach(() => {
    ...
    db.upsertSession({
      ...
      filePath: join(sourceDir, 'claude-code', 'session1.jsonl'),
      sizeBytes: 100,
    });
    db.upsertSession({
      id: 's2',
      ...
      toolMessageCount: 0,
      systemMessageCount: 0,
      filePath: join(sourceDir, 'claude-code', 'session2.jsonl'),
      sizeBytes: 80,
    });
    db.upsertSession({
      id: 's3',
      ...
      toolMessageCount: 0,
      systemMessageCount: 0,
      filePath: join(sourceDir, 'codex', 'session3.jsonl'),
      sizeBytes: 50,
    });
    db.upsertSession({
      id: 's4',
      ...
      toolMessageCount: 0,
      systemMessageCount: 0,
      filePath: join(sourceDir, 'claude-code', 'other.jsonl'),
      sizeBytes: 60,
    });
  });
  ...
  it('creates symlinks grouped by source', async () => {
    ...
    expect(readlinkSync(link1)).toBe(
      join(sourceDir, 'claude-code', 'session1.jsonl'),
    );
    expect(lstatSync(link2).isSymbolicLink()).toBe(true);
    expect(lstatSync(link3).isSymbolicLink()).toBe(true);
    expect(readlinkSync(link3)).toBe(
      join(sourceDir, 'codex', 'session3.jsonl'),
    );
  });
  ...
  it('resolves project aliases', async () => {
    ...
    db.upsertSession({
      ...
      toolMessageCount: 0,
      systemMessageCount: 0,
      filePath: join(sourceDir, 'codex', 'session5.jsonl'),
      sizeBytes: 30,
    });
  ...
  it('replaces symlink when same filename points to different target', async () => {
    ...
    db.upsertSession({
      ...
      toolMessageCount: 0,
      systemMessageCount: 0,
      filePath: join(altDir, 'session3.jsonl'),
      sizeBytes: 40,
    });
