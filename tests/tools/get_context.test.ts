...
describe('get_context', () => {
  ...
  it('returns only sessionIds, not full session objects', async () => {
    ...
    expect(
      result.sessionIds.every((id: string) => typeof id === 'string'),
    ).toBe(true);
  });
  ...
  it('uses vector search when deps provided and task is given', async () => {
    db.upsertSession({
      ...
      cwd: '/Users/test/myapp',
      project: 'myapp',
      summary: 'Configured OAuth auth flow',
      messageCount: 10,
      userMessageCount: 5,
      assistantMessageCount: 0,
      toolMessageCount: 0,
    ...
    const result = await handleGetContext(
      db,
      { cwd: '/Users/test/myapp', task: 'Fix auth issue' },
      { vectorStore: mockVectorStore, embed: mockEmbed },
    );
  ...
  describe('environment data — new blocks', () => {
    ...
    it('includes cost suggestions when session_costs has expensive sessions', async () => {
      ...
      db.upsertSession({
        ...
        cwd: '/Users/test/myapp',
        project: 'myapp',
        summary: 'another expensive session',
        messageCount: 60,
        userMessageCount: 25,
        assistantMessageCount: 0,
        toolMessageCount: 0,
      ...
      db.raw
        ...
        .run('sCost1', 'claude-opus-4-5', 6.5, 100000, 150000, 0, 0);
      db.raw
        .prepare(
          `INSERT OR REPLACE INTO session_costs (session_id, model, cost_usd, input_tokens, output_tokens, cache_read_tokens, cache_creation_tokens) VALUES (?, ?, ?, ?, ?, ?, ?)`,
        )
        .run('sCost2', 'claude-opus-4-5', 7.2, 120000, 180000, 0, 0);
