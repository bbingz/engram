...
describe('lint_config', () => {
  ...
  describe('extractBacktickRefs', () => {
    it('extracts backtick-wrapped references', () => {
      const refs = extractBacktickRefs(
        'Run `npm run build` and edit `src/foo.ts`',
      );
  ...
  describe('looksLikeFilePath', () => {
    ...
    it('rejects URLs', () => {
      ...
      expect(looksLikeFilePath('https://docs.example.com/guide.html')).toBe(
        false,
      );
    });
  ...
  describe('handleLintConfig', () => {
    it('finds broken file references', async () => {
      ...
      const fileErrors = result.issues.filter((i) =>
        i.message.includes('src/nonexistent.ts'),
      );
    ...
    it('finds broken npm script references', async () => {
      const result = await handleLintConfig({ cwd: TEST_DIR });
      const npmWarnings = result.issues.filter((i) =>
        i.message.includes('deploy'),
      );
    ...
    it('does not flag valid references', async () => {
      const result = await handleLintConfig({ cwd: TEST_DIR });
      const validFile = result.issues.filter((i) =>
        i.message.includes('src/index.ts'),
      );
      expect(validFile.length).toBe(0);
      const validScript = result.issues.filter(
        (i) =>
          i.message.includes('npm run build') &&
    ...
    it('returns score of 100 for clean config', async () => {
      ...
      writeFileSync(
        join(cleanDir, 'CLAUDE.md'),
        'Run `npm run build` to compile.\nEdit `src/app.ts` for the entry.',
        'utf-8',
      );
    ...
    it('ignores path traversal references that escape project root', async () => {
      const traversalDir = join(
        tmpdir(),
        `engram-lint-traversal-${Date.now()}`,
      );
      mkdirSync(traversalDir, { recursive: true });
      writeFileSync(
        join(traversalDir, 'CLAUDE.md'),
        [
          '# Config',
      ...
      const traversalIssues = result.issues.filter(
        (i) =>
          i.message.includes('etc/passwd') ||
          i.message.includes('sibling/file.ts'),
      );
