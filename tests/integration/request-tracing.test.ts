...
import { createLogger, LogWriter } from '../../src/core/logger.js';
import { runWithContext } from '../../src/core/request-context.js';
import { Tracer, TraceWriter, withSpan } from '../../src/core/tracer.js';
...
describe('end-to-end request tracing', () => {
  ...
  it('correlates logs and traces within an MCP tool call', async () => {
    ...
    await runWithContext(
      { requestId: 'mcp-req-1', source: 'mcp' },
      async () => {
  ...
  it('separate ALS contexts get different request_ids', async () => {
    ...
    await runWithContext(
      { requestId: 'req-A', source: 'indexer' },
      async () => {
        log.info('file 1');
      },
    );
    await runWithContext(
      { requestId: 'req-B', source: 'indexer' },
      async () => {
        log.info('file 2');
      },
    );
    ...
    const logs = db.raw
      .prepare('SELECT trace_id FROM logs ORDER BY id')
      .all() as any[];
