...
describe('CIDR utilities', () => {
  it('ipToUint32 converts correctly', () => {
    ...
    expect(ipToUint32('198.51.100.100')).toBe(
      ((198 << 24) | (51 << 16) | (100 << 8) | 100) >>> 0,
    );
  });
  ...
  it('parseCIDR parses network and mask', () => {
    ...
    const c2 = parseCIDR('198.51.100.0/24');
    expect(c2.network).toBe(ipToUint32('198.51.100.0'));
  ...
  it('ipMatchesCIDR matches IPs within range', () => {
    ...
    expect(ipMatchesCIDR('198.51.100.1', cidrs)).toBe(false);
  });
  ...
  it('ipMatchesCIDR handles IPv4-mapped IPv6', () => {
    ...
    expect(ipMatchesCIDR('::ffff:198.51.100.1', cidrs)).toBe(false);
  });

  it('ipMatchesCIDR supports multiple CIDRs', () => {
    const cidrs = [parseCIDR('10.0.0.0/8'), parseCIDR('198.51.0.0/16')];
    expect(ipMatchesCIDR('10.1.1.1', cidrs)).toBe(true);
    expect(ipMatchesCIDR('198.51.5.5', cidrs)).toBe(true);
