...
import { AiAuditWriter, AiAuditQuery } from '../../src/core/ai-audit.js'
...
describe('AI Audit API', () => {
  ...
  describe('GET /api/ai/audit', () => {
    ...
    it('returns records when configured', async () => {
      writer.record({ caller: 'title', operation: 'generate', durationMs: 100, model: 'qwen2.5:3b' })
      writer.record({ caller: 'viking', operation: 'pushSession', durationMs: 200 })
    ...
    it('supports pagination', async () => {
      for (let i = 0; i < 5; i++) {
        writer.record({ caller: 'title', operation: 'generate', durationMs: i * 10 })
      }
  ...
  describe('GET /api/ai/audit/:id', () => {
    ...
    it('returns a single record by id', async () => {
      const id = writer.record({ caller: 'title', operation: 'generate', durationMs: 150, model: 'qwen2.5:3b' })
  ...
  describe('GET /api/ai/stats', () => {
    ...
    it('returns stats shape', async () => {
      writer.record({ caller: 'title', operation: 'generate', durationMs: 100, promptTokens: 500, completionTokens: 80 })
      writer.record({ caller: 'viking', operation: 'pushSession', durationMs: 200 })
