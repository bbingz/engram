...
describe('runAllHealthChecks', () => {
  ...
  it('returns correct schema: issues array, score 0-100, checkedAt ISO string', async () => {
    const result = await runAllHealthChecks(db, { force: true, shell: noopShell })
  ...
  it('force: true bypasses the cache and returns fresh result', async () => {
    ...
    const first = await runAllHealthChecks(db, { force: true, shell: noopShell })
    ...
    const second = await runAllHealthChecks(db, { force: true, shell: noopShell })
  ...
  it('cached result is returned within TTL when force is false', async () => {
    ...
    const cached = await runAllHealthChecks(db, { force: false, shell: noopShell })
  ...
  it('issues have correct shape when shell reports a problem', async () => {
    ...
    const staleBranchIssue = result.issues.find(i => i.kind === 'stale_branches')
