...
describe('queryTraces', () => {
  ...
  beforeEach(() => {
    ...
    stmt.run('t1', 's1', 'indexAll', 'indexer', '2026-03-22T10:00:00.000', '2026-03-22T10:00:02.500', 2500, 'ok')
    stmt.run('t2', 's2', 'vikingPush', 'viking', '2026-03-22T11:00:00.000', '2026-03-22T11:00:05.000', 5000, 'error')
    stmt.run('t3', 's3', 'ftsSearch', 'search', '2026-03-22T12:00:00.000', '2026-03-22T12:00:00.100', 100, 'ok')
  })
  afterEach(() => { db.close() })
  ...
  it('filters by slow threshold', () => {
    ...
    expect(result[0].name).toBe('vikingPush')
  })

  it('filters by name pattern', () => {
    const result = queryTraces(db.raw, { name: 'viking%' })
    expect(result).toHaveLength(1)
  })
