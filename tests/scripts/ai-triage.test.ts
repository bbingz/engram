...
import { createTriageProvider } from '../../scripts/ai-triage.js';
...
describe('VlmProvider.analyze', () => {
          ...
          choices: [
            {
              message: {
                content:
                  '{"verdict": "acceptable", "confidence": 0.95, "reason": "Font anti-aliasing difference"}',
              },
            },
          ],
        }),
    ...
    const result = await provider.analyze(
      {
        ...
        actual: Buffer.from('png'),
        diff: Buffer.from('png'),
      },
  ...
  it('returns uncertain on API error', async () => {
    ...
    const result = await provider.analyze(
      ...
      { ssim: 0.9, pixel_diff_percent: 2, phash_distance: 10 },
    );
  ...
  it('handles null diff buffer (size_mismatch)', async () => {
          ...
          choices: [
            {
              message: {
                content:
                  '{"verdict": "regression", "confidence": 0.9, "reason": "Size mismatch indicates layout change"}',
              },
            },
          ],
        }),
    ...
    const imageBlocks = body.messages[0].content.filter(
...
describe('needsTriage', () => {
  it('returns true for failed status', async () => {
    ...
    expect(
      needsTriage({
        status: 'failed',
        metrics: { ssim: 0.9, pixel_diff_percent: 2, phash_distance: 10 },
      } as any),
    ).toBe(true);
  });
  ...
  it('returns true for passed with low SSIM', async () => {
    ...
    expect(
      needsTriage({
        status: 'passed',
        metrics: { ssim: 0.97, pixel_diff_percent: 0.05, phash_distance: 1 },
      } as any),
    ).toBe(true);
  });
  ...
  it('returns false for passed with good metrics', async () => {
    ...
    expect(
      needsTriage({
        status: 'passed',
        metrics: { ssim: 0.99, pixel_diff_percent: 0.05, phash_distance: 1 },
      } as any),
    ).toBe(false);
  });
  ...
  it('returns false for new status', async () => {
    ...
    expect(
      needsTriage({
        status: 'new',
        metrics: { ssim: 0, pixel_diff_percent: 0, phash_distance: 0 },
      } as any),
    ).toBe(false);
  });
});
