...
- [修复] #234 已基于 `main@19fa698b` 把 lint-staged 17.0.8 升到 17.2.0；registry integrity/Node engine 已核对，lockfile 净减 473 行（8+/481−）来自上游移除 Listr2，picomatch 4.0.4→4.0.5 为共享 hoist。真实 staged 成功探针通过；部分暂存失败探针揭示 `.husky/pre-commit` 会被后续 `if` 吞掉 lint 失败，现加 fail-fast 及可执行回归，hook 已能非零阻断且完整保留 staged blob、unstaged sentinel、工作树哈希和既有 stash。clean `npm ci`、no-staged、build/typecheck/lint/knip 与 Node 全量 129 files / 1,509 tests 通过；audit 既有 6 项不含 lint-staged 必需依赖。
