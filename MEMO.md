...
- [验证] #234 已基于 `main@19fa698b` 把 lint-staged 17.0.8 升到 17.2.0；registry integrity/Node engine 已核对，lockfile 净减 473 行（8+/481−）来自上游移除 Listr2，picomatch 4.0.4→4.0.5 为 Knip/Vitest/Vite 共享 hoist。真实 staged 成功探针与实际 Husky `.husky/_/pre-commit` 部分暂存失败探针均通过：wrapper 以 `sh -e` 阻断、同时完整保留 staged blob、unstaged sentinel、工作树哈希和既有 stash；直接执行用户 hook 的 exit-0 不是 Git commit 路径，未保留 hook/test 改动。clean `npm ci`、no-staged、build/typecheck/lint/knip 与 Node 全量 128 files / 1,508 tests 通过；audit 既有 6 项不含 lint-staged 必需依赖。
