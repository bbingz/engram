...
- [验证] #235 exact head `4a0237eb` 经异家 review 获 `APPROVE / NO FINDINGS`，fresh PR Tests/CodeQL/Dependency Review 全绿后 squash merge 为 main `19fa698b`；post-merge Tests `30183278956` 与 CodeQL `30183278980` 全绿，full UI 31/31，未 rerun、未 deploy。此记录由 #234 仅作 durable-doc carrier，不属于 lint-staged 行为变更。
- [修复] #234 已基于 `main@19fa698b` 把 lint-staged 17.0.8 升到 17.2.0；registry integrity/Node engine 已核对，lockfile 净减 473 行（8+/481−）来自上游移除 Listr2，picomatch 4.0.4→4.0.5 为 Knip/Vitest/Vite 共享 hoist。真实 staged 成功探针通过；部分暂存失败探针揭示 `.husky/pre-commit` 会被后续 `if` 吞掉 lint 失败，现只定点传播 lint-staged 错误，并在隔离临时目录直接执行 hook，覆盖 lint 与 macOS drift guard 的成功/失败状态；既非零阻断又完整保留 staged blob、unstaged sentinel、工作树哈希和既有 stash。该 fail-closed 行为会在 npx/Node/npm PATH 不可用时有意阻断 commit，须先恢复依赖与 Git 客户端 PATH。clean `npm ci`、no-staged、build/typecheck/lint/knip 与 Node 全量 129 files / 1,512 tests 通过；audit 既有 6 项不含 lint-staged 必需依赖。
