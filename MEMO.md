...
- [变更] #266 已 rebase 到 `main@b8a1cb7a`：当前源码仍钉 27 个 MCP tools；Claude Code plugin 有 `.mcp.json`、三项 skills 与 `SessionStart`，但仍无 `Stop` / `SessionEnd` write-back hook。row 20 的窄验收只是给旧报告补 dated correction，因此在本分支转为 Landed，不代表 plugin P0 全部完成；镜像 backlog 更新为 22 landed + 2 partial + 11 open，仅文档。
