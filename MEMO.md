...
- [验证] #235 exact head `4a0237eb` 经异家 review 获 `APPROVE / NO FINDINGS`，fresh PR Tests/CodeQL/Dependency Review 全绿后 squash merge 为 main `19fa698b`；post-merge Tests `30183278956` 与 CodeQL `30183278980` 全绿，full UI 31/31，未 rerun、未 deploy。
- [变更] #234 已基于 `main@19fa698b` 把 lint-staged 17.0.8 升到 17.2.0；registry integrity 与 Node engine 已核对，482 行 lock 缩减来自上游移除 Listr2 终端依赖树。真实 staged TS 探针验证 Biome 格式化后重新暂存，探针已完整清理；no-staged、build/typecheck/lint/knip 与 Node 全量 128 files / 1,508 tests 通过。audit 既有 6 项不含 lint-staged 新依赖。
