...
- Five open PRs were recorded on the review board as mergeable but had a
  failing `swift-unit` and `CI Gate` on their current heads. All five failed at
  the same first step, `cd macos && xcodegen generate && git diff --exit-code
  Engram.xcodeproj`, so no test in those jobs ever ran. Confirmed against each
  head SHA via `gh api repos/bbingz/engram/commits/<sha>/check-runs`; no
  superseding successful re-run existed.
- Two distinct root causes. (a) New Swift files committed without the
  regenerated `project.pbxproj`: `PerfSignpost.swift` (#249),
  `MCPActivationSupport.swift` + `MCPActivationOnboardingTests.swift` (#252),
  `TranscriptAccessibilityTests.swift` + `UIUXPolishWiringTests.swift` (#248).
  Those files were outside the build, so their tests had never been compiled or
  executed in CI. (b) `project.pbxproj` regenerated by a local xcodegen 2.46.0
  while CI pins 2.45.4 (#245, #251); 2.46.0 emits an extra
  `EngramRemoteServerCore.framework in Embed Frameworks` entry and the opposite
  target ordering.
- Fixed by regenerating with the CI-pinned binary, installed through the repo's
  own `scripts/ci/install-xcodegen.sh` (SHA-256 verified): `8ce6b492` (#245),
  `d9359c27` (#248), `0d7484cf` (#249), `bb7bd2d8` (#251), `2c7b5be5` (#252).
  A control run in the main checkout produced an empty diff, confirming `main`
  matches the pin. After the fix, `PopoverUsageSectionTests`,
  `TranscriptAccessibilityTests`, `UIUXPolishWiringTests` and
  `MCPActivationOnboardingTests` executed for the first time and passed.
- With the gate cleared, #248 exposed a false positive in the pre-existing
  `ViewMainThreadReadTests.testFilterChangesUseCancellingTaskId`. The assertion
  forbade the bare substring `Task { await loadData() } }` anywhere in
  `TimelinePageView.swift`; the row-19 Retry action on the load-failure
  `AlertBanner` renders exactly that sequence. The `.task(id:)` guard the test
  protects was untouched. Anchored the assertion to the `.onChange` closure
  signature in `d3b31a39`, matching how the sibling `SessionsPageView`
  assertion was already written, and verified by mutation: reintroducing
  `.onChange(of: sortOrder) { _, _ in Task { await loadData() } }` still trips
  it.
- CodeQL raised two high `js/regex-injection` alerts on #245, visible only via
  `code-scanning/alerts?ref=refs/pull/245/merge`, at
  `scripts/check-adapter-format-drift.ts:862,879`. Injection is not reachable:
  `--format` must equal a monitored support-matrix key or the script exits
  (lines 790-798 and 806-810). The latent bug is real — a format name holding a
  regex metacharacter makes the accept path rewrite whichever matrix row sits
  above the intended one. Added an exported `escapeRegExp` and escaped both
  interpolations in `b632452f`, with a test that fails when the helper is
  reduced to the identity function.

### Added: local xcodeproj drift gate (2026-07-25)

- Added `scripts/check-xcodeproj-drift.sh`, which reads `XCODEGEN_VERSION` and
  `XCODEGEN_SHA256` straight from `.github/workflows/test.yml` so the pin has a
  single source of truth, refuses to run on any other xcodegen rather than
  emitting a `project.pbxproj` CI will reject, then regenerates and diffs the
  way `swift-unit` does. `--install` fetches the pinned build once into
  `~/.cache/engram/xcodegen` through the existing CI installer; the hook itself
  never touches the network. `.husky/pre-commit` invokes it only when `macos/`
