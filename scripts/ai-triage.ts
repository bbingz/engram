...
class KimiProvider implements VlmProvider {
  ...
  async analyze(
    ...
    content.push({
      type: 'image_url',
      image_url: {
        url: `data:image/png;base64,${images.baseline.toString('base64')}`,
      },
    });
    content.push({
      type: 'image_url',
      image_url: {
        url: `data:image/png;base64,${images.actual.toString('base64')}`,
      },
    });
    if (images.diff) {
      content.push({
        type: 'image_url',
        image_url: {
          url: `data:image/png;base64,${images.diff.toString('base64')}`,
        },
      });
    }
    ...
    try {
      ...
      if (err.name === 'TimeoutError')
        return makeUncertain(this.model, 'timeout', Math.round(elapsed));
      return makeUncertain(
        this.model,
        `API error: ${String(err).slice(0, 200)}`,
        Math.round(elapsed),
      );
    }
  }
  ...
  async analyze(
    ...
    content.push({
      ...
      source: {
        ...
        data: images.baseline.toString('base64'),
      },
    });
    content.push({
      type: 'image',
      source: {
        type: 'base64',
        media_type: 'image/png',
        data: images.actual.toString('base64'),
      },
    });
    if (images.diff) {
      content.push({
        ...
        source: {
          type: 'base64',
          media_type: 'image/png',
          data: images.diff.toString('base64'),
        },
      });
    }
...
export function needsTriage(result: {
  status: string;
  metrics: { ssim: number; pixel_diff_percent: number };
}): boolean {
