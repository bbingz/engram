...
> **前置**: `/Users/bing/-Code-/_项目扫描报告/engram_接管_项目目录操作_handoff.md`
> **路径**: A (全 TS 原生重写)
...
参照用户 `/Users/bing/-Code-/_项目扫描报告/CLAUDE.md` 的 `_archive/` 结构：
...
3. 多个 distinct cwd（很少但可能，比如 project 名相同但跨 `/Users/bing/-Code-/foo` 和 `/Users/bing/-Automations-/foo`）→ 弹 picker 让用户选
...
- 普通路径 `/Users/bing/-Code-/X` → `/Users/bing/-Code-/Y`
...
| 1 | +1 周 | `/Users/bing/-Code-/_项目扫描报告/mvp` 变 shim：`exec engram project move "$@"`；Python 代码搬到 `engram/archive/mvp-reference.py` 作 diff-test 锚点 |
...
- [ ] `/Users/bing/-Code-/_项目扫描报告/mvp` → shim: `exec engram project move "$@"`
...
离"完成"的定义：
1. `/Users/bing/-Code-/_项目扫描报告/mvp -y /old /new` 和 `engram project move -y /old /new` 的**产物 bytes 完全一致**（diff-test 全过；注意：必须走 Buffer 字节路径，不经 JSON.parse）
